...
| Value | Shape | Meaning |
| --- | --- | --- |
| metric input | `catalog.get("sales.revenue")` | A catalog metric object or its `SemanticRef`. Bare strings are rejected. |
| dimension input | `catalog.get("sales.orders.region").ref` | A catalog dimension ref, used for `dimensions`, `where` keys, and `axis`. |
...
| `AlignmentPolicy` | `mv.AlignmentPolicy(kind="window_bucket")` | How two windows are paired for `compare` / `correlate`. |
...
Keys are catalog dimension refs; values are a scalar (`==`), a list (`in`), or a
...
`AlignmentPolicy.kind` is one of `window_bucket` (default), `dow_aligned`,
`holiday_aligned`, or `holiday_and_dow_aligned`; the calendar-backed kinds also
take `calendar=mv.CalendarRef(...)`.
...
| `axis` | dimension ref | Yes | — | The segment axis to attribute over. |
