...
Use `catalog.list(...)` to discover refs, then inspect every metric, dimension,
and time dimension used by the analysis with `.details().show()`. The details
output is where authored `ai_context` appears; resolve scoped readiness blockers
