...
| dimension input | `catalog.get("sales.orders.region").ref` | A `DimensionRef` or `TimeDimensionRef` (see [Semantic refs](/en/latest/concepts/semantic-layer/#semantic-refs)). Used for `dimensions`, `where` keys, and `axis`. |
