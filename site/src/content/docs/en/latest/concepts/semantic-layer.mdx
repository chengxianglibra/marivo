...
Discovery returns evidence, signals, issues, and judgment targets. It does not
choose business meaning, unit, additivity, or the object to author. Use the
matching `ms.prepare_entity(...)` API next, author exactly one object, then run
`ms.verify_object(...)`.
