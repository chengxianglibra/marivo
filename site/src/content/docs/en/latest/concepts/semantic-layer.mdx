...
Discovery returns bounded evidence, signals, and issues — it does not choose
business meaning, unit, additivity, or the object to author. Agents read
discovery evidence through `.show()` / `.render()`; concrete result classes and
internal evidence fields are not public authoring inputs. `ms.help(...)` tells
the agent which parameters must be settled (the static authoring contract);
discovery supplies the runtime evidence. Author exactly one object, then run
`ms.verify_object(...)`.
