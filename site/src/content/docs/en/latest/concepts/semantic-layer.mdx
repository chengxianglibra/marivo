...
business meaning, unit, additivity, or the object to author. `ms.help(...)`
tells the agent which parameters must be settled (the static authoring
contract); discovery supplies the runtime evidence. Author exactly one object,
then run `ms.verify_object(...)`.
