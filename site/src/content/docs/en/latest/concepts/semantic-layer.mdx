...
| `source` | source builder | Yes | — | `ms.table(...)`, `ms.parquet(...)`, `ms.csv(...)`, or `ms.json(...)`. |
...
| `ms.json(path)` | `path` | `format` | Fixed-schema JSON files, glob patterns, or DuckDB httpfs `http(s)://` URLs. |
...
Consult `md.help("authoring")` for the staged datasource workflow
(register/test/inspect then discover -> settle -> author -> verify),
...
`md.inspect_table(...).show()` includes schema, comments, nullable flags,
partition hints, and any low-cost physical profile that the backend exposes
such as estimated rows or table size. Missing profile metadata is non-blocking.
