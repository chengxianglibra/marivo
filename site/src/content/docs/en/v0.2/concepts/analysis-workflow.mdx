---
slug: en/v0.2/concepts/analysis-workflow
...
readiness blockers in the [semantic layer](/en/v0.2/concepts/semantic-layer/)
before observing.
:::
...
| dimension input | `catalog.get("sales.orders.region")` | A catalog dimension object or its `DimensionRef` / `TimeDimensionRef` (see [Semantic refs](/en/v0.2/concepts/semantic-layer/#semantic-refs)). Used for `dimensions`, `where` keys, and `axes`. |
...
See [Evidence](/en/v0.2/concepts/evidence/) for the full model.
