---
slug: docs/v0.2/concepts/analysis-workflow
...
readiness blockers in the [semantic layer](/docs/v0.2/concepts/semantic-layer/)
before observing.
:::
...
| dimension input | `catalog.get("sales.orders.region")` | A catalog dimension object or its `DimensionRef` / `TimeDimensionRef` (see [Semantic refs](/docs/v0.2/concepts/semantic-layer/#semantic-refs)). Used for `dimensions`, `where` keys, and `axes`. |
...
entry point for "why did it change?" analysis. If a requested axis is missing
from the delta and the source observe/compare lineage is replayable, Marivo
materializes the expanded delta before decomposing it.
...
See [Evidence](/docs/v0.2/concepts/evidence/) for the full model.
