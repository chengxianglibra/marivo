---
slug: docs/v0.3/concepts/analysis-workflow
...
readiness blockers in the [semantic layer](/docs/v0.3/concepts/semantic-layer/)
before observing.
:::
...
| dimension input | `catalog.get("dimension.sales.orders.region")` | A semantic-id string, catalog dimension object, or its `DimensionRef` / `TimeDimensionRef` (see [Semantic refs](/docs/v0.3/concepts/semantic-layer/#semantic-refs)). Used for `dimensions`, `slice_by` keys, and `axis`. |
...
See [Evidence](/docs/v0.3/concepts/evidence/) for the full model.
