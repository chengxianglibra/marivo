...
| dimension 输入 | `catalog.get("sales.orders.region")` | semantic-id 字符串、catalog dimension 对象，或其 `DimensionRef` / `TimeDimensionRef`（见 [Semantic refs](/zh-cn/latest/concepts/semantic-layer/#semantic-refs)）。用于 `dimensions`、`slice_by` 的 key 和 `axis`。 |
