---
slug: zh-cn/v0.3/concepts/analysis-workflow
title: 分析流程
...
[语义层](/zh-cn/v0.3/concepts/semantic-layer/) 中解决 readiness blockers。
:::
...
| dimension 输入 | `catalog.get("dimension.sales.orders.region")` | semantic-id 字符串、catalog dimension 对象，或其 `DimensionRef` / `TimeDimensionRef`（见 [Semantic refs](/zh-cn/v0.3/concepts/semantic-layer/#semantic-refs)）。用于 `dimensions`、`slice_by` 的 key 和 `axis`。 |
...
完整模型见 [证据链](/zh-cn/v0.3/concepts/evidence/)。
