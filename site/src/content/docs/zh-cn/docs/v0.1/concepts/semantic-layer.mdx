...
- `mv.make_ref(id, kind)` —— 为给定类型构造对应的 per-类型子类（目录内部使用）。
- `as_ref_id(value)` —— 从 `SemanticRef`、`SemanticObject` 或纯 `str` 中提取 `.id` 字符串。
  对字符串容忍：原始 id 直接通过。

## `ai_context`：人与智能体之间的契约

每个语义对象都接受可选的 `ai_context` dict。业务含义和约束就放在这里
—— 这是智能体在使用对象前会读取的上下文。所有键都是可选的，但未知键会被拒绝。
...
| `ai_context` | `AiContext` | 否 | `None` | 面向智能体的上下文（见上）。 |
...
| `ai_context` | `AiContext` | 否 | `None` | 领域的面向智能体上下文。 |
...
| `ai_context` | `AiContext` | 否 | `None` | 面向智能体的上下文。 |
...
度量是你打算聚合的行级数量表达式（例如金额或数量）。和维度一样，它是一个返回
单个 ibis 表达式的装饰器 —— 但它携带可加性和可选的单位。

| 参数 | 类型 | 必填 | 默认 | 含义 |
| --- | --- | --- | --- | --- |
| `name` | `str` | 否 | 函数名 | 度量名称。成为 `<domain>.<entity>.<name>`。 |
| `entity` | `EntityRef \| str` | 是 | — | 所属实体。 |
...
### 来自 ibis 请求体的指标 —— `@ms.metric`

当数值是一个表达式时使用装饰器。请求体返回一个 ibis 聚合；你直接声明 `additivity`。
...
| `ms.weighted_average(name, value, weight)` | 两个引用 | 加权平均；之后 `decompose` 拆分混合与 rate |
...
就绪检查如何判断“是否就绪”，见 [就绪检查](/zh-cn/docs/v0.1/concepts/readiness/)。
分析如何记录其结论，见 [证据](/zh-cn/docs/v0.1/concepts/evidence/)。
然后继续阅读 [分析流程](/zh-cn/docs/v0.1/concepts/analysis-workflow/)。
