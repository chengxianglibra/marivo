...
用 `marivo init` 初始化项目（参见[安装](/zh-cn/docs/v0.1/installation/#初始化一个项目)），
...
你不必手写这些对象。`marivo init` 已经把 **`marivo-semantic` 技能** 安装到 `.claude/skills/`
和 `.codex/skills/`，因此在项目中运行的编码智能体（Claude 代码或 Codex）可以和你一起构建
语义层。把它指向你的表并提出请求：
...
- **遵循 ladder** —— 领域 → 实体 → 维度 → 时间维度 → 度量 →
  指标 → 关系 —— 让每个对象的依赖先于它存在。
- **先规划，再编写。** 对每个对象，智能体先调用 `ms.prepare_*` API（例如
  `ms.prepare_metric`），根据返回的 brief 分支判断，再把声明写入
...
缺失 `business_definition` 会阻塞分析，缺失 `guardrails` 会发出警告。完整的 `ai_context`
契约参见 [语义层](/zh-cn/docs/v0.1/concepts/semantic-layer/)。
...
[就绪检查](/zh-cn/docs/v0.1/concepts/readiness/)。
