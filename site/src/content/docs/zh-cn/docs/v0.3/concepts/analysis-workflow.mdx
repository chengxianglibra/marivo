...
[语义层](/zh-cn/docs/v0.3/concepts/semantic-layer/) 中解决 readiness blockers。
:::
...
| dimension 输入 | `catalog.get("dimension.sales.orders.region")` | semantic-id 字符串、catalog dimension 对象，或其 `DimensionRef` / `TimeDimensionRef`（见 [Semantic refs](/zh-cn/docs/v0.3/concepts/semantic-layer/#semantic-refs)）。用于 `dimensions`、`slice_by` 的 key 和 `axis`。 |
...
- `compare`、`attribute` 和 `forecast` 在 v1 中拒绝累计 frame。请使用 base 流量 metric
  执行这些意图——一个窗口内的累计差值等于该窗口内的 base 总量。
