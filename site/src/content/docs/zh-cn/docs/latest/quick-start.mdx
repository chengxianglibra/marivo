...
空目录也是有效的 Marivo 项目：在可选的 `marivo.toml` 覆盖默认值前，项目名使用目录名、不加载
外部语义层，并启用本地遥测。无需先执行 `marivo init`。
...
4. 目录能加载且没有语义缺口时，直接进入[第一次分析](/zh-cn/docs/latest/first-analysis/)。

> 使用项目现有的 Marivo 语义层，检查它是否已经能够支持已完成订单收入分析。
> 告诉我缺少的配置、语义对象或需要重新确认的业务口径；如果没有阻塞项，不要重建已有定义。

只有缺少必要对象、业务负责人要求修改口径，或者已编写的变更需要重新认证时，
才使用 `marivo-semantic`。

## 新项目

完成[安装](/zh-cn/docs/latest/installation/)，然后告诉智能体数据源、业务目标和已经确定的规则：

> 使用 `marivo-semantic`，为这个项目建立能够分析已完成订单收入的语义层。
> 把当前请求和项目中可归因且不冲突的文档作为权威。只有某个可复用业务选择仍未解决时，
> 才问我一个问题并在开始分析前停止。

## 需要确定的业务定义

通过当前请求、已批准的项目定义、可归因且不冲突的文档，或一个专项回答，确定收入包含和
排除什么、取消和退款如何处理、时间轴、币种和可加性、允许的拆分维度、限制和负责人。
数据模式与样本只能帮助智能体起草和提问，不能自行确定业务含义。
...
- 当前权威已经确定定义中的每个可复用业务选择；
- 智能体报告新模型的限定范围的编写认证没有阻塞项。

就绪检查用于认证刚刚完成编写的模型，不会自行产生业务权威，也不需要在每次分析前
重跑。没有未解决的可复用选择后，继续[让智能体完成第一次分析](/zh-cn/docs/latest/first-analysis/)。
