...
:::note[智能体发现]
从命令行开始，先用 `marivo --help` 查看 CLI 入口。要验证选定环境并获取 Python 交接，运行：

```bash
marivo doctor
```

使用安装了 `marivo` 的 Python 环境。doctor 报告中的版本号、Python 可执行路径、包路径和项目
状态构成环境指纹——从 `PATH` 解析到的裸 `marivo` 只有在指纹与分析解释器一致时才可信。进入
分析时只需验证一次该指纹。此后优先使用手中已有的公开对象：用 `.show()` 读取当前状态，在不熟悉的
组合操作前读取 `.contract()`。可以用 `marivo.help("analysis")` 打开分析索引；其中包含
...
会话。
会话的精简 API 为 `get_or_create`、`current()`、`recent()`、`inspect(name)` 和 `delete(name)`。

复用同名会话但传入不同的非空 `question` 会触发 `SessionQuestionMismatchWarning`，
并保留原 question——请传入新名称来开始新的分析，或省略 `question` 以恢复现有会话。
