...
| `mode` | `"joint" \| "hierarchy" \| "multiresolution"` | 否 | 多轴默认为 `"joint"` | 可加/组件允许 `joint` 或 `hierarchy`；不可加允许 `joint` 或独立 `multiresolution`。单轴调用会忽略该参数。合法组合与默认值以差值 admission 为准。 |
...
指标多轴调用默认采用 `mode="joint"`；需要 `hierarchy` 或 `multiresolution`
布局时再显式指定。单轴应省略 `mode`，即使传入也不会改变输出。多轴的有效
默认值也可从
...
`mode="multiresolution"` 表示“独立多分辨率归因”：每个有序轴 prefix 都是独立重算、
排名和对账的 game，完整结果不能跨解析求和。消费前用精确语义-引用 prefix
...
检查，包括 generic-v2 和 cumulative-flow 行契约。固定检查覆盖行契约、结果
