...
A knowledge base is a better home for durable context, but it cannot carry this responsibility alone. It still presents the agent with text to retrieve and interpret. A relevant passage may not be retrieved; conflicting versions may both appear; and the document itself cannot establish that a rule applies to the current question. Even when “revenue excludes refunds” is retrieved correctly, the text does not bind revenue to a particular entity, metric, expression, and timeline. Nor can it stop the analysis when a source field changes or the calculation no longer runs.

This does not make the knowledge base unimportant. It is well suited to policy background, rationale, exceptions, and worked examples. Stable definitions, however, need an executable semantic contract governed by code, version control, and runtime checks. The knowledge base supplies context; the semantic layer supplies a stable business identity that the agent can reference and the runtime can validate.
