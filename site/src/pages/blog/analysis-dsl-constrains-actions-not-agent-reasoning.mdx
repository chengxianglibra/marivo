...
### Run statistical checks and forecasts, then inspect quality

`correlate` measures the statistical association between two `MetricFrame` results. `hypothesis_test` runs a declared statistical test. `forecast` projects a historical frame into future intervals. They return `AssociationResult`, `HypothesisTestResult`, and `ForecastFrame` rather than collapsing everything into a generic analysis response. Quality is already evaluated when a frame is constructed and remains available through `frame.quality_summary` and `frame.contract().issues`; it is not a separate analysis action or result type.
...
The following example places the three actions beside the current frame's quality disclosures. `order_count_frame` uses the same period, grain, and region dimension as `current`:
...
These results and disclosures answer different questions: whether revenue and order count move together, whether the current and baseline difference passes a statistical test, how the next three periods might develop, and whether the current data has known quality concerns. None is a substitute for another.
