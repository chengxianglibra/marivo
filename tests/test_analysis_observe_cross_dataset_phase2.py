...
def _bootstrap_derived_ratio(tmp_path):
    ...
    (semantic_dir / "datasets.py").write_text(
        ...
        "    return orders.amount.sum()\n"
        "@ms.metric(datasets=[sessions, users], root_dataset=sessions, additivity='additive', decomposition=ms.sum(), name='session_count', verification_mode='python_native',)\n"
    ...
    (semantic_dir / "relationships.py").write_text(
        ...
        "ms.relationship(name='orders_to_users', from_dataset=orders, to_dataset=users, from_fields=[order_user_id], to_fields=[user_id])\n"
        "ms.relationship(name='sessions_to_users', from_dataset=sessions, to_dataset=users, from_fields=[session_user_id], to_fields=[user_id])\n"
    )
...
def _bootstrap_axis_unreachable(tmp_path):
    ...
    (semantic_dir / "datasets.py").write_text(
        ...
        "def gmv(orders, users):\n"
        "    return orders.amount.sum()\n"
        "@ms.metric(datasets=[sessions], additivity='additive', decomposition=ms.sum(), name='session_count', verification_mode='python_native',)\n"
...
def test_component_version_mismatch_raises_on_mode_difference(tmp_path):
    ...
    (semantic_dir / "datasets.py").write_text(
        ...
        "    return orders.amount.sum()\n"
        "@ms.metric(datasets=[sessions, user_profile_daily], root_dataset=sessions, additivity='additive', decomposition=ms.sum(), name='sessions_by_tier', verification_mode='python_native',)\n"
    ...
    (semantic_dir / "relationships.py").write_text(
        ...
        "ms.relationship(name='orders_to_profile', from_dataset=orders, to_dataset=user_profile_daily, from_fields=[order_user_id], to_fields=[profile_user_id])\n"
        "ms.relationship(name='sessions_to_profile', from_dataset=sessions, to_dataset=user_profile_daily, from_fields=[session_user_id], to_fields=[profile_user_id])\n"
    )
