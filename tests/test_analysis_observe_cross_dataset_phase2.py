...
def _bootstrap_derived_ratio(tmp_path):
    ...
    (semantic_dir / "datasets.py").write_text(
        ...
        "def orders(backend):\n"
        "    return backend.table('orders')\n"
        "@ms.dataset(name='sessions', datasource='warehouse', primary_key=['session_id'])\n"
        ...
        "@ms.metric(datasets=[orders, users], root_dataset=orders, additivity='additive', decomposition=ms.sum(), name='gmv')\n"
...
def test_component_version_mismatch_raises_on_mode_difference(tmp_path):
    ...
    (semantic_dir / "datasets.py").write_text(
        ...
        "def tier(user_profile_daily):\n"
        "    return user_profile_daily.tier\n"
        "@ms.metric(datasets=[orders, user_profile_daily], root_dataset=orders, additivity='additive', decomposition=ms.sum(), name='gmv_by_tier')\n"
...
def test_derived_components_can_span_datasources(tmp_path):
    ...
    (semantic_dir / "datasets.py").write_text(
        ...
        "def orders(backend):\n"
        "    return backend.table('orders')\n"
        "@ms.dataset(name='sessions', datasource='analytics', primary_key=['session_id'])\n"
        "def sessions(backend):\n"
        "    return backend.table('sessions')\n"
