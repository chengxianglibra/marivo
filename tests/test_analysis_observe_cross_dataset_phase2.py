...
def _bootstrap_derived_ratio(tmp_path):
    ...
    (semantic_dir / "datasets.py").write_text(
        ...
        "@ms.metric(entities=[orders, users], root_entity=orders, additivity='additive', decomposition=ms.sum(), name='gmv', )\n"
        "def gmv(orders, users):\n"
        "    return orders.amount.sum()\n"
        "@ms.metric(entities=[sessions, users], root_entity=sessions, additivity='additive', decomposition=ms.sum(), name='session_count', )\n"
...
def _bootstrap_axis_unreachable(tmp_path):
    ...
    (semantic_dir / "datasets.py").write_text(
        ...
        "def gmv(orders, users):\n"
        "    return orders.amount.sum()\n"
        "@ms.metric(entities=[sessions], additivity='additive', decomposition=ms.sum(), name='session_count', )\n"
...
def test_component_version_mismatch_raises_on_mode_difference(tmp_path):
    ...
    (semantic_dir / "datasets.py").write_text(
        ...
        "def tier(user_profile_daily):\n"
        "    return user_profile_daily.tier\n"
        "@ms.metric(entities=[orders, user_profile_daily], root_entity=orders, additivity='additive', decomposition=ms.sum(), name='gmv_by_tier', )\n"
        "def gmv_by_tier(orders, user_profile_daily):\n"
        "    return orders.amount.sum()\n"
        "@ms.metric(entities=[sessions, user_profile_daily], root_entity=sessions, additivity='additive', decomposition=ms.sum(), name='sessions_by_tier', )\n"
