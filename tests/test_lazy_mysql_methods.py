"""Real SELECT-only MySQL composed methods and retained state acceptance."""
...
def registry_for(database: str) -> tuple[Registry, CompiledExpressionSidecar]:
    registry, sidecar = scalar_registry(Path("unused"), engine="mysql")
    entities = {}
...
def method_database(monkeypatch: pytest.MonkeyPatch) -> Iterator[str]:
    ...
    with mysql.connection(admin=True) as connection, connection.cursor() as cursor:
        for name in ("orders", "customers", "lines", "snapshots", "validity"):
            cursor.execute(
                f"CREATE TABLE {database}{name} (id BIGINT, tenant TEXT, customer_id BIGINT, order_id BIGINT, amount DOUBLE, weight DOUBLE, region TEXT, channel TEXT, day DATE, start DATE, `end` DATE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_bin"
            )
        cursor.executemany(
            f"INSERT INTO {database}orders(id,customer_id,amount,weight,channel,day) VALUES (%s,%s,%s,%s,%s,%s)",
            [
        ...
        cursor.execute(f"INSERT INTO {database}customers(id,region) VALUES (1,'EU'),(2,'US')")
        cursor.execute(
...
def test_composed_state(
    tmp_path: Path, method_database: str, name: str, expected: list[float], total: float
) -> None:
    registry, sidecar = registry_for(method_database)
