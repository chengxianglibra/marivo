...
def test_comparison_releases_row_method_parts_before_next_stage(
    ...
    def collect(
    ...
    ) -> tuple[local_execution._Frames, int]:
        frame = pd.DataFrame(
            {
        ...
        return local_execution._Frames(frame, (), pa.Schema.from_pandas(frame)), len(frame)
    ...
    monkeypatch.setattr(local_execution, "_collect_input", collect)
    monkeypatch.setattr(local_execution, "execute_retained_suffix", row_step)
