...
def test_ir_has_no_io_imports() -> None:
    ...
    for attr_name in dir(mod):
        ...
        if module and any(module.startswith(f) or module == f for f in forbidden):
            pytest.fail(
                f"app.core.semantic.ir references I/O module {module!r} via attribute {attr_name!r}"
            )
...
def test_analysis_step_ir_table_name() -> None:
    from app.core.semantic.ir import AnalysisStepIR
...
def test_execution_plan_ir_semantic_resolution() -> None:
    from app.core.semantic.ir import ExecutionPlanIR, SemanticResolutionIR
...
def test_step_ir_from_mapping() -> None:
    from app.core.semantic.ir import AnalysisStepIR, step_ir_from_mapping
...
def test_request_from_session_payload() -> None:
    from app.core.semantic.ir import AnalysisRequest, request_from_session_payload
...
def test_typed_dict_classes_are_importable() -> None:
    ...
    from app.core.semantic.ir import (
        IrBundle,
        IrPlan,
        IrPlanHeader,
    )
