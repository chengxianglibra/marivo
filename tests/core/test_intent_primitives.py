...
def test_primitives_has_no_io_imports() -> None:
    ...
    mod_name = "marivo.core.intent.primitives"
    ...
    for attr_name in dir(mod):
        ...
        if module and any(module.startswith(f) or module == f for f in forbidden):
            pytest.fail(
                f"marivo.core.intent.primitives references I/O module {module!r} "
...
def test_step_category_for_reflects_taxonomy() -> None:
    from marivo.core.intent.primitives import STEP_TAXONOMY, step_category_for
...
def test_primitive_step_types_non_empty() -> None:
    from marivo.core.intent.primitives import PRIMITIVE_STEP_TYPES

    assert len(PRIMITIVE_STEP_TYPES) > 0


def test_atomic_intent_types_non_empty() -> None:
    from marivo.core.intent.primitives import ATOMIC_INTENT_TYPES
