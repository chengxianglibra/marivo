...
def test_preview_from_pandas_redacts_sensitive_values_and_warns_once() -> None:
    df = pd.DataFrame(
        {
            "customer_email": ["alice@example.com", "bob@example.com"],
            "phone": ["+1 415 555 1212", "not a phone"],
            "amount": [10, 20],
        }
    )
    preview = preview_from_pandas(
    ...
    assert preview.rows == (
        {"customer_email": "[redacted]", "phone": "[redacted]", "amount": 10},
        {"customer_email": "[redacted]", "phone": "[redacted]", "amount": 20},
    )
    assert [(warning.kind, warning.columns) for warning in preview.warnings] == [
        ("redacted_column", ("customer_email",)),
        ("redacted_column", ("phone",)),
    ]


def test_preview_from_pandas_redaction_can_be_disabled() -> None:
