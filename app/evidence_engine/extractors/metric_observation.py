...
class MetricObservationExtractor(ExtractorContract):
    ...
    - observation_type: observation type; defaults to ``metric_observation``
    ...
    as metric_query(single_window), which only requires
    ``("current_value", "current_sessions")``.
    """

    name = "metric_rows"
    artifact_type: ClassVar[str] = "metric_rows"
    observation_types: ClassVar[list[str]] = ["metric_observation"]
    ...
    def extract(
        ...
        if unmapped_required_keys:
            ...
            raise ValueError(
                "metric_rows extractor requires payload_fields mappings for "
        ...
        for index, row in enumerate(rows):
            ...
            if missing:
                ...
                raise ValueError(
                    "metric_rows extractor requires row fields for mapped payload keys "
