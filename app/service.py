...
class SemanticLayerService:
    ...
    def __init__(
        ...
        self.intent_registry.register("compare", self._run_compare_intent)
    ...
    def _run_compare_intent(self, session_id: str, params: dict[str, Any] | None) -> dict[str, Any]:
        ...
        left_artifact = self._resolve_artifact_for_ref(left_session_id, left_step_id)
        if left_artifact is None:
            raise ValueError(
                f"compare: STEP_NOT_FOUND - no committed artifact for step '{left_step_id}'"
            )
        right_artifact = self._resolve_artifact_for_ref(right_session_id, right_step_id)
        ...
        if fatal:
            raise ValueError(f"compare: NOT_COMPARABLE - {issues[0]['message']}")
        ...
        self._insert_step(step_id, session_id, "compare", summary, result, provenance=provenance)
        return result
