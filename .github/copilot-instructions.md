...
- Evidence packaging is a core design concept: sessions -> steps -> artifacts -> observations -> claims -> evidence edges -> recommendations. Deterministic extraction lives under `app/evidence_engine/`; language generation should explain or synthesize, not invent evidence structure. Final synthesis is explicitly layered: claim synthesis -> relation discovery -> causal promotion -> recommendation derivation -> edge materialization.
...
- Supported step types are `compare_metric`, `profile_table`, `sample_rows`, `aggregate_query`, `attribute_change`, `correlate_metrics`, and `synthesize_findings`. Session constraints are injected into the executable query steps.
