...
`semantic_kind` 沿用 `MetricFrameMeta` / `DeltaFrameMeta` 已有取值
（`scalar` / `time_series` / `segmented` / `panel`），不引入 `_delta` 后缀变体 —
`source_kind="delta_frame"` 已经足以区分 family，dispatch table 也按 `(source_kind,
semantic_kind)` 联合 gate；这与 `compare` / `decompose` 当前的判断方式一致。
...
selector_json        string                  JSON object; key=DimensionRef.id,
                                              value=scalar
...
recommended_followups_json  string           "[]" if none — per-item typed list
                                              (see §5.5 row parser)
```

JSON 列存储沿用现 `discover.py` 的 `keys_json` 模式（`reason_codes` / `source_refs` /
`selector` / `keys` / `peer_scope` / per-item `recommended_followups` 是 list 或 map，
parquet 不天然存）；`window` / `baseline_window` 用两列 datetime（已有
`AbsoluteWindow.start/end`）；`axis` 是单字符串列，直接对应 `DimensionRef.id`。

### 2.3 各 shape 必填列矩阵

| Shape | 必填列（在公共列 `item_id` / `score` / `reason_codes_json` / `source_refs_json` / `recommended_followups_json` 之外）|
|---|---|
...
列填 NA / `""` / `"[]"`、`recommended_followups_json` / `reason_codes_json` /
`source_refs_json` 列每行能解析（`recommended_followups_json` 通过
`FollowupAction.model_validate`，见 §5.5）。
...
`(objective, source_kind, semantic_kind)` 三元组到内部 scorer。`semantic_kind` 沿用
现有 `MetricFrameMeta` / `DeltaFrameMeta` 取值；`source_kind` 区分 metric vs delta
family。
...
| `period_shifts` | `delta_frame` | `time_series` / `panel` | `_score_period_shifts` | `period_shift` |
...
| `interesting_windows` | `delta_frame` | `time_series` / `panel` | `_score_interesting_windows` | `window` |
...
- 输入 `DeltaFrame`，`semantic_kind ∈ {"time_series", "panel"}`，按 `bucket_column`
  滚动窗口（`window_size = max(7, len // 10)`），对 `delta` 列计算窗口均值的 z-score。
...
- panel / `DeltaFrame[panel]`：先按时间 sum 一次再算（spec § "decompose" 同思路）。
...
- 输入 `MetricFrame` / `DeltaFrame`，`semantic_kind ∈ {"time_series", "panel"}`。
...
`SemanticKindMismatchError`。`select` 只接受 `CandidateSet`；`AttributionFrame` /
`MetricFrame` / `DeltaFrame` 上的 row selector 是后续 PR。

本轮不引入 `CandidateSet.projection()` 这层中间对象（当前 `BaseFrame` 没有
`projection()`，加它需要单独设计 projection 类型与契约）。`mv.select` + `as_<shape>()`
是仅有的 public surface。
...
| `slice` | ✗ | `dict[DimensionRef, Any]` | optional | ✗ | optional | `selector.<k>` / `keys.<k>` 标量 |
...
`select(field="selector")` 把 `selector_json` 反序列化后**重新装回
`{DimensionRef(dim_id): value}`** 形式，因为 `transform(op="slice", where=...)` 当前
`_resolve_slice_column`（`transform.py:1071`）只把 `DimensionRef` key 当作 dimension id
解析；裸字符串 key 会被当作列名查找，dimension id ≠ 列名时直接 `is not an axis column`
报错。返回 `dict[DimensionRef, Any]` 是与 transform 契约对齐的最小衔接成本。
...
喂。下面所有示例使用现有的 module-function API（`mv.compare(..., session=session)`
等）— `Session` 类目前只暴露 `jobs() / job() / frames() / close()`，不携带 fluent
operator 方法，本轮不新增 session-bound wrapper。

```python
session = mv.session.create(name="demo")

# spec § "不知道沿哪个轴看"
delta = mv.compare(current, baseline, session=session)
...
drivers = mv.decompose(delta, axis=selected_axis, session=session)  # 现有签名直收

# spec § "找异常并复查"
anomalies = mv.discover(series, objective="point_anomalies", session=session)
window = mv.select(anomalies, rank=1, field="window")               # -> AbsoluteWindow
local = mv.transform(series, op="window", window=window, session=session)
...
# selector == {DimensionRef("country"): "US", DimensionRef("platform"): "mobile"}
focus = mv.transform(delta, op="slice", where=selector, session=session)
```

`transform(op="slice", where=...)` 当前 `_resolve_slice_column`
（`transform.py:1071`）按 `DimensionRef` key 解析 dimension id；上面 `selector` 形式
即可直收，`select` 实现负责把 `selector_json` 还原为 `dict[DimensionRef, Any]`
（§4.2）。

### 4.4 `AttributionFrame` 的 selector 路径

spec § "组合方式" 的分层 drilldown 写法 `select(country_attr, ...)` 使用了 attribution
frame 上的 select。`AttributionFrame` 不是 `CandidateSet`，本次 `select` 严格只支持
`CandidateSet`；`AttributionFrame` 上的 select 推广是后续 PR 范围。

drilldown 在本轮实现里仍能工作：要么用 `mv.discover(delta, objective="interesting_slices")`
取 selector candidate，再 `mv.transform(delta, op="slice", where=selector)`；要么
agent 用 `country_attr.to_pandas().iloc[0][axis_column]` 直接读 top key。两条路线都
不依赖本轮新加的 select。
...
### 5.5 Per-item `recommended_followups` 解析

spec § "Candidate consumption protocol" 把 `recommended_followups` 列为 candidate item
的必填字段。本设计在 `_df` 层用 `recommended_followups_json` 列存储 JSON
（行：`'[{"action_id": ..., "kind": ..., ...}, ...]'`，无 followup 时 `'[]'`），并提供
内部 row parser：

```python
def _parse_item_followups(raw: str) -> list[FollowupAction]:
...
- `_validate_shape_columns(shape, df)` 写盘前对每行 `recommended_followups_json` 调一次
  `_parse_item_followups`，校验失败时抛 `FrameMetaInvalidError`（与不必填列检查同源）。
- `mv.select(candidate_set, rank=N, field="recommended_followups")` 返回
  `list[FollowupAction]`（typed list；空列即返回 `[]`）。
- `assess_quality(candidate_set)` 等下游 consumer 可以同样调用 row parser，无需各自
  写 JSON 解析。

`select` 的 field enum 因此扩成
`Literal["axis", "selector", "window", "baseline_window", "direction", "score",
"item_id", "recommended_followups"]`（外加 `keys.<k>` / `selector.<k>` 字符串路径）。

scorer 在本轮不主动产生 followup（一致于 §3.4：`recommended_followups_json` 默认
`'[]'`），但格式契约是固定的；future scorer / composite operator 可以填充而不需要
改 frame schema。
...
| `select(slice, field="selector")` | 返回 `dict[DimensionRef, Any]`，可直接喂 `mv.transform(op="slice", where=...)`（端到端断言一次到 `_resolve_slice_column` 通过）|
| `select(slice, field="keys.country")` | 返回标量 |
| `select(any_shape, field="recommended_followups")` | 返回 `list[FollowupAction]`；空列时 `[]`；含 followup 时 `isinstance(..., FollowupAction)` |
...
| Per-item `recommended_followups_json` 写入 + 读回 | 构造 candidate row 含 followup，`select(field="recommended_followups")` 回到原 typed list |
| 非法 per-item followup（缺 `kind`） | `_validate_shape_columns` 抛 `FrameMetaInvalidError` |
