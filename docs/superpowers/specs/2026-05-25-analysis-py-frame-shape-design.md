...
- `AlignmentPolicy` 必须为 `kind="calendar_bucket"`，其它 kind 抛
  `AlignmentPolicyNotApplicableError`，hint 提示
  "segmented compare 仅接受 AlignmentPolicy(kind='calendar_bucket')"。
...
- 完整支持现有 4 种 `AlignmentPolicy.kind`：`calendar_bucket`,
...
| ---------------------- | --------------- | ------ |
| `scalar` / `time_series` | `calendar_bucket` | 现行 `_align_and_compute` |
| `time_series` | 其它 calendar kind | 现行 calendar align 分支 |
| `segmented` | `calendar_bucket` | 新 `_align_segmented` |
...
| `AlignmentPolicyNotApplicableError` | segmented + 非 `calendar_bucket` |
...
- `panel × panel` happy path（`calendar_bucket`）。
