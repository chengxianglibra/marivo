...
| 入口 | 改动 | unit 语义 |
|---|---|---|
| `ms.dimension(kind="measure")` | 新增 `unit=` kwarg + `_validate_unit` + measure-only 守卫(照搬 `authoring.py:697-703` 的 additivity 守卫)+ 传入 `DimensionIR.unit` | 权威声明处 |
...
| `DimensionDetails` | **不加 unit**——镜像 additivity(声明在 measure IR、消费在 metric) |
...
| `DerivedMetricBrief.unit_hint` | 抽出共享代数 helper,brief 预览**等于** loader 真实推导("自动得 `1`,无需写"/"得 `None`,要就显式声明") |
...
- 共享代数:`_resolve_derived_unit` 与 `unit_hint` 预览同源,预览即真相。

## 测试策略

沿 `tests/conftest.py` / `tests/shared_fixtures.py` 共享 fixtures,先窄后宽。

1. **authoring**:`dimension()` 接受 measure 上的 unit、`_validate_unit` 拒绝非法值、categorical/time 上传 unit 报 measure-only 守卫错;`aggregate(unit=...)` override 现在真正落到 IR。
...
4. **validator**:linear `≥2` 互异已知单位报结构化错;override 不豁免;含 `None` 不报。
5. **catalog / help**:派生出的 metric unit 透出;`DimensionDetails` 无 unit 字段。
6. **richness**:count 类产生专属文案;tier-1-measure-未标注的文案指向 measure。
7. **unit_hint**:brief 预览与 `_resolve_derived_unit` 输出一致。
...
- 单计划顺序:IR 字段(+守卫)→ 授权(`dimension` kwarg + `aggregate` 修 bug)→ loader 三解析函数 →
  validator 规则 → richness 文案 → `unit_hint` 共享代数 → 测试。
