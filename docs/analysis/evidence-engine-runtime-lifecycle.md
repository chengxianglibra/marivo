...
`artifact` 在 typed intent 成功执行且结果通过该 step 的 canonical artifact contract 后进入后续 canonical write 路径。

创建要求：

- artifact payload 必须先通过自身 artifact contract 校验
- 对 `mandatory extraction artifact`，artifact 与 findings 必须作为同一次 committed canonical write 对外可见
- 对 `non-fact artifact`，是否单独 committed 由其自身 contract 决定，但它不进入 finding layer
- artifact 一旦进入 committed state，其 lineage、step ref、schema boundary 即固定
- 同一次 step 执行若产出多个 artifact，应各自拥有独立 `artifact_id`

#### 提交边界

对 `mandatory extraction artifact`，committed canonical state 的最小可见单元不是裸 `artifact`，而是：
...
- `mandatory extraction artifact committed with 0 findings` 非法
- proposition seeding 只能在 committed findings 上运行
...
若 `mandatory extraction artifact` 在 extraction gate 中失败：

- 该 artifact 不进入 committed canonical state
- proposition registration 不得启动
- assessment / action proposal 不得消费该 artifact 的中间态
- runtime 可保留 audit / retry 信息，但不得把它伪装成 committed canonical object

### Finding Lifecycle

#### 创建

`finding` 由单个 committed artifact 做 deterministic extraction 得到。
...
- `mandatory extraction artifact` 的成功 committed 前提是 finding set 非空
...
- 只有 finding extraction 成功并进入 committed state 后，系统才按适用的 deterministic seed templates 扫描 findings
