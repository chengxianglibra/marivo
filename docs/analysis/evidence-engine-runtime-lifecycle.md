...
本文档定义 Factum 证据引擎在目标态 canonical model 下的 runtime lifecycle 设计。

状态：draft design。本文是 `docs/analysis/` 下的运行时生命周期设计文档，不表示对应 HTTP endpoint、持久化表结构或当前实现已经完成。

## 目的

在 canonical evidence chain 已经收敛为：

`artifact -> finding -> proposition -> assessment -> action proposal`

之后，系统仍需要一份独立的 runtime lifecycle 规范，回答以下问题：

- 每一层 canonical object 何时创建、何时刷新、何时退出当前态
- 增量执行与 replay 时，哪些对象必须复用 identity，哪些必须产生新版本
- 当上游对象失效、不可解引用或被重新评估时，state 应如何演化
- 幂等、快照、最新态选择与读取面之间的责任边界如何划分

本文目标是把这些规则固定为目标态设计基线，使后续实现、评审与 API 绑定都共享同一套 runtime semantics。
...
目标态下，Evidence Engine 的 canonical runtime pipeline 固定为：

1. typed intent 执行并提交 artifact
2. 基于 artifact 做 deterministic finding extraction
3. 基于 findings 按 seed template 自动注册 proposition
4. 对受影响 proposition 执行 assessment recompute
5. 仅在 canonical assessment 输出发生变化时落新 snapshot
6. 基于 latest assessment 刷新 action proposal
7. state/context 读取层只暴露 latest/live canonical state

这条流水线体现以下边界：

- action surface 负责执行 typed intent，不直接定义 judgment state
- canonical object layer 负责事实抽取、命题注册、评估快照与动作候选刷新
- read surface 负责读取 latest/live objects，不回写 canonical object 本体

## Lifecycle 原则

### 1. runtime lifecycle 围绕 canonical objects，而不是围绕 step response

step response 可以携带 lineage、artifact ref 与 bounded projection，但它不是 canonical state 的权威来源。

canonical state 的运行时更新必须以对象层为主语：

- `artifact` 先提交
- `finding` 再抽取
- `proposition` 再注册
- `assessment` 再评估
- `action proposal` 最后刷新

### 2. identity boundary 与 version boundary 必须分离

不同 canonical object 的 identity 稳定边界不同：

- `artifact_id` 绑定单次 artifact lineage
- `finding_id` 绑定单个 artifact 内的 canonical fact item
- `proposition_id` 绑定 session-local judgment semantics
- `assessment_id` 绑定 proposition 下的具体 snapshot
- `action_proposal_id` 绑定 assessment snapshot 与 proposal payload semantics

这些对象的 schema/version 升级、提取器版本变化或读取 projection 变化，不得未经明确语义变化就改写对象 identity。
...
- 具体暴露方式包括 `missing_seed_finding_refs`、新的 gaps、support/oppose membership 收缩
...
- 读取层通过 `missing_seed_finding_refs` 暴露当前缺失
...
- `seed_findings` 只解引用 `seed_finding_refs`
- `missing_seed_finding_refs` 显式暴露 seed 当前不可读状态
...
9. seed finding 当前不可解引用时，proposition 仍可读取，且 `missing_seed_finding_refs` 非空。
