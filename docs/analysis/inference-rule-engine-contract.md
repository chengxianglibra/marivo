...
- 按 `gap_kind + missing_requirement` 的 requirement semantics 收敛 family-level candidates
- 为 candidate assessment materialize 当前 snapshot 的 `gap_memberships`

输出约束：

- `opened_gap_ids` / `resolved_gap_ids` 必须由具体记录驱动
- 缺口的打开、解决不得 out-of-band 写入
- `blocking` / `severity` 属于 snapshot-owned classification，不得回写到 `EvidenceGap` 本体
...
- 会阻止状态升级或稳定收敛的缺口，必须在当前 snapshot 的 `gap_memberships` 中标记 `blocking = true`
- 只提供警示、但不改变当前可用判断的缺口，必须在当前 snapshot 的 `gap_memberships` 中标记 `blocking = false`
