...
本文档总结 Factum 证据引擎（Evidence Engine）的目标抽象、分层边界与设计准则。
...
在 `docs/analysis/` 的类型化分析意图（typed analysis intent）设计下，证据引擎的职责应当是：

- 接住类型化意图（typed intent）产出的完整工件
- 以确定性方式抽取事实单元（finding）
- 把”事实””命题””评估””动作候选”分层表达
- 为 agent 提供低歧义、可规划、可局部读取的分析状态

本文的目标，是在后续调整证据引擎时，保证其：

- 与类型化意图（typed intent）/ 工件（artifact）/ 投影（projection）设计保持一致
- 保持证据边界清楚，避免概念漂移
- 优先服务 agent，而不是模仿人类分析报告
- 为未来意图（intent）扩展保留稳定抽象
...
它可以为 UI 或 agent 提供投影（projection），但这些投影不得反向定义底层证据语义。

### 动作候选（Recommendation）不是核心证据

动作候选（Recommendation / action proposal）属于外层动作支持接口，而不是核心证据主体。

证据引擎必须允许 agent 在完全绕过动作候选（recommendation）的情况下，仅依赖 facts + assessments 自主规划下一步动作。
...
在新的原子意图（atomic intents）设计中，`compare`、`decompose`、`detect`、`test`、`forecast` 的核心结果不都适合被视为 observation。

如果把所有类型化步骤（typed step）结果都强行压成 observation，会损失原始分析语义。
...
因此动作候选（recommendation）应是外层投影，而不是底层主语义。
...
推荐的规范抽象链路（canonical abstraction）为：

`artifact -> finding -> proposition -> assessment -> action proposal`

其中推断（`inference`）不是孤立的一层展示对象，而是驱动 proposition/assessment 演化的显式规则过程；在需要可审计、可回溯的规范状态时，应持久化为独立的推断记录（`inference_record`）。

## 抽象层级

### 1. 工件层（Artifact Layer）

工件（Artifact）是类型化意图（typed intent）执行后的完整工件。

职责：

- 保存完整、可复现、可审计的步骤结果
- 承载下游步骤引用的权威对象
- 为溯源信息（provenance）、调试、重放提供稳定基础

边界：

- 工件（artifact）不是 agent 的主决策接口
- 工件（artifact）不负责表达命题或动作建议
- 工件（artifact）不应为了上下文预算而牺牲语义完整性

### 2. 事实层（Fact Layer）

事实层推荐使用事实单元（`finding`）作为总类，而不是继续让 `observation` 统称一切事实单元。

事实单元（finding）表示从工件（artifact）中以确定性方式抽取出的、可被单独引用的事实单元。

`finding` 的具体规范 Schema 见 `docs/analysis/finding.md`。

可能的事实单元（finding）子类型包括：
...
- 表达系统确定知道的东西
- 保留类型化意图（typed intent）的原生结果差异
- 为后续命题（proposition）/ 评估（assessment）提供结构化证据输入

边界：

- 事实单元（finding）不应直接承载动作建议
- 事实单元（finding）不应直接声称因果
- 事实单元（finding）必须 deterministic、可重放、可引用

### 3. 判断层（Judgment Layer）

判断层必须把”命题本身”和”对命题的当前评估”分开。

#### 命题（Proposition）

命题（proposition）表示一个待评估的结构化命题。
...
- 提供稳定的推理对象
- 作为事实单元（findings）聚合、对抗、验证的中心对象

边界：

- 命题（proposition）不包含 confidence
- 命题（proposition）不包含 tentative / confirmed / insufficient 等状态
- 命题（proposition）表达的是”要判断什么”，不是”已经判断成什么”

#### 评估（Assessment）

评估（assessment）表示系统当前对命题（proposition）的评估状态。
...
- 显式表达不确定性、缺口与推进条件
- 随新事实单元（findings）到来而更新

边界：

- 评估（assessment）是会话内局部、时态性的推断状态
- 评估（assessment）不是不可变事实
- 评估（assessment）不应退化成只有一个总分的黑盒对象

### 4. 动作支持层（Action-Support Layer）

最外层是动作候选（action proposal），而不是动作候选（recommendation）作为核心证据层节点。

动作候选（action proposal）表示系统基于当前评估（assessments）给出的动作候选。
...
- 为 agent 提供可采纳的动作候选
- 显式说明动作服务于哪个命题（proposition）/ 评估（assessment）
- 暴露风险、成本、预期收益或信息增益

边界：

- 动作候选（action proposal）不是事实
- 动作候选（action proposal）不反向定义命题（proposition）或事实单元（finding）
- 动作候选（action proposal）应被视为投影（projection）/ policy output，而不是证据本体

## 关键实体边界

证据引擎至少要保持以下三条边界清楚：

### 1. 事实单元（`Finding`）和命题（`Proposition`）不能混

事实单元（finding）是事实单元；命题（proposition）是待评估命题。

如果一个对象既在说”观察到了什么”，又在说”因此得出什么结论”，说明抽象已经混层。

### 2. 命题（`Proposition`）和评估（`Assessment`）不能混

命题（proposition）是静态命题；评估（assessment）是系统当前对该命题的动态评估。
...
则通常说明命题（proposition）与评估（assessment）被错误地塞进了同一个实体。

### 3. 评估（`Assessment`）和动作候选（`ActionProposal`）不能混

评估（assessment）回答”当前判断到什么程度”；动作候选（action proposal）回答”下一步做什么可能最有价值”。
...
- 当前有哪些活跃命题（active propositions）
- 它们各自的评估（assessment）如何
- 哪些事实单元（findings）是核心依据
- 哪些缺口（gaps）阻塞了下一步
- 哪些动作候选最值得执行
...
agent 通常不会一次性消费整个证据图（evidence graph），而是围绕某个 metric、slice、意图（intent）或命题（proposition）拉取局部上下文。

因此证据引擎应支持围绕某个主题返回局部最小闭包：

- 相关事实单元（relevant findings）
- 目标命题（target proposition）
- 当前评估（current assessment）
- 阻塞性缺口（blocking gaps）
- 支撑工件引用（backing artifact refs）

### 5. 动作候选（Recommendation）只能是可选 shortcut

对 agent 来说，动作候选（action proposal）可以提高效率，但不能成为唯一可用的 planning 接口。

系统必须允许 agent：

- 不看动作候选（recommendation）
- 直接读取命题（proposition）+ 评估（assessment）
- 自主生成下一步计划

### 6. 所有判断都要能回溯到确定性证据和规则

评估（assessment）中的每个关键判断都应能回溯到：

- 哪些事实单元（findings）参与了评估
- 哪些规则被触发
- 哪些规则未满足
