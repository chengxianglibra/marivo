...
状态：draft design。本文是 `docs/analysis/` 下 state surface 的 canonical schema 提案，不表示对应 HTTP endpoint 或持久化模型已经实现。

## 目的

state surface 的职责，是把已经存在的 canonical objects 组织成稳定、可决策的 session 级主读取视图，而不是让 agent 回读完整 artifact、claim graph 或 narrative summary 自己反推当前状态。

设计目标：

- 以 `proposition + latest_assessment` 作为 session 级主决策骨架
- 复用既有 canonical objects，而不是发明平行 state object
- 保持 projection 可排序、可截断，但不重写 canonical semantics
- 为未来替换 `reflection-context` 提供更规范的主读取基线

## 核心设计决策

### 1. analysis state surface 是读取契约，不是新的核心证据层

canonical evidence chain 仍保持：

`artifact -> finding -> proposition -> assessment -> action proposal`

state surface 不新增新的核心证据实体；它定义的是如何把已有 canonical objects 组织成 agent 默认读取的稳定 session view。
...
- 其余 live evidence 通过 typed refs 指向顶层集合或 proposition-level context closure

这样做的原因是：

- agent 默认决策最依赖 proposition 与 current assessment
- finding / gap / inference record 仍保留 canonical object 身份
- 避免完全内嵌导致的重复载荷与跨条目不一致

### 4. v1 不把 session 级 action proposal 纳入主状态

尽管 `action proposal` 属于整体 analysis state surface 的外层 shortcut，但 v1 state surface 不把 `recommended_next_actions` 作为默认返回字段。

原因：

- 当前未承诺 session 级 proposal 默认排序 policy
- 主状态需要先把 proposition-centered 决策读取边界定稳
...
- `latest_assessment = null` 的 live proposition 仍必须作为 judgment-track object 可见，不得因默认排序或顶层截断被语义性降格为 inactive

`supporting_finding_refs` 与 `opposing_finding_refs` 只允许引用当前 `latest_assessment` 的 live evidence membership，不允许混入历史 snapshot 成员。
...
- session 主视图默认不内嵌完整 inference record payload，以避免跨 proposition 重复载荷
...
在未截断时，它是 `active_propositions` 当前 live evidence 所需 finding 的去重并集；当顶层截断作用于 `active_propositions` 时，它只允许覆盖 returned propositions 的 live evidence closure。

它允许包含：

- support / oppose findings
- proposition seed findings
- assessment / inference record 直接引用的 relevant findings

它不得包含：

- 仅用于展示的 summary item
- 与当前 state view 无关的 session 全量 finding
- 无 canonical identity 的 projection fragment
...
- 当顶层截断作用于 `active_propositions` 时，只允许覆盖 returned propositions 的 blocking gap closure
...
`StateArtifactRef` 只承担最小查找句柄（lookup handle）职责：
...
- 当顶层截断作用于 `active_propositions` 时，`artifact_refs` 只允许覆盖 returned propositions 的 live evidence closure
...
1. 存在 blocking gaps 的已评估 proposition 优先，按 blocking gap severity descending
2. 其余已评估 proposition 按 blocking gap count descending
...
- state surface 的主职责之一是暴露“已注册但尚未评估”的 judgment-track objects；convenience ranking 不得覆盖该目标
...
- 截断不得改变 canonical object identity
...
- `assessment_presence = "unassessed"` 仅匹配 `latest_assessment = null` 的 live proposition
...
本文定义的 state surface 目标上替换当前 `reflection-context` 作为 agent 默认主读取基线，但二者不等价：

- `reflection-context` 是 compact summary
- state surface 是 proposition-centered canonical state view
- readiness、tentative claims 与 compact evidence-gap summary 不并入 v1 canonical state object
...
- proposition-centered 主骨架需要先独立定稳
- agent 必须能够在完全绕过 proposal 的情况下完成下一步决策

这是一种阶段性收敛，不是否定 action proposal 在更完整 state surface 中的地位。后续若补入 proposal shortcut，应通过独立扩展恢复，而不是回写主状态骨架。

## Test Cases

后续实现至少应满足以下 schema-level 验收场景：

1. live proposition 尚未评估时，`latest_assessment = null`，且 assessment-derived refs 也为 `null`
...
12. 不允许在 canonical ref 位置使用裸字符串 locator
...
5. 不允许把 readiness、narrative summary 或 recommendation text 混入 canonical state object
6. 不允许把 `focus_subjects` 在截断状态下解释成全量 subject 索引
7. 不允许把 artifact lookup handle 误写成 artifact provenance payload
8. 不允许把被顶层截断排除的 proposition 所需 findings / gaps / artifacts 残留在 supporting collections
9. 不允许 `has_blocking_gaps = false` 继续承担 “no blocking gap or unassessed” 的混合语义
