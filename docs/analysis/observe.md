...
本文档定义 `observe` 分析步骤的拟议类型契约。

状态：draft design。本文是规划中的原子 `observe` 意图 Schema 提案，不表示对应 HTTP endpoint 已经实现。

## 目的

`observe` 是 Factum 中稳定的 evidence-entry step（证据入口步骤），用于在类型化观测契约下读取一个 semantic metric（语义指标）。
...
- 支持面向统计检验的 inferential-ready observation artifact
...
`standard` 是默认观测家族，沿用 scalar / time-series / segmented 三种输出分支；最终输出类型由 `granularity` 与 `dimensions` 决定。
...
返回面向下游 `test` 的数值型样本摘要，而不是普通业务指标值。
...
返回面向下游 `test` 的 rate 样本摘要，包含 `successes`、`trials` 和派生 `rate`。
...
已发布的 semantic metric 名称。该 metric 必须存在于语义层，并声明自己支持哪些：

- `time_scope.kind`
- observation modes
- inferential summary contracts

这也是 `observe` 是否可以产出 inferential-ready artifact 的能力边界。普通 scalar / time-series / segmented 输出不能在下游被隐式自动转成统计检验摘要。
...
仅在 `time_series` 模式中合法，表示返回序列的 bucket 大小。

### dimensions

仅在 `segmented` 模式中合法，表示按哪些 semantic dimensions 切分。空数组会归一化为 `null`。

segment 排序由 artifact contract 固定为：

1. `value desc`
2. dimension-key lexical order

artifact 必须返回完整 segment 集合；消费端若有 top-k、截断或其他排序偏好，应放在 projection layer。

## Artifact Identity 与 Versioning

本文档定义的是 `observe` 的完整 artifact semantics，而不是面向 agent 的压缩视图。

`observe` artifact 的 identity boundary 应至少绑定：

- 本次执行产生的 `artifact_id`
- 本次执行的 typed request 语义
- 本次执行所绑定的 source lineage

规则：

- `observe` artifact 绑定本次执行 lineage
- `observe` artifact 默认视为 immutable
- 重读同一 `artifact_id` 是读取同一 lineage 下的同一对象
- 重新执行 `observe` 即使请求语义相同，也应产生新 artifact，而不是与旧 artifact 隐式复用 identity

artifact contract 应显式区分：

- `schema_version`：`observe` artifact 自身的 contract version
- `metric_contract_version`：该 metric 被解释时的语义契约版本
- `derivation_version`：抽取与归一化逻辑版本

## Response Shape

所有响应都是 observation object，包含共享头部和类型特定 payload。
...
- time-series bucket 采用半开区间语义：`[window.start, window.end)`
...
- `quality_status` 只表达单边质量与可消费性提示，不表达双边 comparability 结论
...
- `ready`：当前 observation 满足默认消费条件，无已知硬阻断
- `needs_attention`：当前 observation 可消费，但存在必须显式暴露的质量告警
- `not_ready`：当前 observation 不满足安全消费条件，调用方不应把它当作无保留硬证据
...
- `compare` 应消费 `observe` 产出的 observation refs
- `correlate` 只应消费 `observe(time_series)`
- `test` 只应消费 inferential-ready 的 `observe` 工件
- `forecast` 只应消费完整的 `observe(time_series)` artifact，而不是 projection
- projection 不是下游 typed reference 的合法目标；下游步骤必须引用完整 artifact

## Artifact 与 Projection

本文档定义的是 `observe` 的完整 artifact semantics。

未来 agent-facing projection 可以：

- 截断 `segmented` 的长尾分段，并显式披露截断元数据
- 压缩长 time-series 的展示形式
- 用摘要形式重述样本统计
