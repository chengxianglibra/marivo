...
本文档定义 Factum 推断规则引擎中 `quality_gate` 规则族的拟议契约。

状态：draft design。本文是 `docs/analysis/` 下的规范 family-level 设计提案，不表示对应实现、持久化模型或 HTTP endpoint 已经存在。

## 目的

[`inference-rule-engine-contract.md`](inference-rule-engine-contract.md) 已定义 `quality_gate` 属于固定评估顺序中的门槛族，并约束其不能越权写最终判断。
...
其中 gap identity convergence、`open / keep / resolve / reopen` 生命周期，以及 snapshot-owned `blocking` / `severity` classification 的总规则，以 [`gap-management-contract.md`](gap-management-contract.md) 为准；本文只定义 `quality_gate` 如何贡献 requirement-level candidates、gap ids 与结构化质量影响。
...
- finding 的 `null` 语义必须复用 [`finding.md`](finding.md) 与 [`observe.md`](observe.md) 中已声明的单义约束，不得由 `quality_gate` 重新解释
...
- [`assessment.md`](assessment.md) 定义 `EvidenceGap`、`InferenceRecord`、`confidence_rationale` 的 canonical schema
- [`finding.md`](finding.md) 与 [`observe.md`](observe.md) 定义 quality fields 的单义语义
