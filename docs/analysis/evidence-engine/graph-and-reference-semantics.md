...
- mixed-version refs、tombstone 与 invalidation 如何影响读取完整性
...
历史层允许保留指向旧 version boundary 的 refs，但固定要求：

- externally visible latest bundle 不得混入当前 policy 下已失效的 identity boundary
- 若 ref 仍指向历史对象但当前 bundle 已不允许直接消费，应通过 migration / recompute 轨道修复，而不是由读取层静默重写

### Tombstone-first baseline

当 ref target 被 invalidated / redacted / controlled deletion 影响时：

- hard refs：不得静默降级；若 current latest closure 无法满足完整性，必须通过 bundle 回退、recompute 或显式 inconsistency 处理
- soft refs：允许目标当前不可解引用，但必须显式暴露 tombstone / missing target 状态

读取层不得因为 target 缺失而擅自：

- 删除 ref
- 改写 edge 语义
- 把 lineage 缺损伪装成“从未建立该关系”
...
- [`migration-and-invalidation.md`](migration-and-invalidation.md)：version 与失效治理
