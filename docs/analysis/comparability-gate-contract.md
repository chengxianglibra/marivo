...
[`inference-rule-engine-contract.md`](inference-rule-engine-contract.md) 已定义 `comparability_gate` 属于固定评估顺序中的门槛族，并约束其不能越权写最终判断。
...
其中 gap identity convergence、`open / keep / resolve / reopen` 生命周期，以及 snapshot-owned `blocking` / `severity` classification 的总规则，以 [`gap-management-contract.md`](gap-management-contract.md) 为准；本文只定义 `comparability_gate` 如何贡献 requirement-level candidates、gap ids 与结构化 comparability impact。
...
`anomaly_assessment` 与 `forecast_assessment` 不默认复用本 family；若未来需要，应先更新 `inference-rule-engine-contract.md` 的映射，再扩展本 family contract。
...
- 无论采用哪种来源，都不得重定义 [`compare.md`](compare.md) 或 [`test.md`](test.md) 中已声明的 canonical comparability schema
...
- `confidence_shaping` 必须复用 [`assessment.md`](assessment.md) 与 [`inference-rule-engine-contract.md`](inference-rule-engine-contract.md) 已声明的全局 guardrails
...
- [`compare.md`](compare.md) 与 [`test.md`](test.md) 定义 compare / test 上游 artifact 的 comparability 语义入口
