...
本文档定义 Factum 派生分析意图（derived analysis intents）的设计规则。

状态：draft design。本文是面向规划中派生意图家族及其 typed API contracts（类型化 API 契约）的设计指南，不表示文中所有意图都已实现。

## 目的

Factum 的 derived intent layer（派生意图层）用于暴露更高层的分析动作，同时避免退回到 ad hoc workflow（临时工作流）或 SQL-shaped API（SQL 形状 API）。
...
它们不是 Factum 可执行意图，而是给外部 orchestrator（例如 agent）的引导。
...
派生意图同样遵守 Factum 的 artifact / projection 分层。
...
任何面向用户可调用的 Factum intent 都必须在 bounded output（有界输出）下仍然有意义。
...
派生意图的请求和响应契约设计必须遵守 Factum 的 Canonical Schema 设计原则。
...
详见 [`attribute` 派生意图 Schema](attribute.md)。
...
详见 [`diagnose` 派生意图 Schema](diagnose.md)。
...
- 回答“观察到的差异是否通过了某个定义好的统计检验？”

详见 [`validate` 派生意图 Schema](validate.md)。

展开：

- 准备或解析 inferential-ready observations
- `test(left_ref, right_ref, hypothesis, method)`

为什么成立：

- hypothesis 是结构化的
- test method family 是类型化的
- 最终输出拥有稳定的 inferential contract
