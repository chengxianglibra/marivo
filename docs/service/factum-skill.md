...
本文档定义 Factum agent skill 的目标、边界、内容范围与发布形态。它是一份 agent 使用策略设计说明，不是 HTTP API 契约，也不是 MCP 工具清单。

Factum 仍然保持 HTTP-only。`factum-mcp` 是对规范 HTTP API 的客户端适配层；`factum skill` 是 agent 侧的使用守则与决策辅助层。两者配合使用，但职责必须严格分离。

## 背景

仅有 `factum-mcp` 时，agent 已经可以调用 Factum 的能力，但仍然容易出现以下问题：

- 选错 Factum surface，例如把 state surface 当作 action surface
...
这些问题不是协议映射问题，而是 agent 使用策略问题。因此需要单独的 `factum skill` 来约束 agent 如何正确使用 Factum。

## 目标

- 为 agent 提供 Factum 的正确使用策略，而不是再次定义协议
- 帮助 agent 在 action、state、context、semantic、infrastructure surfaces 之间做正确路由
- 为常见分析任务提供稳定的默认操作 loop
- 明确高频错误与反模式，降低错误调用率
- 在不复制 HTTP/MCP 契约的前提下，提高 agent 使用 Factum 的成功率和一致性
...
`factum-mcp` 解决的是“如何正确连接和调用 Factum”；`factum skill` 解决的是“什么时候该调用什么，以及哪些行为是错误的”。
...
- `factum-mcp`：默认必装，提供执行能力
- `factum skill`：默认推荐启用，提供使用策略

## 规范关系模型

Factum agent 使用的目标关系应为：

```text
User request
  -> Agent
    -> factum skill (decide how to use Factum)
    -> factum-mcp (execute MCP tools/resources)
    -> HTTP Factum (canonical execution boundary)
```

其中：

- `factum skill` 不直接产生证据
- `factum-mcp` 不决定调查策略
- HTTP Factum 仍然是唯一的规范执行边界
...
- `factum-mcp` 负责“能不能正确调用”
- `factum skill` 负责“应不应该这样调用”
...
当 skill 与 HTTP/MCP 文档出现不一致时，应以 HTTP 契约和 `factum-mcp` 可执行接口为准。Skill 只能做路由和守则，不能成为第二份协议真相。
...
本文档描述的是目标发布形态，而不是当前实现状态。`factum skill` 的发布目标分为三个层次。

### 1. 最小可发布目标

- 提供一个可被 agent 触发的 `factum` skill
- 明确说明 Factum 是 HTTP-only，MCP 是适配层
- 提供 surface 路由规则
- 提供一个默认操作 loop
- 提供常见错误与 guardrails
- 提供最小的“读下一份参考资料”索引
...
一个最小可发布的 `factum skill` 应至少包括以下模块。

### 1. Factum 是什么

需要明确：

- Factum 是 HTTP-only agentic analytics system
- MCP 是客户端适配层，不是第二执行面
...
- 用户需要使用 Factum 的 semantic layer 或 investigation surfaces
...
为了避免职责漂移，以下内容不应进入 `factum skill` 本体：
...
### 与 `factum-mcp` README 的边界

- `factum-mcp` README 定义安装、运行、MCP transport、工具暴露和环境变量
- skill 不承载运行时安装说明
...
- prompt 决定当前任务如何表述
- skill 决定 Factum 相关任务的稳定使用守则

## 维护规则

为避免 skill 漂移，维护时应遵守以下规则：

- HTTP 契约变化时，优先更新 API 文档与 `factum-mcp`
...
一个合格的 `factum skill` 至少应满足：

- agent 能判断何时启用 Factum
...
`factum skill` 是 Factum 的 agent-side 使用策略层，不是执行层，也不是协议层。

期望的稳态是：

- `factum-mcp` 提供唯一的可执行 MCP 接入面
- `factum skill` 约束 agent 如何正确使用这套接入面
- HTTP Factum 继续作为唯一的规范执行边界
- skill 与 MCP 之间保持清晰边界，不重复定义契约

这样既能保留 Factum 的 HTTP-first 架构，也能显著降低 agent 在真实分析任务中的误用率。
