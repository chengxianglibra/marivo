...
本文档描述的是目标发布形态，不代表当前仓库已经全部实现。文中出现的命令、配置片段和初始化流程，除非另有说明，均表示目标态 UX，不应被误读为当前版本 quickstart。
...
如果 agent 支持本地子进程 MCP，目标态下用户可在 MCP server 宿主环境中安装 `marivo-mcp`，再执行：
...
如果未来需要支持 “HTTP MCP + 本地自动托管”，则必须额外满足：

- HTTP MCP 服务端可以稳定获得唯一工作区根目录
- 服务端对该工作区有本地文件系统访问能力
- 服务端能够安全隔离不同工作区的本地运行时状态

在未满足上述条件前，HTTP MCP 默认应优先服务远程显式连接场景。
...
2. 加入工作区解析优先级与显式失败规则
3. 加入对工作区本地 `.marivo/runtime.json` 的发现与复用
4. 在 Marivo core 中加入稳定的 `marivo serve-local` 入口
5. 在 `marivo-mcp` 中加入 `marivo-mcp init` 统一初始化命令
6. 加入启动健康检查与超时处理
7. 加入自动初始化本地 `.marivo/marivo.yaml`
8. 在 `marivo-mcp` 和共享 agent 指南中记录统一的 agent 运行时流程
9. 补齐面向用户的安装、注册、部署命令与示例配置
