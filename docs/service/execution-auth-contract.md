...
本文档定义 Marivo 在 execution engine 侧读取数据时的身份认证与鉴权目标契约。
...
因此，Marivo 需要把 execution auth 收敛为三个分离层次：
...
- 把 secret manager / vault / token broker 的实现细节纳入 Marivo public contract
...
secret 的解析、换取、续约与注入，属于运行时责任。Marivo 不应要求外部调用方在 typed intent payload 中提交 access token、password 或 ticket 明文。
...
`engine auth policy` 用于定义某个 engine 在 Marivo 内应如何解释和使用身份。
...
- 当前 engine 在 Marivo 中默认按哪种模式取身份
...
- engine 无法直接按用户做原生鉴权
- 审计只能依赖 Marivo 自身日志，而不是 engine-side principal
...
- Marivo 使用 service credential 连接 Trino
- 同时把 `actor_ref -> effective_principal` 作为 proxy principal 注入

## 结论

Marivo 应把 execution-side 身份认证与鉴权收敛为四层：
...
这是在企业环境中同时满足“数据平面按用户鉴权”和“Marivo 保持稳定可治理 contract”的最小充分设计。
