...
本文是服务/运行时设计说明，不是当前 HTTP API 契约，也不是当前代码已经支持 `metadata.engine=mysql` 的声明。当前实现仍以 SQLite 为默认 metadata backend；后续 schema、storage、startup、tests 和 operator 文档实现必须以本文为 1.1 - 1.5 的冻结依据。
...
| `mysql` | 生产级共享 metadata backend | 多进程/多实例共享 control-plane metadata 的生产部署 | 待实现 |
...
任务 3.1 - 3.6 已落地为 DDL/bootstrap contract；真实 MySQL 连接、配置接入、连接池和 rollback 语义仍属于 4.x：
