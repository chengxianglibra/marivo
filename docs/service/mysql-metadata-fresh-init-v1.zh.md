...
- [`mysql-metadata-operator-runbook.zh.md`](./mysql-metadata-operator-runbook.zh.md) 说明空库创建、账号授权、MySQL 配置、启动验证、连接池 sizing、备份恢复和常见 fail-closed 错误处理。
- [`mysql-metadata-fresh-init-v1-decision-record.zh.md`](./mysql-metadata-fresh-init-v1-decision-record.zh.md) 记录 v1 non-goals 与后续扩展入口，避免把迁移工具、online migration、PostgreSQL 或 schema migration framework 误认为当前承诺。
