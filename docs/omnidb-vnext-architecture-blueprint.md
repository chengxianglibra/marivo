...
本文档给出 OmniDB 下一版本的目标架构蓝图。它不是对当前实现状态的描述，而是对下一阶段核心模块边界、执行链路和演进策略的设计提案。

目标是回答以下问题：

- OmniDB 的“核心分析内核”应该长什么样
...
HTTP、MCP、UI 只是协议与交互层，不承载分析领域逻辑。
...
MCP 应继续保持 thin proxy 风格，但可扩展为三类工具：
...
app/
  ...
  mcp/                # MCP models and tool wrappers only
  session/            # session manager, checkpoints
  planner/            # planning, validation, replanning
  semantic/           # semantic runtime and catalog services
  governance/         # policy engine, quality engine, approvals hooks
  analysis_core/      # IR, compiler, executor, artifact schemas
  evidence/           # extractors, synthesizers, scoring
  execution/          # routing, engine capability, translation, federation
  storage/            # metadata and analytics backends
  ui/                 # static files and UI-specific helpers
```

这不是要求一次性重构完成，而是建议的目标边界。
...
OmniDB vNext 的关键不是“再加几个服务”，而是建立一个清晰的分析内核，使 semantic、planning、governance、routing、evidence 都能围绕同一个中间表示协作。

一句话概括：

**把 OmniDB 从“结构良好的 MVP”推进为“以 IR 为中心的 agentic analytics runtime”。**
