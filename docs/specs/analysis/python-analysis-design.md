...
```text
frame.transform.<op>(metric_frame) -> metric_frame
frame.transform.<op>(delta_frame) -> delta_frame
```

`frame.transform` 是泛型 family-preserving 算子，仅作用于 `metric_frame` 与 `delta_frame`。它的输出 family 由输入 family 决定，不能通过 `op` 改变。`attribution_frame` 不提供 `transform` 入口。跨 family 派生必须使用命名算子：`metric_frame -> delta_frame` 只能通过 `compare`，`delta_frame -> attribution_frame` 只能通过 `decompose`。
...
| `attribution_frame`          | `decompose`                          | delta 的贡献分配                                    |
...
```python
mobile_dau = dau.transform.slice(
    slice_by={session.catalog.get("sales.orders.platform"): "mobile"},
)

top_declines = delta.transform.topk(
    by="delta_pct",
    limit=10,
)
```
...
local_series = series.transform.window(window=window)
```
...
country_delta = delta.transform.slice(
...
dropoff = dropoff_metric.transform.topk(by="dropoff", limit=10)
```
...
| Surface 2 session-bound | 中（关键节点） | 5 默认入口（`SessionKnowledge`、`ObservationSummary`、`Fact`、`OpenItem`、`BlockedFollowup`）+ 5 细分 typed fact（按需） | 综合判断、跨 step 推理、session 恢复 |
| Surface 3 object-bound | 低（审计） | 3：`Finding`、`Proposition`、`Assessment`（`ActionProposal` 不暴露，预留未来） | 回放、UI 绑定 |

Surface 2 的默认 agent surface 压缩为 `knowledge.observations() / facts() / open_items() / next_steps()`——agent 在常规分析中只需识别 5 类。
...
class SessionKnowledge:
    ...
    def observations(self) -> list[ObservationSummary]:
        """按 commit 顺序返回观察摘要：observe / derive_metric_frame 每次 commit 的有界 digest 投影。"""
...
### 观察摘要（`ObservationSummary`）

`facts()` 只承载「已判定的主张」（proposition + assessment 投影）。观察本身是 ground truth，不是待检验主张，不进入 assessment 生命周期，因此 `FactKind` 保持封闭、不新增 observation kind。但观察是综合判断与 session 恢复的证据基座，也是最高频算子（`observe`）的输出——Surface 2 必须能回答「本 session 观察到了什么」，否则 closeout 与跨进程恢复只能依赖 agent 上下文记忆或逐 frame 重读，违背 Surface 2 的职责定位。

来源与写入规则：

- `metric_frame` family（`observe` / `derive_metric_frame`）commit 时，除既有 per-value `metric_value` finding（仅 scalar / time_series，行为不变）外，**必须**追加恰好一条 `finding_type="observation"` 的 digest finding，覆盖全部四种 shape（scalar / time_series / segmented / panel）。
- digest finding 不 seed proposition、不产生 assessment；seeding 分派对它返回空。
- digest 在 commit 时由 DataFrame 同步确定性计算；payload 有界，大小与行数无关。

digest payload 按 shape 封闭分派（closed variants，不做 optional-field mega-class）：

| shape | 字段 |
| --- | --- |
| `scalar` | `value` |
| `time_series` | `bucket_count`、`first_bucket`、`last_bucket`、`first_value`、`last_value`、`min_value`、`max_value`、`mean_value`、`direction ∈ {increase,decrease,flat,undefined}`（末桶对首桶） |
| `segmented` | `segment_count`、`total_value`、`top_segments`（按 \|value\| 降序至多 5 项，含 `keys`、`value`、`share`） |
| `panel` | `bucket_count`、`segment_count`、`first_bucket`、`last_bucket`、`top_segments`（按各 segment 值总和的 \|·\| 降序至多 5 项） |

composition 语义门控：`total_value`、`share` 与 panel 的 `top_segments`（依赖跨桶求和）只在 metric
的 `additivity == "additive"` 时填充。ratio、weighted_average、percentile、sampled fold 等非可加
指标下这些字段为 None / 空，segmented 的按 \|value\| 排名与逐段取值保留。additivity 由
observe / derive_metric_frame 在 commit 时从语义层快照进 `MetricFrameMeta.additivity`
（`additive` / `semi_additive` / `non_additive`；缺失按非可加处理，宁缺毋错）。

`ObservationSummary` 字段（与 fact 家族同名字段同义；`metric_id` 经 `subject.metric` 表达，不重复设字段）：

| 字段 | 取自 |
| --- | --- |
| `id` | digest finding 的 `finding_id` |
| `subject`、`source_refs` | finding 行 |
| `window`、`semantic_kind`、`analysis_purpose`、`row_count` | finding payload（commit 时从 frame meta 快照） |
| `digest` | shape-dispatched payload |

投影与排序规则：

- `observations()` 直接投影自 findings 表（`finding_type='observation'`），不经过 proposition / assessment join——这是 Surface 2 中唯一不经 proposition 的默认区段，先例同 `OpenQuestion` 之于 blocking_issues。
- 按 `committed_at` 升序、`finding_id` 次序稳定排序。
- `for_subject(subject)` 按 subject canonical key 同样过滤观察摘要。
- 无 `status` / `confidence` / `latest_assessment_id` 字段——观察不是可检验主张。

兼容性：本节引入前 commit 的 metric_frame artifact 没有 digest finding，`observations()` 对这些 artifact 返回空缺；不做回填。
...
| spec 能力 | 落地 | 说明 |
| --- | --- | --- |
| finding 抽取（7 个 family + observation digest） | ✅ 全实现 | 严格遵循本文 finding 抽取规则；`metric_frame` commit 额外追加一条 observation digest finding（见[观察摘要](#观察摘要observationsummary)） |
