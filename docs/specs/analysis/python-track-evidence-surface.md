...
for action in knowledge.next_steps(top=5):
    if action.operator == "assess_quality":
        frame = mv.load_frame(action.input_refs[0], session=session)
        result = session.assess_quality(frame)
    elif action.operator == "decompose":
        delta = mv.load_frame(action.input_refs[0], session=session)
        result = session.decompose(delta, axis=mv.DimensionRef(id=str(action.params["axis"])))
    else:
