...
状态：target design。本文定义 `marivo.analysis_py` 暴露给通用 agent（Claude Code、Codex 等）的 evidence 访问面、Python 轨道实现边界，以及它与 [Python Analysis 算子集](python-analysis-operator-design.md) 和 [Evidence Engine 规则](evidence-engine/overview.md) 的衔接方式。

Python 轨道是 evidence-engine spec 的目标实现，但它是 **Python-native independent implementation**：实现代码落在 `marivo.analysis_py` 内，不直接依赖旧 `marivo.runtime.evidence`、`marivo.core.evidence`、`marivo.adapters.*` 或 MCP / HTTP contract。旧 evidence 代码只允许作为一次性 copied-code reuse 的来源；复制后必须改成 Python 轨道类型、异常和存储边界。

MCP / HTTP 等外部 transport 是历史遗留，目标态会下线；本文不维护与那条链路的一致性，也不要求双写、兼容读取或数据迁移。
...
- MCP / HTTP / CLI transport（目标态下线）
- 旧 MCP / HTTP evidence 链路的数据迁移、双写兼容或 wire parity
...
| [`evidence-engine/overview.md`](evidence-engine/overview.md) | evidence chain 总览 | 本文是该规则在 Python 轨道的实现 |
| [`evidence-engine/artifact-finding-generation-rules.md`](evidence-engine/artifact-finding-generation-rules.md) | finding 抽取规则 | 本文遵循 |
| [`evidence-engine/finding-proposition-seeding.md`](evidence-engine/finding-proposition-seeding.md) | seed template registry | 本文遵循 |
| [`../../superpowers/specs/2026-05-27-python-track-evidence-design.md`](../../superpowers/specs/2026-05-27-python-track-evidence-design.md) | 本文的落地架构设计 | 固定 Python-track package 边界、copied-code reuse 策略、测试范围 |
...
agent 必须能仅通过算子和 read method 完成完整工作流，不强制学习 evidence engine 内部分层。
...
`marivo.analysis_py.evidence` 是 Python 库唯一 evidence runtime。实现层不得 import 旧 runtime / core / adapter / transport 模块，也不得通过 adapter repository 绕过 Python-track isolation。
...
复制后的代码必须改成 `analysis_py` frame/meta/lineage 输入、`AnalysisError` exception taxonomy、session-local `judgment.db` repository，并纳入 import-linter 边界。旧 action proposal policy、server evidence repositories、publish switch、MCP / HTTP projection、旧 runtime replay recovery 不进入 Python 轨道。
...
delta.recommended_followups   # 只含 C1 + C2
delta.blocking_issues
delta.quality
delta.evidence_status
```

**不存在** `delta.evidence.*` 嵌套命名空间。所有 Surface 1 字段都是 top-level。
...
### `result.quality` 与 `session.assess_quality(...)` 的边界

| 项 | `result.quality` | `session.assess_quality(result)` |
| --- | --- | --- |
...
- 进程重启后 in-memory result 丢失，无法通过 `session.findings()` / `session.knowledge()` 找回
...
1. 下游 operator 可仅以当前 artifact + **默认参数** / **该 operator 自身的封闭枚举参数**执行——不需要 runtime 推断新的 ref（如 `decompose` 的 `axis: DimensionRef`，runtime 不挑），不需要 runtime 推断 policy（如 `compare` 的另一侧 `metric_frame`，runtime 不配对）
...
| `MetricFrame[time_series]` | `discover(objective="point_anomalies")`、`discover(objective="interesting_windows")`、`forecast(horizon=default)`、`assess_quality` |
| `MetricFrame[segmented]` | `discover(objective="interesting_slices")`、`discover(objective="cross_sectional_outliers")`、`assess_quality` |
| `MetricFrame[panel]` | `discover(objective="point_anomalies")`、`discover(objective="cross_sectional_outliers")`、`discover(objective="interesting_windows")`、`forecast(horizon=default)`、`assess_quality` |
| `DeltaFrame[*]` | `discover(objective="driver_axes")`、`discover(objective="period_shifts")`（time_series_delta / panel_delta）、`discover(objective="interesting_slices")`、`assess_quality` |
...
| `ForecastFrame` | `evaluate_forecast`（仅当 session 中已有同 metric / 同 horizon 的 actual `MetricFrame`）、`assess_quality` |
| `ForecastEvaluationResult` | `assess_quality` |
| `QualityReport` | （空——不再续接） |

**显式不发 C1 followup 的 operator**：

- `decompose`：需要 `axis: DimensionRef`，属于 C3 enumeration
- `compare`：需要另一侧 `metric_frame`，runtime 不配对
- `correlate`：需要另一侧 frame + `LagPolicy`，runtime 不挑
...
"已执行"判定靠 step lineage 中的 `triggered_by_followup` 字段（见 [Followup execution lineage](#followup-execution-lineage)）。
...
```python
result = session.run_followup(action: FollowupAction) -> Result
```

`session.run_followup` 内部按 `action.operator` dispatch 到对应算子（`session.observe / compare / decompose / discover / correlate / test / forecast / evaluate_forecast / assess_quality`），把 `action.input_refs / action.params` 作为参数。执行成功后，新 step 的 lineage 中写入 `triggered_by_followup={action_id, source_artifact_id}`，供 `next_steps()` 去重消费。

所有 followup 已是 C1 / C2 可靠类，**不需要** `accept_class` 参数。

agent 也可以直接按算子展开调用（`session.decompose(delta, axis=action.params["axis"])`），但这种写法**不会自动写入** `triggered_by_followup`，`next_steps()` 可能在下一次推荐时把它当成未执行。建议优先用 `run_followup`。
...
Python 轨道暴露的 `Finding` / `Proposition` / `Assessment` 字段集**遵循** [`evidence-engine/schemas/`](evidence-engine/) 规则，Python 表达层约束：
...
Python 轨道是 evidence-engine spec 的实现，主动收窄实现范围以控制规模。下表是 spec 全集 vs 实际落地：
...
| spec 能力 | 落地 | 说明 |
| --- | --- | --- |
| finding 抽取（7 个 family） | ✅ 全实现 | 严格遵循 [artifact-finding-generation-rules.md](evidence-engine/artifact-finding-generation-rules.md) |
| seed template registry（T1-T6） | ✅ 全实现 | 严格遵循 [finding-proposition-seeding.md](evidence-engine/finding-proposition-seeding.md) |
...
| 跨 session 引用 | ❌ session-local | 与 evidence engine baseline 一致 |
| `agent_authored` proposition | ❌ 仅 system-seeded | |
| `discover` 多 shape seeding | ❌ 仅 anomaly | 等 `discover` 多 shape finding subtype 设计 |
| `forecast_evaluation_result` finding family | ❌ 不实现 | `evaluate_forecast` operator 可用，但不进 evidence chain；等 finding family 设计 |
...
<project_root>/.marivo/analysis/
  ...
  └── sessions/<session_id>/
      ...
      ├── jobs/<job_id>.json           # existing analysis_py job records
      ├── frames/<artifact_id>/
...
CREATE TABLE followups (
  ...
  executed_step_id     TEXT,                     -- set when run_followup completes
  created_at_us        INTEGER NOT NULL
);
...
| `proposition_id` | per [finding-proposition-seeding.md Phase 6](evidence-engine/finding-proposition-seeding.md) identity normalization | 跨 replay 稳定 |
...
实现位置：`analysis_py/_subject.py::canonical_key(subject)`。
...
`session.run_followup(action)` 自动填充，`via="run_followup"`。agent 手动按算子展开时**不会自动填充**；可显式传 `session.compare(..., _triggered_by=action)`，`via="manual"`。

`next_steps()` 去重时识别两种 `via`。
...
agent: session.compare(current, baseline)
          ...
          ▼
   ┌──────────────────────────────────────────────────┐
   │ analysis_py commit_result(...) pipeline          │
...
`result.confidence_scope` 是**暴露字段**，不是 runtime 自动门控。agent 在跨 result 推理时**应**调 `confidence_scope.compatible_with(other)` 自查；runtime 不会自动拒绝 step。
...
class ConfidenceScope:
    ...
    definition_versions: Mapping[MetricRef, str]
...
[`marivo-skill/marivo-py-analysis/SKILL.md`](../../../marivo-skill/marivo-py-analysis/) 应仅写入：

- 算子集
- `session.knowledge()` 与 `knowledge.facts/open_items/next_steps()` 默认入口（4 类）
- result 扁平字段名 + walkthrough 示例
- `session.assess_quality(...)` 与 `result.quality` 的区分
- `session.run_followup(action)` 用法
...
SKILL.md 必须使用 [python-analysis-operator-design.md](python-analysis-operator-design.md) canonical 写法：`session = analysis.session()` + `session.observe(time=, grain=)`。不容忍两套并行写法。
...
- `analysis_py.session.session()` + `observe` + `compare` 三个 operator hook 进 executor
...
- `session.run_followup(action)` typed dispatch（仅支持 `observe` / `compare` / `assess_quality` 几个 operator）
- `triggered_by_followup` lineage 写入

**验收**：完整脚本 `observe` → `compare` → `knowledge.facts(kind="change")` → `next_steps()` → `run_followup` 跑通；followup 生成规则 conformance test 通过。
...
- `decompose` / `discover` / `correlate` / `test` / `forecast` / `evaluate_forecast` / `assess_quality` / `transform` 全部接入 executor pipeline
- C1 白名单表补齐其余行（`MetricFrame[scalar/segmented/panel]`、`AttributionFrame`、`CandidateSet`、`AssociationResult`、`HypothesisTestResult`、`ForecastFrame`、`ForecastEvaluationResult`）
...
- `BlockedFollowup` 计算
- `for_subject()` 过滤
- `session.findings` / `propositions` / `assessments` 查询 API
...
- `marivo-skill/marivo-py-analysis/SKILL.md` 迁移到 `session.observe(time=, grain=)` canonical 写法
...
- [`evidence-engine/overview.md`](evidence-engine/overview.md)
- [`evidence-engine/artifact-finding-generation-rules.md`](evidence-engine/artifact-finding-generation-rules.md)
- [`evidence-engine/finding-proposition-seeding.md`](evidence-engine/finding-proposition-seeding.md)
- [`evidence-engine/schemas/finding.md`](evidence-engine/schemas/finding.md)
- [`evidence-engine/schemas/proposition.md`](evidence-engine/schemas/proposition.md)
- [`evidence-engine/schemas/assessment.md`](evidence-engine/schemas/assessment.md)
