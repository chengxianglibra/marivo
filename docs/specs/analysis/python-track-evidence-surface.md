...
SKILL.md 必须使用 [python-analysis-operator-design.md](python-analysis-operator-design.md) canonical 写法：`session = analysis.session.get_or_create(...)` + `session.observe(window=..., dimensions=...)`。不容忍两套并行写法。
...
- `marivo-skill/marivo-py-analysis/SKILL.md` 迁移到 `session.observe(window=..., dimensions=...)` canonical 写法
