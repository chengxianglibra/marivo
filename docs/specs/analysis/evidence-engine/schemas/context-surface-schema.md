...
本文档定义 Marivo 在 agent-first 读取路径中的上下文面（`context surface`）拟议类型契约。

状态：draft design。本文是 `specs/analysis/` 下 上下文面 的 canonical schema 提案，不表示对应 HTTP endpoint 或持久化模型已经实现。
...
- seed hydration、assessment-derived refs 的 closure integrity 与 dangling 处理语义以 [`evidence-engine/graph-and-reference-semantics.md`](../graph-and-reference-semantics.md) 为准
...
对 `applied_inference_records` 的 `rule family` grouping 与版本解释，必须复用 [`rule-registry-contract.md`](../rules/rule-registry-contract.md) 中定义的稳定 registry，不得由 上下文面 自行发明分类。
