...
状态：draft design。本文是 `specs/analysis/` 下 状态面 的 canonical schema 提案，不表示对应 HTTP endpoint 或持久化模型已经实现。
...
- assessment-derived refs 的 closure integrity 与 dangling 处理语义以 [`evidence-engine/graph-and-reference-semantics.md`](../graph-and-reference-semantics.md) 为准
...
`rule family` 的汇总来源必须是 [`rule-registry-contract.md`](../rules/rule-registry-contract.md) 中定义的稳定 registry，不得由 `rule_id` 字符串前缀、目录名或实现类名推断。
