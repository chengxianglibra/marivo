...
| `forecast_evaluation_result` | `evaluate_forecast`                 | forecast vs actual 的预测效果评估                     |
...
| `evaluate_forecast` | `evaluate_forecast(actual=MetricFrame[time_series | panel], forecast=ForecastFrame, alignment=AlignmentPolicy) -> ForecastEvaluationResult` |
...
`evaluate_forecast` 是 core operator，不是 composite：它有独立 canonical family `forecast_evaluation_result`，并使用专用 `AlignmentPolicy(kind="forecast_horizon")` 对齐 forecast origin、horizon index 和 submission timestamp，无法由其它 core operator 加 typed policy 平替。`forecast_frame` 不能直接进入普通 `compare`：普通 `compare` 比较两个已观测事实面；`evaluate_forecast` 明确处理 predicted-vs-observed，输出误差、区间覆盖、bias、drift signal 和可跟进建议。
...
显著性验证统一走 `test`。Forecast 风险若能由 forecast 自身计算，应进入 `forecast_frame` metadata；若要比较 actual vs forecast，应使用 core operator `evaluate_forecast`；若需要更长的多步诊断，应作为 composite operator。
...
1. **不可平替**：不能由单个 core operator 加一个 typed policy（`AlignmentPolicy`、`LagPolicy`、`SamplingPolicy`、`PromotionPolicy` 等）平替。
...
只满足"高频"或"省键盘"不够。那种情况下应该使用 `analysis.session()` step-wise 写法，让 agent 在中间结果之上自己决定下一步。
...
| 排除项 | 等价路径 | 排除原因 |
| --- | --- | --- |
| `validate` | `observe + observe + test(..., sampling=SamplingPolicy(...))` | 3 步纯胶水，没有 sampling policy 之外的隐藏约束。 |
| `lag_scan` | `correlate(..., lag_policy=LagPolicy(mode="sweep", ...))` | typed `LagPolicy` 已覆盖。 |
| `driver_axis_scan` | `discover(objective="driver_axes", search_space=[...])` | 与现有 `discover` objective 字面等价。 |
| `metric_profile` | `metric.profile()` projection | projection 不产生 canonical artifact family，应使用 §Projection / read methods。 |
| `evaluate_forecast` | 直接调用 core operator `evaluate_forecast` | 有独立 canonical family 与独立 `AlignmentPolicy(kind="forecast_horizon")`，不是 DAG 组合；见 §最终算子表。 |
...
| `evaluate_forecast` | `MetricFrame[time_series | panel]` actual + `forecast_frame` + `AlignmentPolicy(kind="forecast_horizon")` | `forecast_evaluation_result` | 评估 forecast vs actual 的误差、区间覆盖、bias、drift |
