...
```text
transform(metric_frame) -> metric_frame
transform(delta_frame) -> delta_frame
transform(attribution_frame) -> attribution_frame
```
...
| -------------- | ------------------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| `canonical`    | 稳定 request / response schema、确定性 DAG、完整 lineage / evidence / failure semantics | `attribute`, `diagnose`, `validate`, `auto_decompose`, `evaluate_forecast`  |
| `exploratory`  | 可辅助探索，不默认进入 canonical evidence 链路                                              | `driver_axis_scan`, `driver_attribution_scan`, `metric_profile`, `lag_scan` |
...
| `hypothesis_test_result`     | `test`                              | 明确统计假设的检验结果                                    |
| `forecast_frame`             | `forecast`                          | 未来 bucket 的模型投影                                |
| `forecast_evaluation_result` | `evaluate_forecast` composite       | forecast vs actual 的预测效果评估                     |
| `quality_report`             | `assess_quality`                    | artifact 质量、覆盖率和前提可用性评估                        |
| `diagnosis_result`           | `diagnose` composite                | 针对候选异常或业务问题的多步骤诊断闭包                            |
| `driver_scan_result`         | `driver_attribution_scan` composite | 多 axis attribution scan 的固定结果                  |
| `metric_profile_result`      | `metric_profile` composite          | 指标 profile 的稳定 projection result               |

以下对象不是公开核心算子的 output family：

- `sample_frame`：`test` 内部 materialized sampling node，可进入 lineage / manifest，但不要求 agent 手写。
...
| Policy | 用途 | 典型字段 |
| --- | --- | --- |
| `AlignmentPolicy` | compare / correlate / test 的跨输入对齐，以及 transform 的单 frame 时间对齐 | `kind`, `calendar`, `fiscal_calendar`, `campaign_window`, `forecast_origin`, `horizon_index`, `submission_time_policy`, `timezone` |
| `LagPolicy` | correlate 单 lag 或 lag sweep | `mode`, `offset`, `range`, `step`, `max_lag`, `selection_metric` |
| `SamplingPolicy` | test 内部 sampling / pairing / null handling | `unit`, `method`, `pairing`, `null_handling`, `min_n` |
...
所有 provider 字段都必须是 typed ref，而不是裸字符串：`CalendarRef`、`FiscalCalendarRef`、`CampaignWindowRef`、`ForecastOriginRef`。`evaluate_forecast` 必须支持 forecast-specific alignment：按 forecast origin、horizon index 和 submission timestamp policy 对齐滚动预测。
...
candidates = analysis.discover(delta, objective="driver_axes") # candidate_set
```
...
| `transform` | `transform(Frame[T], op=...) -> Frame[T]` |
...
| `discover(point_anomalies)` | `discover(MetricFrame[time_series | panel], objective="point_anomalies") -> CandidateSet[point_anomaly]` |
| `discover(period_shifts)` | `discover(DeltaFrame[time_series_delta | panel_delta], objective="period_shifts") -> CandidateSet[period_shift]` |
| `discover(driver_axes)` | `discover(DeltaFrame[*], objective="driver_axes") -> CandidateSet[driver_axis]` |
| `discover(interesting_slices)` | `discover(MetricFrame[*] | DeltaFrame[*], objective="interesting_slices") -> CandidateSet[slice]` |
| `discover(interesting_windows)` | `discover(MetricFrame[time_series | panel] | DeltaFrame[time_series_delta | panel_delta], objective="interesting_windows") -> CandidateSet[window]` |
| `discover(cross_sectional_outliers)` | `discover(MetricFrame[segmented | panel], objective="cross_sectional_outliers") -> CandidateSet[cross_sectional_outlier]` |
| `correlate` | `correlate(MetricFrame[T], MetricFrame[T], lag_policy=LagPolicy) -> AssociationResult[single_lag | lag_sweep]` |
| `test` | `test(MetricFrame[T], MetricFrame[T], hypothesis=..., sampling=SamplingPolicy) -> HypothesisTestResult` |
...
```python
axis_candidates = analysis.discover(delta, objective="driver_axes")
...
- `transform(metric_frame, op="compare") -> delta_frame`
- `transform(delta_frame, op="decompose") -> attribution_frame`
- `transform(frame, op="discover") -> candidate_set`
- `transform(frame_collection, op="align") -> frame_collection`
...
跨 frame alignment 不属于 `transform`。`compare`、`correlate`、`test` 需要对齐多个输入时，应在各自请求中声明 `AlignmentPolicy`，并把对齐结果写入该算子的 lineage / metadata。`transform(op="align_time")` 只处理单个 frame artifact 的时间轴规范化，例如 fiscal week、campaign-relative time、cohort-relative period 或 calendar bucket 重写；它不能接受 frame collection，也不能输出多个 frame。
...
### Candidate discovery 统一到 `discover`

未来目标态不再保留独立 `detect` core operator。用于选择下一步分析对象的候选发现结果，统一沉淀为 `candidate_set`。异常检测、变点检测、driver axis 搜索、窗口发现、跨段离群都统一为：

```text
discover(source_artifact, objective, strategy?) -> candidate_set
```

`objective` 表达 agent 要找什么；`strategy` 表达 runtime 使用哪类检测/排序策略。二者都必须来自封闭枚举，不能是自然语言字符串。
...
`discover` 只输出候选，不输出 attribution、diagnosis、hypothesis test result 或新的事实 frame。`candidate_set` 的职责是表达“下一步值得看哪里”，不是表达“看完以后得出的结论”。候选生成前提是否可靠由 `assess_quality(candidate_set)` 判断；候选是否为真异常或真驱动因素，需要后续 `test`、composite validation workflow 或 agent judgment。
...
3. 下游消费路径能通过 `select`、`transform`、`decompose`、`test`、`forecast` 或 composite operator 完成。
...
如果 agent 需要复用某种业务时间表达，例如 fiscal week、campaign-relative day 或 cohort-relative period，应先 `observe` 原始 metric，再用 `transform(op="align_time", policy=AlignmentPolicy(...))` 生成单 frame 的 aligned view。两个 frame 之间的 pairwise alignment 仍属于 `compare`、`correlate` 或 `test` 的职责。
...
```python
mobile_dau = analysis.transform(
    dau,
    op="slice",
    where={"platform": "mobile"},
)

top_declines = analysis.transform(
    delta,
    op="topk",
    by="delta_pct",
    direction="decrease",
...
`decompose` 回答“这个 delta 由谁贡献”。核心 `decompose` 不负责自动找轴；自动找轴属于 `discover(objective="driver_axes")` 或 `analysis.composites.auto_decompose(...)`。
...
```python
anomalies = analysis.discover(
    dau,
    objective="point_anomalies",
    strategy="seasonal_residual",
    sensitivity="balanced",
    limit=5,
)

axis_candidates = analysis.discover(
    delta,
...
`correlate` 输出 association，不输出 causation。它不做 metric scan 或因果控制。lag 必须通过 typed `LagPolicy` 表达：

- `LagPolicy(mode="single", offset="3d")` 产出 `AssociationResult[single_lag]`。
- `LagPolicy(mode="sweep", range=("0d", "7d"), step="1d", selection_metric="abs_corr")` 产出 `AssociationResult[lag_sweep]`。

lag sweep 是 leading indicator 分析的高频需求，不应要求 agent 手写循环跑 N 次 `correlate`。但 lag sweep 仍是 association，不表示因果。

`association_result` 是描述性关联结果。核心 DAG 不支持 `association_result -> test` 或 `association_result -> decompose`：如果 agent 要验证关联是否显著，应回到产生该关联的 source `metric_frame`，用明确 hypothesis 调用 `test` 或 composite validation workflow；如果要分析哪些 segment 驱动了关联，应使用 composite operator 回到源 frames 分段计算，而不是把 association 本身当作可分解 delta。
...
relationship = analysis.correlate(
    ...
    activation_rate,
    method="spearman",
    lag_policy=LagPolicy(mode="sweep", range=("0d", "7d"), step="1d"),
)
```
...
`test` 直接接受 `metric_frame` 或已解析的 test-ready input。Agent 不需要手写 `sample_summary`。如果检验需要样本摘要，runtime 应在 `test` 的 execution manifest / lineage 中 materialize 内部 sampling node，记录 sampling policy、null handling、rate numerator / denominator、pairing 和 sample size。
...
### `evaluate_forecast` composite

职责：比较已观测 actual `metric_frame` 与先前生成的 `forecast_frame`，评估预测效果。
...
`forecast_frame` 不能直接进入普通 `compare`。普通 `compare` 比较两个已观测事实面；`evaluate_forecast` 明确处理 predicted-vs-observed，输出误差、区间覆盖、bias、drift signal 和可跟进建议。
...
evaluation = analysis.evaluate_forecast(
    actual=actual,
    forecast=forecast,
    alignment=AlignmentPolicy(
        kind="forecast_horizon",
        forecast_origin=ForecastOriginRef("gmv_daily_forecast"),
        horizon_index="day",
        submission_time_policy="latest_before_actual_bucket",
    ),
)
```
...
当输入是 `candidate_set` 时，`assess_quality` 只能检查生成候选所需的前提是否可靠，例如源数据覆盖率是否足够、扫描窗口是否过短、eligible series 数是否不足、检测阈值是否可解释、是否存在明显 truncation。它不能判断“候选是否为真异常”，也不能通过 bootstrap、resampling 或重复扫描来做结果验证；这类验证必须回到 `test`、composite validation workflow 或 human/agent decision。

显著性验证统一走 `test`。Forecast 风险若能由 forecast 自身计算，应进入 `forecast_frame` metadata；若要比较 actual vs forecast，应使用 `evaluate_forecast`；若需要更长的多步诊断，应作为 composite operator。
...
Projection 不产生 canonical artifact family，不直接 seed evidence proposition，也不能作为 `compare`、`decompose`、`discover`、`test` 的输入。下游核心算子必须引用 canonical artifact、selector ref 或 typed input。
...
不是所有 result artifact 都应该变成 frame。`hypothesis_test_result`、`association_result`、`quality_report`、`forecast_evaluation_result` 默认仍然是 judgment / evaluation result，不直接作为 `compare`、`decompose`、`discover` 的输入。
...
| Result | Follow-up 示例 |
| --- | --- |
| `association_result` | 对 source frames 做 segment-level correlation scan；用 `test` 验证明确假设 |
| `hypothesis_test_result` | 若验证的是 delta，可回到 source `delta_frame` 做 `decompose` |
| `quality_report` | 缩短窗口、切换 source、调整 sampling、先做 `transform(..., op="impute_nulls")` |
| `forecast_evaluation_result` | 诊断 drift window、重训 forecast、切分 segment 评估 |
...
跨长时间窗 `compare`、`forecast`、`test` 必须检查 `metric_definition_ref`。如果 metric 定义在窗口内发生版本变化，runtime 应返回 warning 或 blocking issue，不能把定义漂移静默当作业务变化。
...
对通用 agent，默认 authoring model 是 `analysis.session()`。复杂互联网分析几乎每一步都可能需要 agent 看中间结果后决定下一步；因此 session 必须支持 step-wise execution，并保持 continuous lineage。

### 决策树

| 场景 | 写法 |
| --- | --- |
| 复杂业务分析，需要 agent 看中间结果再决定下一步 | 使用 `analysis.session()`，逐 step 提交并读取 projection |
...
metric = session.observe(metric=MetricRef("revenue"), time="last_90d", grain="day")
summary = metric.summary()
candidates = session.discover(metric, objective="interesting_windows")
```
...
local_series = session.transform(series, op="window", window=window)
```
...
- `test` 直接消费 metric frames 和 sampling policy。
- runtime 在 lineage 中记录内部 sampling node。
- `test` 输出 `hypothesis_test_result`，不输出业务诊断。
...
evaluation = session.evaluate_forecast(actual=actual, forecast=forecast)
```
...
- `evaluate_forecast` 明确处理 predicted-vs-observed，不走普通 `compare`。
...
复杂互联网分析中，forecast evaluation 不应长期落在弱契约 escape hatch。它们是否属于 core operator 的判断仍看固定输出 family 和语义原子性；但只要它们高频、结构稳定、能产生 typed artifact，就应该进入 composite registry，并用 `contract_level` 明确 runtime 承诺。

| 名称 | Contract level | 固定输出 | 展开方式 / 作用 |
| --- | --- | --- | --- |
| `attribute` | `canonical` | `attribution_frame` | `observe -> observe -> compare -> decompose` |
| `diagnose` | `canonical` | `diagnosis_result` | candidate slice、local compare、decompose、quality checks 的诊断闭包 |
| `validate` | `canonical` | `hypothesis_test_result` | `observe -> observe -> test` |
| `auto_decompose` | `canonical` | `attribution_frame` | `discover(driver_axes) -> select -> decompose`，axis candidates 写入 metadata |
| `evaluate_forecast` | `canonical` | `forecast_evaluation_result` | actual `metric_frame` vs `forecast_frame` |
| `driver_axis_scan` | `exploratory` | `CandidateSet[driver_axis]` | 扫描多个 axes 并返回候选轴 |
| `driver_attribution_scan` | `exploratory` | `driver_scan_result` | 对多个候选轴执行 attribution scan |
| `lag_scan` | `exploratory` | `AssociationResult[lag_sweep]` | `correlate` lag sweep 的封装或模板 |
| `metric_profile` | `exploratory` | `metric_profile_result` | `observe -> projection summaries` |
...
- `exploratory`：排序、启发式或扫描导向，主要产出候选、建议或探索性 result，不默认 seed evidence。
...
| Source type | 合法下游 |
| --- | --- |
| `MetricFrame[scalar]` | `transform`, `compare` with same shape, `correlate` with same shape, `test`, `assess_quality`, composite operators |
| `MetricFrame[time_series]` | `transform`, `compare` with same shape, `discover(point_anomalies | interesting_windows)`, `correlate` with same shape, `test`, `forecast`, `evaluate_forecast(actual)`, `assess_quality`, composite operators |
| `MetricFrame[segmented]` | `transform`, `compare` with same shape, `discover(interesting_slices | cross_sectional_outliers)`, `correlate` with same shape, `test`, `assess_quality`, composite operators |
| `MetricFrame[panel]` | `transform`, `compare` with same shape, `discover(point_anomalies | interesting_windows | cross_sectional_outliers)`, `correlate` with same shape, `test`, `forecast`, `evaluate_forecast(actual)`, `assess_quality`, composite operators |
| `DeltaFrame[scalar_delta]` | `transform`, `decompose`, `discover(driver_axes | interesting_slices)`, `assess_quality`, composite operators |
| `DeltaFrame[time_series_delta]` | `transform`, `decompose`, `discover(period_shifts | driver_axes | interesting_windows | interesting_slices)`, `assess_quality`, composite operators |
| `DeltaFrame[segmented_delta]` | `transform`, `decompose`, `discover(driver_axes | interesting_slices)`, `assess_quality`, composite operators |
| `DeltaFrame[panel_delta]` | `transform`, `decompose`, `discover(period_shifts | driver_axes | interesting_windows | interesting_slices)`, `assess_quality`, composite operators |
...
| `ForecastFrame` | `evaluate_forecast`, `assess_quality` |
| `ForecastEvaluationResult` | typed follow-up / evidence assessment |
...
- `forecast_frame -> compare`：forecast 与 observed metric 的认识论地位不同，forecast vs actual 只能走 `evaluate_forecast`。
...
- `exploration_result` 不能直接喂给 `compare`、`decompose`、`discover`、`test` 等核心算子。
...
- pandas dataframe 默认是 scratch result，不能直接喂给 `compare`、`decompose`、`discover`、`test` 等核心算子。
...
focused_delta = analysis.promote_delta_frame(
    ...
    policy=PromotionPolicy(
        auto_infer=True,
        semantic_anchors={
            "source_delta": ArtifactRef(delta.artifact_id),
        },
        on_missing="fail_closed",
    ),
)
...
| `test` | `MetricFrame[T]` inputs + hypothesis + `SamplingPolicy` | `hypothesis_test_result` | 检验统计假设 |
...
- limited `transform(...)`：仅允许兼容矩阵声明的 `filter`、`slice`、`window`、`topk` / `bottomk`。
...
funnel = session.composites.funnel(
    ...
    segment_by=[DimensionRef("channel"), DimensionRef("country")],
)
...
dropoff = session.transform(dropoff_metric, op="topk", by="dropoff")
```
