...
| Policy | 用途 | 典型字段 |
| --- | --- | --- |
| `AlignmentPolicy` | compare / correlate / test 的跨输入对齐，以及 transform 的单 frame 时间对齐 | `kind`, `mode`, `strict_lengths`, `calendar`, `fiscal_calendar`, `campaign_window`, `forecast_origin`, `horizon_index`, `submission_time_policy`, `timezone` |
