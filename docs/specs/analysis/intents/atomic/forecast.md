...
- `source_artifact_id` 解析得到的 observe artifact lineage
- `horizon`
...
`forecast` 消费 typed artifact ID，而不是裸字符串 step id 或 inline reference object。

AOI 协议通过 `source_artifact_id` 直接引用 observe artifact。`source_ref` 只出现在输出
lineage 中，由 runtime 从 artifact 元数据自动解析。
...
{
  "step_type": "forecast",
  "source_artifact_id": "obs_artifact_dau_daily",
  "horizon": 14
}
```
...
- `interval_level = 0.95`，该值不作为公开请求参数
...
- `horizon <= 0`
- `source_artifact_id` 解析到 projection-only、incomplete 或未完成 artifact

推荐错误码：`INVALID_ARGUMENT`。

## 归一化规则

- `interval_level` 固定归一化为内部默认 `0.95`，请求中不得传入该字段
...
表示预测区间的置信水平。v1 固定使用内部默认 `0.95`，不接受请求覆盖。

契约要求：

- point forecast 不应依赖 `interval_level`
- 后续版本若公开 `interval_level`，改变该值只能改变 interval 宽度，不得改变历史拟合语义
- 后续版本若公开 `interval_level`，该值越高，预测区间应弱单调变宽
...
- `ARTIFACT_NOT_FOUND`
  `source_artifact_id` 无法解析到已提交 source artifact
- `UNSUPPORTED_OPERATION`
