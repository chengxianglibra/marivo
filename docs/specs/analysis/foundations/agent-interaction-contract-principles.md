...
本文档定义 Marivo 在 `specs/analysis/` 语境下的 agent interaction contract（Agent 交互分析接口）设计准则。

状态：draft design。本文是 `specs/analysis/` 下关于 agent-first 分析接口的总纲性设计文档，不表示对应 HTTP endpoint 或 session-state wire contract 已经实现。

## 目的

Marivo 的主要上层消费者是 agent。对于这类系统，最容易出现的偏移通常不是“算不出结果”，而是把对外接口设计成了错误的主语：
...
本文的目标，是为 `specs/analysis/` 下的 typed intent、canonical evidence schema 与 projection 设计提供一组统一准则，使 Marivo 面向 agent 的交互接口始终保持：
...
在 agent-first 设计下，Marivo 与 agent 的默认协作应遵循固定闭环，而不是临时拼接的 request/response 模式：

1. agent 将用户问题翻译为 typed intent
2. Marivo 执行 intent，并提交 canonical artifact
3. Marivo 基于 artifact 更新 canonical evidence state
4. agent 读取 命题中心 状态面
5. agent 决定：
   - 直接回答
   - 追加新的 typed intent
...
- Marivo 负责执行、抽取、评估、状态组织
- agent 负责选动作、解释结果、决定下一步

因此，Marivo 不应被设计成“自动跑完整调查”的 workflow brain，agent 也不应退化成负责拼接 SQL、手工维护 evidence graph 的 orchestration glue。

## 非目标

Marivo 面向 agent 的交互接口不应被设计为：
...
开放式解释、业务叙事、行动建议整合，默认仍属于 agent 的职责；Marivo 负责提供确定性执行与结构化证据状态。
...
当 contract 同时需要表达 session-level task boundary 与 planning hint 时，二者也应显式分离；不得把 focus hint 混入 enforced scope。对于 Marivo 的根容器 schema，这意味着 session root 不应持有 enforced execution scope，执行约束应进入具体 action 的 step params。
...
相应地，Marivo 应提供的是低歧义、可决策的状态接口，而不是替 agent 固化停止规则、阈值或动作选择算法。
...
### 2. 优先消费 artifact ID，而不是重复描述上游语义

当某个动作依赖上游结果时，应优先传 artifact ID，并在输出 lineage
中保留 typed refs，例如：
...
- Marivo 负责将其纳入统一 inference / assessment / gap 流程
...
为避免文档与实现后续重新滑回混合模型，`specs/analysis/` 下的交互设计应明确拒绝以下形态：
...
本文是 `specs/analysis/` 中面向 agent 交互接口的总纲。
...
- [`evidence-engine/overview.md`](../evidence-engine/overview.md) 负责说明 状态面 中 artifact / finding / proposition / assessment / action proposal 的规范分层
- [`state-surface-schema.md`](../evidence-engine/schemas/state-surface-schema.md) 负责说明 命题中心 session state 的 canonical 读取契约
- [`context-surface-schema.md`](../evidence-engine/schemas/context-surface-schema.md) 负责说明 命题中心 上下文面 的 canonical 读取契约
