...
| 机械可预填参数，例如当前 artifact ref、已解析 time_scope | 填业务判断参数 |
...
默认推荐显式 absolute `time_scope`，不要把自然语言短语作为核心 API：
...
Skill 可以教 agent 如何把“last 7 days”翻译成 absolute `time_scope`，但 Marivo API 不应把
自由字符串作为主要入口。
...
- `semantic_model` 不作为参数暴露。库从 `metric` 的 catalog 所属模型推断，避免 agent 重复填写
  并引入不一致。
- `time_scope` / `grain` 表示本次 derivation 的观测范围与时间粒度；如果 query 已经编码过滤，
...
version、columns binding、time_scope、grain、datasource freshness marker 和 Marivo artifact schema
...
- resolved params，包括 absolute time_scope、grain、dimensions、policies；
...
7. 默认时间窗口使用 explicit absolute `time_scope`；relative window 只能是 typed constructor，
   不接受自由字符串主路径。
8. `derive_metric_frame` 是唯一默认公开 governed escape hatch；`derive_delta_frame` /
   `derive_attribution_frame` 不进入 agent-facing surface。
9. `derive_metric_frame` 采用 `metric + query + columns + time_scope + grain + label` 契约；
...
3. 所有示例使用 explicit absolute `time_scope`，避免 `mv.window("last_7d")` 这类自由字符串主路径。
