...
状态：draft design with v1 scope freeze。本文保留较完整的设计空间，但 v1 实施边界以后续 scope note 为准：`specs/semantic/calendar-alignment-policy-v1-scope-note.zh.md`。
...
- `specs/analysis/intents/atomic/observe.md`
- `specs/analysis/intents/atomic/compare.md`
- `specs/analysis/evidence-engine/rules/comparability-gate-contract.md`
...
- `bucket_pairing plan` 在 v1 只作为 observation artifact 的 `resolved_policy_summary` metadata 输出，不升格为可单独引用的一等 artifact；见 `specs/semantic/calendar-bucket-pairing-artifact-decision.zh.md`
