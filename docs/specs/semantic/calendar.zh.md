...
Calendar data 通过 MCP `update_calendar_data` 增量维护。该入口使用 upsert 语义：
只新增或覆盖请求中给出的 calendar row，不删除未提到的既有 rows。

调用方可先通过 MCP `list_calendar_data` 查询当前已录入的 holiday/exception rows。加载时校验 `day_kind` 只能是 `holiday` 或 `adjusted_workday`，且 `holiday` row 必须提供非空 `holiday_group_id`。仓库不再维护内置 CN calendar 生成脚本；调用方负责提供可信 holiday/exception rows。
