...
Holiday 对齐读取覆盖 current window 和右侧 artifact baseline window 的 dense calendar rows。若配置缺失或数据不完整，compare 返回 blocking failure，而不是回退到隐式 policy。
...
- 给定相同请求输入、相同 source/version 组合，必须可重放出同一 bucket pairing
