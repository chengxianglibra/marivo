...
catalog.list("metric").show()                                  # all metrics across every domain
...
| `catalog.list(kind, scope=None)` | kind-first 浏览：`catalog.list("metric")` 枚举所有 metric；`catalog.list("dimension", scope="entity.sales.orders")` 枚举 scope 子树内的 dimension。`kind` 接受字符串或 `SemanticKind` 枚举 |
...
`find_project()` 的 project 判定只要求 `models/semantic/` 目录存在。空目录也算语义项目：`SemanticProject` 可返回，load 后 registry 为 `ready`，`catalog.list("domain").objects` 返回空 tuple。如果 `models/semantic/` 存在但不是目录，必须 fail closed。

Use `catalog.get("<kind>.<semantic_id>")` to resolve a known object, and use `catalog.list("<kind>", scope="<kind>.<semantic_id>")` to browse under a known domain, datasource, or entity. Pass `SemanticObject.ref` into `catalog.preview(...)`, `catalog.readiness(...)`, and `catalog.verify_object(...)`. The catalog surface is deterministic and does not depend on fuzzy or embedding-based recall. At the top level (no scope), `catalog.list("metric")` searches across all domains; `catalog.list("metric", scope="domain.sales")` scopes to a single domain.

`catalog.list(kind, scope=...)` 和 `catalog.get(...)` 不写 stdout。需要人类可读输出时显式调用 `.show()`；程序化消费使用 `SemanticObject.ref`、`.details()`、`.children` 和 `SemanticObjectList.objects`。`details()` 返回的结构化 dataclass 也支持 `.render()` / `.show()`，用于 agent-facing bounded card 输出。
...
domain 是业务域边界，例如 `sales`、`marketing`、`subscription`。domain 名称参与下游 semantic id，例如 `sales.revenue`。agent 不应用自然语言近似匹配替代 domain id；如果不确定，应先 `catalog.list("domain")` / `catalog.get(...)`。
...
- 对 user/agent 诊断，优先使用 `marivo doctor --semantic --format json` 输出结构化 errors / warnings / readiness report / parity summary，便于 agent 稳定解析；`python -m marivo.semantic.check` 保留为兼容性 runner。
...
- reader/introspection：`ms.load()` 返回 `SemanticCatalog`；`catalog.list(kind, scope=...)`、`catalog.get(...)`、`catalog.preview(...)`、`catalog.readiness(...)` 是 agent-facing read/handoff surface。
