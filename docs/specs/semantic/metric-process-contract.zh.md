...
> **过渡说明**：本文档已更新为反映 OSI Metric + MARIVO 扩展模型。原有的三层分工（metric / process object / intent）已简化为二层：metric 直接声明 measurement semantics（通过 `observed_dataset`、`observation_grain`、`primary_time_field`、`additivity`、`filters`），过程构造语义不再作为独立的 process object 类型存在。具体映射如下：
...
>
> 原有的 `ProcessObjectHeader`、`ProcessInterfaceContract` 等结构已不再使用。metric 通过 `observed_dataset`、`observation_grain`、`primary_time_field`、`filters` 等 MARIVO 扩展字段直接声明度量语义。
...
- `DatasetBinding`：Dataset/Field 的 typed wiring
...
先把 measurement identity、comparability、additivity_constraints、inferential capability 从 `definition_sql` 中剥离出来，形成更完整的 metric contract。同时引入 `observed_dataset`、`observation_grain`、`primary_time_field`、`filters` 等 MARIVO 扩展字段。
