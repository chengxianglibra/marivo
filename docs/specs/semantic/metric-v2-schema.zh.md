...
> - `metric_family` → 删除；Metric 类型通过 `sample_kind` / `aggregation_semantics` / `additive_dimensions` 推导
...
> - `additivity_constraints`（`dimension_policy` + `time_axis_policy`）→ `additive_dimensions`（扁平列表）+ `aggregation_semantics`
...
- metric 如何通过 MARIVO 扩展（relationship-resolved dataset graph、runtime/request grain、`additive_dimensions`、`aggregation_semantics`、`filters`）表达完整度量语义
...
更稳定的做法是让 metric 通过 relationship-resolved dataset graph、runtime/request grain、`additive_dimensions`、`aggregation_semantics`、`filters` 声明自己的 measurement 语义，过程前提通过 compiler 兼容性校验保证。
...
class MetricHeader(TypedDict):
    ...
    aggregation_semantics: Literal["sum", "ratio", "weighted_average"]  # default "sum"
...
- `additivity_constraints`（含 `dimension_policy` / `time_axis_policy` / `notes`）：替换为 `additive_dimensions`（扁平列表）+ `aggregation_semantics`
- `sample_kind` 的 `binary` / `survival` 枚举值：删除；仅保留 `numeric` / `rate`

**以下字段不应进入 public metric schema，由 compiler 从核心字段推导：**

- `supported_intents`：由 `sample_kind` + `additive_dimensions` + `aggregation_semantics` 推导
...
注：`supports_test` 和 `supports_validate` 不再作为加法性推导的能力。统计检验能力由 `aggregation_semantics` 和 `sample_kind` 决定。
...
| `aggregation_semantics` | enum | yes | 聚合语义：`"sum"` / `"ratio"` / `"weighted_average"`；默认 `"sum"`；替代原 `additivity_constraints.dimension_policy` + `time_axis_policy` |
...
- `aggregation_semantics` 决定统计检验方法和分解策略
...
- `additive_dimensions = []`, `aggregation_semantics = "sum"` 或 `"ratio"`
...
`additive_dimensions` 和 `aggregation_semantics` 是 metric v2 中非常关键的字段，替代了旧的结构化 `additivity_constraints`（含 `dimension_policy` / `time_axis_policy`）。
...
- `additive_dimensions` / `aggregation_semantics` 不是 compare artifact 本身
...
- additive_dimensions / aggregation_semantics
- result mode support
...
> Marivo 的 metric v2 应以 OSI typed measurement contract 为中心，显式表达 dataset relationship graph、request grain、sample kind、type-specific measurement payload、additive_dimensions、aggregation_semantics、comparability 与 filters（MARIVO 扩展），并把执行细节下沉到 compiler / IR。
