...
- metric 如何通过 MARIVO 扩展（`observed_dataset`、`observation_grain`、`additive_dimensions`、`aggregation_semantics`、`filters`）表达完整度量语义
...
- 通过 `observed_dataset` / `observation_grain` 声明数据源与粒度
...
`compatible` 适用于 display name、description、unit label、文档化 semantics 文案等不改变 dataset grounding 和 compiler contract 的修订。`breaking` 适用于 observed dataset、observation grain、sample kind、value semantics、additive dimensions、aggregation semantics 等会影响 dataset resolution 或编译结果的修订。
...
更稳定的做法是让 metric 通过 `observed_dataset`、`observation_grain`、`additive_dimensions`、`aggregation_semantics`、`filters` 声明自己的 measurement 语义，过程前提通过 compiler 兼容性校验保证。
...
- `primary_time_ref` / `primary_time_field`：删除；时间语义由 AOI `time_scope.field` 解析，不再作为 metric 字段
...
> Marivo 的 metric v2 应以 OSI typed measurement contract 为中心，显式表达 observed dataset、observation grain、sample kind、type-specific measurement payload、additive_dimensions、aggregation_semantics、comparability 与 filters（MARIVO 扩展），并把执行细节下沉到 compiler / IR。
