...
> - `metric_family` → 删除；Metric 类型通过 `sample_kind` / `decomposition_semantics` / `additive_dimensions` 推导
> - `MeasurementComponent` / `qualifier_refs` / `default_predicate_refs` → `Metric.filters`（MARIVO 扩展）
> - `observed_entity_ref` → relationship-resolved dataset graph（OSI Dataset 引用）
> - `runtime_grain_ref` → runtime/request grain（OSI grain 概念）
> - `primary_time_ref` / metric-bound time field → 删除；时间语义由 AOI `time_scope.field` 解析
> - `additivity_constraints`（`dimension_policy` + `time_axis_policy`）→ `additive_dimensions`（扁平列表）+ `decomposition_semantics`
> - `sample_kind` 的 `binary` / `survival` 枚举值 → 删除；仅保留 `numeric` / `rate`
...
- metric 如何通过 MARIVO 扩展（relationship-resolved dataset graph、runtime/request grain、`additive_dimensions`、`decomposition_semantics`、`filters`）表达完整度量语义
...
- 通过 relationship-resolved dataset graph / runtime/request grain 声明数据源与粒度
...
- 最终量出的值是什么
- 它观测什么 Dataset（relationship-resolved dataset graph），以及以什么粒度稳定输出（runtime/request grain）
- 它属于哪类 sample
- 是否可比较、可检验、可分解
...
`compatible` 适用于 display name、description、unit label、文档化 semantics 文案等不改变 dataset grounding 和 compiler contract 的修订。`breaking` 适用于 dataset relationship graph、request grain、sample kind、value semantics、additive dimensions、aggregation semantics 等会影响 dataset resolution 或编译结果的修订。
...
更稳定的做法是让 metric 通过 relationship-resolved dataset graph、runtime/request grain、`additive_dimensions`、`decomposition_semantics`、`filters` 声明自己的 measurement 语义，过程前提通过 compiler 兼容性校验保证。
...
- relationship-resolved dataset graph：metric 直接消费或观测的 Dataset 引用
- runtime/request grain：metric 输出样本的稳定粒度
...
时间语义不再作为 metric 字段；metric 的主时间通过 AOI `time_scope.field` 在请求时解析。
...
class MetricHeader(TypedDict):
    ...
    dataset_relationship_graph: str  # OSI Dataset 引用，替代 observed_entity_ref
    runtime_grain: str  # OSI grain 概念，替代 runtime_grain_ref
    sample_kind: Literal["numeric", "rate"]  # NOT NULL, default "numeric"
    ...
    additive_dimensions: list[str]  # 扁平列表，替代旧 AdditivityConstraints
    decomposition_semantics: Literal["sum", "ratio", "weighted_average"]  # default "sum"
...
- `population_subject_ref`：删除；主体语义由 relationship-resolved dataset graph 表达
- `observed_entity_ref`：替换为 relationship-resolved dataset graph（OSI Dataset 引用）
- `runtime_grain_ref`：替换为 runtime/request grain（OSI grain 概念）
- `primary_time_ref` / metric-bound time field：删除；时间语义由 AOI `time_scope.field` 解析，不再作为 metric 字段
- `additivity_constraints`（含 `dimension_policy` / `time_axis_policy` / `notes`）：替换为 `additive_dimensions`（扁平列表）+ `decomposition_semantics`
- `sample_kind` 的 `binary` / `survival` 枚举值：删除；仅保留 `numeric` / `rate`

**以下字段不应进入 public metric schema，由 compiler 从核心字段推导：**

- `supported_intents`：由 `sample_kind` + `additive_dimensions` + `decomposition_semantics` 推导
...
注：`supports_test` 和 `supports_validate` 不再作为加法性推导的能力。统计检验能力由 `decomposition_semantics` 和 `sample_kind` 决定。
...
| relationship-resolved dataset graph | string | yes | metric 直接消费或观测的 Dataset 引用；替代原 `observed_entity_ref` |
| runtime/request grain | string | yes | metric 输出样本的稳定粒度；替代原 `runtime_grain_ref` |
| `sample_kind` | enum | yes | 样本类型；NOT NULL，默认 `"numeric"`；仅 `"numeric"` / `"rate"` |
| `value_semantics` | enum | yes | 值语义 |
| `aggregation_scope` | enum | no | 聚合作用的主要层次 |
| `additive_dimensions` | list[string] | yes | 可加维度字段名列表；空列表表示不可在任意维度上分解；替代原 `additivity_constraints` |
| `decomposition_semantics` | enum | yes | 聚合语义：`"sum"` / `"ratio"` / `"weighted_average"`；默认 `"sum"`；替代原 `additivity_constraints.dimension_policy` + `time_axis_policy` |
...
- relationship-resolved dataset graph 与 runtime/request grain 是 metric identity 的一部分
- `sample_kind` 直接决定可支持的 inferential summary contract
- `additive_dimensions` 直接影响 `decompose -> attribute` 的合法性：空列表表示不可分解，非空列表表示可在列出的维度上分解
- `decomposition_semantics` 决定统计检验方法和分解策略
...
- relationship-resolved dataset graph 使用 Dataset 引用
- runtime/request grain 使用 grain 引用
- 时间语义通过 AOI `time_scope.field` 在请求时解析，不再作为 metric 字段
...
- `grain`：已由 runtime/request grain 统一表达
...
```json
{
  ...
  "dataset_relationship_graph": "dataset.user_activity",
  "runtime_grain": "grain.user",
  "sample_kind": "numeric",
...
```json
{
  ...
  "display_name": "GMV",
  "dataset_relationship_graph": "dataset.order",
  "runtime_grain": "grain.order",
  "sample_kind": "numeric",
  "value_semantics": "sum",
  "aggregation_scope": "event",
  "additive_dimensions": ["platform", "country", "app_version"],
  "decomposition_semantics": "sum",
  "metric_contract_version": "v1",
  "measure": {
    "name": "paid_amount",
...
```json
{
  ...
  "dataset_relationship_graph": "dataset.user",
  "runtime_grain": "grain.user",
  "sample_kind": "rate",
...
- 如果它依赖 session 粒度，应通过 relationship-resolved dataset graph / runtime/request grain 明确表达
...
```json
{
  ...
  "dataset_relationship_graph": "dataset.session",
  "runtime_grain": "grain.session",
  "sample_kind": "numeric",
...
- `additive_dimensions = []`, `decomposition_semantics = "sum"` 或 `"ratio"`
...
```json
{
  ...
  "dataset_relationship_graph": "dataset.request_event",
  "runtime_grain": "grain.event",
  "sample_kind": "numeric",
...
- `sample_kind` 为 `numeric`（`survival` 已从 `sample_kind` 枚举中移除，survival 语义通过 `value_semantics = "survival_probability"` 表达）
...
- metric 的 relationship-resolved dataset graph / runtime/request grain
...
`additive_dimensions` 和 `decomposition_semantics` 是 metric v2 中非常关键的字段，替代了旧的结构化 `additivity_constraints`（含 `dimension_policy` / `time_axis_policy`）。

当前模型使用扁平化的设计：

- `additive_dimensions: list[str]`：列出 metric 可加的维度字段名。空列表表示 metric 不在任何维度上可加；`["__all"]` 表示对 semantic model 中所有已声明维度可加。
- `decomposition_semantics: "sum" | "ratio" | "weighted_average"`：声明 metric 的聚合语义。
...
### decomposition_semantics 规则

- `"sum"`：可加连续值 metric（如 revenue、latency）— 使用 Welch's t-test
- `"ratio"`：比例/比率 metric（如 conversion rate）— 使用 two-proportion z-test
- `"weighted_average"`：比率之和 metric（如 AOV = SUM(revenue)/COUNT(orders)）— 使用 delta method

### 示例

- `gmv`：`additive_dimensions = ["__all"]`, `decomposition_semantics = "sum"`
- `dau`（count_distinct）：`additive_dimensions = []`, `decomposition_semantics = "sum"`
- `inventory_balance`：`additive_dimensions = ["warehouse"]`, `decomposition_semantics = "sum"`
- `conversion_rate`：`additive_dimensions = []`, `decomposition_semantics = "ratio"`
...
- relationship-resolved dataset graph、runtime/request grain 与 value semantics 不冲突
- `sample_kind` 仅允许 `"numeric"` / `"rate"`（`binary` / `survival` 已移除）
...
- metric 的 `additive_dimensions` 是否满足下游分析动作
- 请求维度若为 `time_derived`，其 `required_time_anchor` 是否被 AOI `time_scope.field` 满足
...
- `additive_dimensions` / `decomposition_semantics` 不是 compare artifact 本身
...
- additive_dimensions / decomposition_semantics
- result mode support
...
> Marivo 的 metric v2 应以 OSI typed measurement contract 为中心，显式表达 dataset relationship graph、request grain、sample kind、type-specific measurement payload、additive_dimensions、decomposition_semantics、comparability 与 filters（MARIVO 扩展），并把执行细节下沉到 compiler / IR。
