...
1. **public semantic contracts**
   ...
   - `Metric` (OSI) + MARIVO extensions (observed_dataset, observation_grain, additivity, filters)
   - `intent`
...
6. additivity_constraints.dimension_policy 是否满足 `decompose` / `attribute`
...
class MetricRefSnapshot(TypedDict):
    ...
    metric_name: str  # OSI metric name within semantic model
    # 只有编译时 resolved/override 的字段才在这里
    resolved_primary_time_field: NotRequired[str | None]
    resolved_observation_grain: NotRequired[list[str] | None]
...
class IrInputSnapshot(TypedDict):
    ...
    metric_name: NotRequired[str | None]  # OSI metric name
    # 编译时 resolved 的字段（可选）
    resolved_metric: NotRequired[MetricRefSnapshot | None]
    resolved_datasets: NotRequired[list[DatasetRefSnapshot] | None]
    resolved_fields: NotRequired[list[FieldRefSnapshot] | None]
...
- `ProcessSemanticSnapshot` → 已删除，process object 不再作为独立输入
...
class MeasurementNode(IrNodeHeader):
    ...
    metric_name: str  # OSI metric name
    observed_dataset: str  # OSI dataset name
    observation_grain: NotRequired[list[str] | None]  # field names
    primary_time_field: NotRequired[str | None]  # field name
...
class IrPlan(TypedDict):
    ...
    nodes: list[MeasurementNode | IntentNode]
