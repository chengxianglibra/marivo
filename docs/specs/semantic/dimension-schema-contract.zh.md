...
> **概念迁移说明**：本节描述的 Dimension 概念已演进为 OSI Field 的 `dimension` 属性，不再作为独立对象存在。
> 在 dataset-native 模型中，维度归属由 Field.dimension 直接表达；维度值域、层级与分组能力作为 Field.dimension 的子属性。
> 原独立 `dimension.*` 命名空间对应为 Field 的 dimension 属性引用；原 `source_field_ref` / `entity.<entity>.field.<field>` 不再需要，因为 Field 已内联于 Dataset 之中。
> 原 `enum_set_ref` / `enum_version` 已删除；原 typed-binding-contract.zh.md 已废弃。
...
- ~~`dimension` 与 `entity`、`metric`、`process object`、typed binding、compiler 的职责边界如何划分~~（typed binding 已废弃；维度现由 Field.dimension 属性承载）
...
- 表达共享分析轴的稳定语义身份
- ~~为 `entity.stable_descriptors`、`process.exported_dimension_refs` 与请求维度解析提供可引用的 `dimension_ref`~~（`stable_descriptors` 已删除；维度归属由 Field.dimension 属性表达；`dimension_ref` 现由 Field.dimension 承接）
- 为 compiler 提供显式的维度合法性、层级性与值治理信息
- ~~与 typed binding contract 解耦，使底层列、lookup、join、枚举表与派生逻辑可以独立演进~~（typed binding 已废弃；物理列映射由 Field.expression 内联完成）

本文不定义：

- 最终数据库 DDL
- 最终 REST endpoint shape
- ~~最终 source binding 的物理字段结构~~（物理字段结构由 Field.expression 内联完成）
...
- **可引用**：可被 `entity`、`metric`、`process object`、compiler 稳定引用（现由 Field.dimension 属性承载）
- **可治理**：支持枚举值集合、层级关系（`enum_set_ref` / `enum_version` 已删除；枚举治理由 Field 自身约束表达）
- **可组合**：可与 `entity.*`、`subject.*`、`grain.*` 组成统一 ref taxonomy
- ~~**可迁移**：允许 runtime 先从字符串数组迁移到 `dimension.*` 引用，并通过 entity field refs 逐步接入物理 grounding~~（物理 grounding 由 Field.expression 内联完成，无需 entity field refs 中转）
...
- `entity.*`：稳定业务实体（对应 OSI Dataset name）
- `subject.*`：总体主体
- `grain.*`：样本 / 输出粒度
- `key.*`：实体身份键
- ~~`time.*`：时间语义~~（时间语义现由 Field.dimension.is_time 属性表达）
- ~~`dimension.*`：分析维度语义~~（维度现由 Field.dimension 属性表达）
- ~~`enum.*`：受治理的维度值集合~~（`enum_set_ref` / `enum_version` 已删除）
- `gate.*`：治理 gate
- ~~`binding.*`：typed binding 中的受治理绑定锚点~~（binding 概念已删除）

这意味着：

- `dimension_ref`（现由 Field.dimension 承接）只能指向维度属性
- `process.exported_dimension_refs` 应只保存维度引用
- ~~`entity.stable_descriptors[*].dimension_ref` 应只保存 `dimension.*`~~（`stable_descriptors` 已删除）
...
~~这些应由 entity binding 与 compiler 表达，dimension 只保留语义 ref、可选
`source_field_ref` 与结构化治理信息。~~
在 dataset-native 模型中，物理列映射由 Field.expression 内联完成；dimension 作为 Field.dimension 属性存在，不需要独立引用 `source_field_ref` 或依赖 binding 层。
...
class DimensionValueDomainSpec(TypedDict):
    ...
    # enum_set_ref: 已删除 — 枚举治理由 Field 自身约束表达
    # enum_version: 已删除 — 枚举版本由 Field 自身约束表达
```
...
- ~~`enum_set_ref` / `enum_version`：若使用受治理枚举，则引用对应枚举集与发布版本；值域本体与版本边界见 [`enum-set-schema-contract.zh.md`](./enum-set-schema-contract.zh.md)~~（已删除；枚举治理由 Field 自身约束表达，不再需要独立枚举集引用）
...
- ~~`source_field_ref`：可选的语义字段来源引用；必须指向
  `entity.<entity>.field.<field>`。它不是 physical column、carrier locator 或 binding
  target，不表达物理绑定；物理落地仍由 entity binding 负责。~~（已删除；在 dataset-native 模型中，Field 已内联于 Dataset，维度作为 Field.dimension 属性直接存在于 Dataset 中，无需跨对象字段引用）
- `value_domain`：维度值域与结构语义。
- `hierarchy`：可选的语义 roll up 邻接。
- `grouping`：可选的分组能力声明。
- `time_derived_requirement`：可选的时间派生前置锚点。
...
它们必须通过 `interface_contract.time_derived_requirement.required_time_anchor_ref` 声明”消费方需要提供哪个时间语义锚点”，例如引用 Field.dimension.is_time 属性。

~~这里的 `time.*` 应引用统一的 `time-schema-contract.zh.md`，而不是由 dimension 自己定义一套局部时间命名。~~
在 dataset-native 模型中，时间语义由 Field.dimension.is_time 属性表达，而非独立 `time.*` 命名空间。

~~其字段来源应由消费方指向 entity field，例如：~~

- ~~entity 的 `primary_time_ref`~~
- ~~process 的 `anchor_time_ref`~~
- ~~metric 的 `primary_time_ref`~~
- ~~`source_field_ref = entity.<entity>.field.<field>`~~

在 dataset-native 模型中，字段来源由 Dataset 中的 Field 直接承载；Field.expression 内联完成列映射。
...
~~这些属于 entity binding / compiler lowering。~~
在 dataset-native 模型中，这些属于 Dataset 的 Field.expression 与 compiler lowering。
...
> `enum_set_ref` / `enum_version` 已删除。枚举治理现由 Field 自身约束表达。
> 以下为原始设计说明，保留用于概念追溯。
...
在 dataset-native 模型中，枚举约束由 Field 自身的值域定义表达，不再需要独立枚举集对象或版本引用。
...
metric 在最小 public contract 中不直接维护维度兼容矩阵。若系统需要更细的维度限制或版本 pinning，应交给 compiler compatibility profile，而不是回写到 dimension 主 contract；profile 的发布形态与 compiler 消费语义见 `specs/semantic/compiler-compatibility-profile.zh.md`。
...
~~dimension contract 不拥有自己的 physical binding。它可以通过
`source_field_ref = entity.<entity>.field.<field>` 声明推荐字段来源；该 field 的物理落地
由 entity binding 负责。~~

在 dataset-native 模型中，维度作为 Field.dimension 属性直接存在于 Dataset 中；Field.expression 内联完成列映射，不再需要独立的 binding 层、`source_field_ref` 或 carrier。

~~entity binding / compiler 负责声明和解析：~~
...
Dataset 通过 `source` + `datasource_id` 直接指向物理数据源；Field.expression 内联完成列映射。

~~而不是让 dimension 文档自己定义这些细节。实现必须拒绝 dimension object 中的
carrier、binding target、physical column、lookup table 等对象级物理绑定字段。~~
...
**注意**：原 `source_field_ref` 已移除（Field 已内联于 Dataset）；原 `enum_set_ref` / `enum_version` 已移除（枚举治理由 Field 自身约束表达）。
...
**注意**：原 `source_field_ref`、`enum_set_ref`、`enum_version` 已移除。
...
**注意**：原 `source_field_ref` 已移除。`required_time_anchor_ref` 在 dataset-native 模型中引用 Field.dimension.is_time 属性。
...
- ~~`typed binding contract`~~（已废弃；物理列映射由 Field.expression 内联完成）
- compiler validation
- metric / process / entity 对象自己的消费边界

~~在 entity-centric 模型下，dimension 可以通过 `source_field_ref` 指向
`entity.<entity>.field.<field>`，并由 validate/activate 阶段检查字段 `value_type` 是否适合
该 dimension 的 `value_domain.value_type`。不兼容时应返回
`invalid_field_type_for_semantic_object` 类 blocker，而不是等 SQL lowering 后失败。~~

在 dataset-native 模型下，维度作为 Field.dimension 属性直接存在于 Dataset 中；
Field.expression 内联完成列映射。compiler 在编译时校验 Field 的值类型是否适合该维度的
...
1. 引入独立维度命名（现由 Field.dimension 属性承载）
   - 为现有 `platform`、`country`、`variant`、`state` 等字符串建立 canonical refs
2. 将 metric 上的 `dimensions / allowed_dimensions` 迁移为维度引用
   - 旧输入通过 translation layer 映射到 Field.dimension
3. 对齐 Dataset / process object
   - ~~`stable_descriptors`、`exported_dimension_refs` 全部引用 dimension catalog~~
   - Field.dimension 属性直接表达维度归属；`exported_dimension_refs` 引用 Field
4. 对齐 compiler
   - `request_dimensions` 先解析维度，再做 metric/process/Field 的时间锚点与治理校验
5. ~~最后补 entity grounding~~（binding 概念已删除）
   - ~~把 source object、field path、lookup/as-of 规则与时间锚点映射放到 entity binding~~
   - Dataset 通过 `source` + `datasource_id` 直接指向物理数据源；Field.expression 内联完成列映射

一言以蔽之：

> dimension 现由 Field.dimension 属性承载，是 semantic layer 中”共享分析轴”的稳定表达；~~entity 负责字段归属与物理落地~~Dataset 通过 Field 直接承载维度归属与物理接地，metric 负责使用边界，process object 负责导出边界，compiler 负责组合校验。
