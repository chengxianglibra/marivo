...
运行时目标解析的权威说明见 [`overview.md`](./agent-runtime/overview.md)。Skill 只引用该边界，不复写 `MARIVO_MODE`、`runtime.json`、`doctor` 或 transport 的字段级契约。
...
- [`specs/service/marivo-skill.md`](./marivo-skill.md)：维护基线，定义目标形态、边界、验收标准与更新规则
...
- `/Users/lichengxiang/source/bili/skills/marivo/references/semantic-readiness.md`：`lifecycle_status` / `readiness_status` 与 blocker 排查
- `/Users/lichengxiang/source/bili/skills/marivo/references/http-contracts.md`：跨 surface 的共享 HTTP 约束
...
- [`overview.md`](./agent-runtime/overview.md)：本地自动托管 / 远程显式连接的总体模型
- [`troubleshooting.zh.md`](./agent-runtime/troubleshooting.zh.md)：用户与 operator 的最小排障路径
- [`error-taxonomy.zh.md`](./agent-runtime/error-taxonomy.zh.md)：结构化错误码与修复建议
