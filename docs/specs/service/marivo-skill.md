...
- 在缺少业务知识材料时，直接从字段名推断指标口径
- 语义口径尚未获批时就开始正式分析，导致结论不可复用
...
- 让 agent 在语义层显式吸收业务知识，并在 analysis 阶段安全发挥 LLM 的多步规划能力
...
- 引导 agent 在 semantic surface 先 intake 业务知识，再形成可确认的 reusable contract
- 约束 agent 只有在 semantic preflight 通过后，才进入 planner-led investigation
...
对 reusable semantic object，skill 应先要求 agent 收集用户提供的业务知识材料，再把业务定义与 live metadata 对齐，最后才进入写入或修复。

### 6. 允许 analysis 多步规划，但不允许绕过语义门禁

analysis skill 应允许 agent 在 session 内自主规划多步 typed intent 链路，但必须建立在已批准口径、已知时间语义和可用 readiness 之上。
...
3. intake 用户提供的业务知识材料，并提炼候选口径
4. 构建、确认或修复 semantic graph
5. 创建 session
6. 在 semantic preflight 通过后启动 investigation
7. 让 agent 在 session 中规划并执行多步 typed intent 链路
8. 在关键分支读取 session state，必要时读取 proposition context
9. 若发现语义缺口，回 semantic surface 修复后再继续
10. 在 investigation 写入结束后显式 terminate session
...
- 何时必须先 intake/确认业务知识，而不是从字段名推断业务定义
- 何时应建 reusable semantic object，而不是做一次性 session work
- 何时应停止 analysis 并退回 semantic layer，而不是继续补丁式解释
...
- 在没有业务知识材料时直接把字段名当最终口径
- 在 semantic contract 未获批时就进入正式 analysis
- 把 analysis planner 当成可绕过 semantic gate 的自由规划器
...
- 如果变化影响“何时必须先 intake 业务知识”“何时 analysis 必须退回 semantic layer”，应更新 skill
...
- agent 能在 reusable semantic work 中先摄取业务知识并获得关键口径确认
- agent 能执行一个稳定的 planner-led investigation loop
- agent 不会把工具包装或运维 surface 当作 canonical evidence
- agent 不会在 semantic gate 未通过时开始正式分析
