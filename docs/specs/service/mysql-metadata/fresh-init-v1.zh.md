...
状态：v1 frozen design；tasks 1.1 - 7.5 completed。
...
- [`../agent-runtime/bootstrap-config.zh.md`](../agent-runtime/bootstrap-config.zh.md) 冻结的是本地 bootstrap 默认配置；本地默认继续使用 SQLite。
- [`../agent-runtime/overview.md`](../agent-runtime/overview.md) 描述 agent 本地运行时自动托管流程；MySQL metadata 不改变该流程的默认体验。
- [`source-execution-mapping-contract.md`](../data-plane/source-engine-mapping-contract.md) 冻结 source、execution engine 与 mapping 的数据平面边界；metadata MySQL 只替换 control-plane store，不新增 source/engine/mapping 类型。
...
## 测试矩阵状态（tasks 6.1 - 6.4）

任务 6.1 - 6.4 已落地为 SQLite/MySQL metadata 测试矩阵的前半段：

- SQLite storage/config 回归继续覆盖初始化、schema marker、模板校验、当前 schema shape、基础 DML 与配置非法组合拒绝。
- MySQL 集成测试由 `MARIVO_TEST_MYSQL_DSN` 显式启用；未配置 DSN 时稳定 skip，不要求默认开发机或默认 CI 具备 MySQL。
- 启用 MySQL 集成测试时，每个测试创建 `marivo_test_<uuid>` 临时 database，使用 `utf8mb4` 初始化并在结束后 drop；测试账号必须具备 `CREATE/DROP DATABASE` 权限。
- MySQL initialize 集成测试覆盖 expected tables、主键、唯一索引、普通索引、外键、schema marker、unknown non-empty schema fail-closed，以及 current schema 缺索引/缺 FK preflight 失败。
- MySQL DML 集成测试覆盖 dict row 输出、`query_one` 空结果、`execute_many`、`insert_ignore`、`upsert_by_key`、FK cascade 与 unique constraint。
- MySQL rollback、app startup、静态扫描与最终 typecheck/lint 验证归属 tasks 6.5 - 6.8。

## 测试矩阵状态（tasks 6.5 - 6.8）

任务 6.5 - 6.8 已补齐测试矩阵后半段：

- Artifact commit boundary 在 SQLite 与 MySQL 下均覆盖 staged artifact 写入后 finding insert 失败的 rollback 场景；异常后不残留 artifact、finding 或 downstream proposition。
- MySQL app startup 集成测试使用 `metadata.engine=mysql` 配置启动应用，并覆盖基础 session create/read/list API。
- SQLite-only SQL 静态扫描覆盖 shared `app/` 与非 SQLite-only 测试文件，防止重新引入 SQLite 专用 DML、时间表达式或 introspection SQL。
- 最终验证以 `make typecheck`、`make lint` 和 `git diff --check` 作为 6.x 测试矩阵收口；真实 MySQL 集成路径仍由 `MARIVO_TEST_MYSQL_DSN` 显式启用。

## 文档与运维状态（tasks 7.2 - 7.5）

任务 7.2 - 7.5 已落地为配置示例、operator runbook 与 decision record：

- `marivo.example.yaml` 是 runtime-only 示例，保留 SQLite 本地默认，并提供 MySQL DSN 与显式字段示例；source、engine、mapping 继续通过 HTTP API 管理。
- [`operator-runbook.zh.md`](./operator-runbook.zh.md) 说明空库创建、账号授权、MySQL 配置、启动验证、连接池 sizing、备份恢复和常见 fail-closed 错误处理。
- [`decision-record.zh.md`](./decision-record.zh.md) 记录 v1 non-goals 与后续扩展入口，避免把迁移工具、online migration、PostgreSQL 或 schema migration framework 误认为当前承诺。
- 运维文档统一强调 MySQL metadata 是 control-plane store，不是 source adapter 或 execution engine；v1 不自动 repair、ALTER、DROP、迁移或 backfill 未识别 schema。
