...
| 业务专家 / 语义建模者 | 业务语义是否可被 agent 稳定使用？ | entity、metric、process、dimension、time、enum、predicate、binding、compiler profile | semantic object active + ready，依赖闭包完整，能力符合分析场景 |
...
1. 注册或检查 source。
2. 浏览 live catalog，选择需要同步的 schema / table。
3. 触发 sync，查看 sync job 和 synced source objects。
4. 注册或检查 engine。
...
- `/health` 当前状态。
- `/metrics` 中的 active sessions、pending jobs、steps failed、sync jobs completed 等摘要。
- source、engine、mapping 的 ready / not_ready 分布。
- 最近失败的 jobs、sync jobs、routing failures。
...
- Sync：sync mode、sync selections、最近 sync job。
- Catalog Browser：live schemas / tables 浏览。
- Synced Objects：同步后的 source objects，突出 authority locator。
- Mappings：治理该 source 的 mapping 摘要。

用户友好性要求：

- `readiness_status != ready` 时，详情页顶部必须显示失败原因和修复入口。
- sync selections 应用表格和批量选择表达，不要求用户手写 JSON。
...
- Routing Impact：当前 mapping 影响的 source objects 和可路由表范围。

用户友好性要求：

- active mapping 的 catalog coverage 缺失必须以差异视图展示，例如“source 已同步 catalog A/B，但 mapping 只覆盖 A”。
...
1. 选择业务域或 source object 范围。
2. 浏览 synced source objects，理解事实表、维表和关键字段。
3. 创建 entity、dimension、time、metric、process、predicate 等语义对象。
4. 创建 typed binding，把语义对象绑定到 source object。
5. validate 对象。
6. activate 对象。
7. 查看 readiness blocker、dependency refs、capabilities。
8. 修复依赖，直到对象 active + ready。
9. 通过 catalog search / semantic resolve 验证对象可被 agent 发现和使用。
...
- 支持按 lifecycle、readiness、ref prefix、source object、dependency ref 过滤。
...
- capability 缺口。
- 建议修复入口，例如创建 binding、activate dependency、修复 source object readiness。
...
- 对依赖缺失场景，要提供跳转到依赖对象或 source object 的入口。
...
- Metric Wizard：metric family、value semantics、time semantics、input slots、capabilities。
- Binding Wizard：carrier source object、metric inputs、key refs、time bindings、dimension exports。
- Dimension Wizard：value domain、semantic role、enum set、source grounding。
...
Source Object Browser 服务语义建模，不是管理员 catalog 管理页的重复。

页面内容：

- synced source objects。
- authority locator。
- schema / table / column 摘要。
- 已有关联 bindings。
- readiness 与 mapping 摘要。

用户友好性要求：

- 支持从 source object 反向创建 binding。
- 支持查看该 source object 关联了哪些 semantic refs。
...
- 运行总览。
- Sources 列表 / 详情 / sync selections / synced objects。
- Engines 列表 / 详情。
