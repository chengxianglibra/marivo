...
Metadata-store changes must remain backward compatible for local SQLite control-plane databases; initialization should reconcile legacy `sessions` schemas to the current session lifecycle/read contract instead of assuming a fresh DB.
...
- `Data Sources` detail panes should recover from partial fetch failures: selecting a source must immediately populate the right-side cards from inventory state, then layer in `/sources/{source_id}`, selections, synced objects, and live catalog responses without dropping back to a global `no source selected` placeholder when one secondary request fails.
