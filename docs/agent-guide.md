...
**Session root surface (Phase 5a):** `GET /sessions/{id}` returns canonical `AnalysisSession` with structured sub-objects: `goal.question`, `governance.{policy_refs, budget, warnings}`, `lifecycle.{status, terminal_reason, ended_at, rollover_from_session_id}`, `state_summary.state_view_ref`, `updated_at`, `schema_version="analysis_session.v1"`. Legacy flat fields (`status`, `constraints`, `budget`, `policy`, `raw_filter`) are no longer present at the top level. `GET /sessions/{id}/runtime-status` returns `SessionRuntimeStatus` (`overall_status`, `last_successful_stage`, `blocked_reason`, `backlog_summary`, `updated_at`, `schema_version="session_runtime_status.v1"`) — operator use only; runtime truth must not be read from canonical `state` / `context` surfaces. Session runtime status is derived from canonical DB state (v1 simplified: no real queue/lease).

**State surface (Phase 5b):** `GET /sessions/{id}/state` returns `SessionStateView` (`session_id`, `focus_subjects`, `active_propositions`, `backing_findings`, `blocking_gaps`, `artifact_refs`, `truncation`, `schema_version="session_state_view.v1"`, `next_page_token`). This is the canonical session-level read baseline for agents; it exposes only *externally visible* proposition-local bundles — propositions without a publish switch appear with `latest_assessment=null` (live judgment-track objects, not absent data). `POST /sessions/{id}/state/query` accepts a `SessionStateQuery` body (adds `slice` support and multi-axis filtering; same response shape). Supported query fields: `metric`, `entity`, `slice` (POST only), `proposition_types`, `origin_kinds`, `assessment_presence` (`"assessed"` / `"unassessed"`), `assessment_statuses`, `has_blocking_gaps`, `limit`. `page_token` is a transport URL parameter (not part of `SessionStateQuery`; not accepted in the `POST /state/query` body). Filter invariants: `assessment_presence="unassessed"` + any `assessment_statuses` returns empty 200 (never 422); `has_blocking_gaps=false` excludes unassessed propositions; `slice` on `GET /state` returns 400. Materialization in `app/evidence_engine/state_view.py:materialize_session_state_view`; HTTP contract in `docs/api/session-state.md`.

**Artifact runtime status (Phase 5b):** `GET /sessions/{id}/artifacts/{artifact_id}/runtime-status` returns `ArtifactRuntimeStatus` (`session_id`, `artifact_id`, `artifact_stage`, `extractor_key.{artifact_type, artifact_schema_version, extractor_version}`, `correlation_id`, `attempt_id`, `last_failure_reason`, `last_failure_at`, `schema_version="artifact_runtime_status.v1"`) — operator use only. `artifact_stage` derives from canonical DB state: D4-allows-empty families (`observation`, `anomaly_candidates`) or artifacts with findings → `"findings_committed"`; non-empty-required artifacts with no findings → `"staged"`. v1 does not emit `"extracting"`, `"seeding_handoff_pending"`, or `"failed"` (synchronous pipeline; no intermediate state tracking). `correlation_id = artifact_id` in v1. Implemented in `app/session/session_manager.py:SessionManager.get_artifact_runtime_status`; HTTP contract in `docs/api/runtime-status.md`.
