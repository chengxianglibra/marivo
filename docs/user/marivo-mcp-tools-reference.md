...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| granularity | `"hour"` \| `"day"` \| `"week"` \| `"month"` \| `"quarter"` \| `"year"` | 否 | 时间轴 selector；传入后结果包含时间轴 |
| dimensions | string[] | 否 | 分组 selector，如 `["cluster", "department"]`；传入后结果包含维度轴 |
| filter_expression | McpExpression | 否 | AOI 过滤表达式对象；不用时省略，不传 `null` |

模式约束：`granularity` 和 `dimensions` 都省略时返回 scalar；只传 `granularity` 返回 time_series；只传 `dimensions` 返回 segmented；两者都传时返回 panel。
...
成功返回的 artifact 是 AOI 合约层的 `metric_frame`。根据参数组合不同，`shape` 分为：

- **scalar**：单一数值 `value`
- **time_series**：按时间轴返回 `series[].points[]`，每个 point 可含 `window`、`value`
- **segmented**：按维度返回多条 `series[]`，每条 series 含 `keys` 和 `points`
- **panel**：同时含时间轴和维度轴
...
interface ObserveArtifact {
  ...
  measures: [{ id: "value"; value_type: "number"; nullable: true; unit?: string | null }];
  payload: { series: MetricFrameSeries[] };
}

interface MetricFrameSubject {
  kind: "metric";
  metric_ref: string;
  time_scope: { field: string; start: string; end: string };
  scope: Record<string, unknown>;
}

type MetricFrameAxis =
  | { kind: "time"; grain: "hour" | "day" | "week" | "month" | "quarter" | "year" }
  | { kind: "dimension"; name: string };

interface MetricFrameSeries {
  keys: Record<string, string | number | boolean | null>;
  points: Array<{
    window?: { start: string; end: string }; // ISO-8601 datetime
    value: number | null;
  }>;
}
...
```json
{
  "data": {
    ...
    "measures": [{ "id": "value", "value_type": "number", "nullable": true, "unit": null }],
    "payload": {
      "series": [
        {
          "keys": {},
