...
- semantic object 如何稳定绑定到 asset surfaces
...
- 把 `entity_ref`、`key_refs`、`primary_time_ref` 等语义引用映射到 asset surfaces
- 把 `dimension_ref`、`stable_descriptors.dimension_ref`、`exported_dimension_refs` 等语义引用映射到 asset surfaces
- 为 `entity`、`process object`、`metric` 提供可引用、可组合的 `binding_ref`
- 把 `process object`、`metric` 已声明的语义接口映射到 asset bindings、asset surfaces 与受治理消费策略
...
1. 这个 semantic object 主要依赖哪些 assets
2. 这个 binding 自身的稳定引用是什么，以及它显式依赖哪些其他 bindings
3. 在这些 assets 上，哪些 surfaces 落到哪些**契约目标路径（contract targets）**
4. 多个 asset bindings 之间允许通过哪些 relation surfaces 连接，以及哪些消费策略属于稳定 binding 约束
...
### 4. binding 应区分 asset binding 与 semantic field binding

建议把 binding 分为两层：

1. **asset binding**
   - 这个语义对象主要绑定到哪些 assets
2. **semantic field binding**
   - 在这些 assets 上，哪个 surface 承担哪个稳定语义角色
...
class BindingInterfaceContract(TypedDict):
    ...
    asset_bindings: list[AssetBinding]
...
- assignment/exposure/event/fact 等 asset bindings
- join relations
...
以下示例按“public binding contract”理解：`field_role`、`revision`、catalog 名称等旧字段不再是主 schema 的必需部分。示例统一采用 `asset_ref + asset_surface_ref` 作为物理锚点；最终 public target vocabulary 建议再单独拍板，不在本轮写死。
