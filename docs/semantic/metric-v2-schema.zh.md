...
class RequiredProcessContract(TypedDict):
    emitted_entity: NotRequired[list[str] | None]
    entity_grain: NotRequired[list[str] | None]
...
| `observed_entity` | enum | yes | metric 直接消费或观测的实体类型 |
| `observation_grain` | enum | yes | metric 输出样本的稳定粒度 |
...
- `observed_entity` 与 `observation_grain` 是 metric identity 的一部分
...
```json
{
  ...
  "observed_entity": "user",
  "observation_grain": "user",
  "sample_kind": "numeric",
...
```json
{
  ...
  "observed_entity": "order",
  "observation_grain": "order",
  "sample_kind": "numeric",
  "value_semantics": "sum",
...
- 如果它依赖 session 粒度，应通过 `observed_entity` / `observation_grain` 与 `required_process_contract` 显式表达
...
```json
{
  ...
  "population_subject_ref": "subject.user",
  "observed_entity": "session",
  "observation_grain": "session",
  "sample_kind": "numeric",
...
```json
{
  ...
  "observed_entity": "event",
  "observation_grain": "event",
  "sample_kind": "numeric",
...
- metric `required_process_contract.emitted_entity`
- metric `required_process_contract.entity_grain`
- metric `required_process_contract.required_capabilities`
- metric 的 `sample_kind`
- metric 的 `supported_intents`
- process 的 `output_contract`

### 例子

- `conversion_rate` 这类实验指标通常要求 process 提供 `variant_split + sample_summary_prep`
- `avg_watch_time` 这类会话质量指标通常要求 process 输出 `session` 且具备 `time_projection`
...
- `observed_entity`、`observation_grain` 与 family 语义不冲突
...
- `required_process_contract` 不是最终 process join / materialization plan
