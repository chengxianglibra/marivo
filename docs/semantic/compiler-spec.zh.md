...
- `process object` 提供 process/output contract
...
- metric contract 自洽
- process object subtype 与 `output_contract` 自洽
- intent schema 自洽
...
- `observed_entity` / `emitted_entity` 是否兼容
- `observation_grain` / `entity_grain` 是否兼容
...
### Gate 6：engine capability precheck

仅校验“是否存在可 lower 的路径”，不执行真正 lowering。
...
在所有 gates 通过、必要的 derived expansion 完成之后，compiler 装配 IR plan。

### IR 装配原则

1. 每个计划必须保留 normalized input snapshot。
2. 每个语义职责进入固定 node family。
3. validation 决策必须在 IR 或同级编译产物中有稳定位置。
4. execution boundary 必须显式，而不是散落在语义节点里。
...
- 至少一个 `IntentNode`
- 零个或多个 `ValidationNode`
- 至少一个 `ExecutionBoundaryNode`

#### ref-based atomic intent

除了本次 intent 自身的 `IntentNode` 外，还必须在 input snapshot 中保留上游 artifact lineage：
...
- 派生层 `IntentNode(intent_kind="attribute" | "diagnose" | "validate")`
- 每个内部 atomic intent 的 `IntentNode`
- wiring 到内部 canonical refs 的 lineage 关系
...
- 生成 `ValidationNode(validation_kind="comparability_gate")`
...
- `ValidationNode(validation_kind="comparability_gate")`
...
- `ValidationNode(validation_kind="engine_capability_gate")`
- `IntentNode(intent_kind="detect")`
- `ExecutionBoundaryNode(boundary_kind="adapter_lowering" | "materialization")`

## 与现有文档的关系

- `metric-v2-schema.zh.md` 负责 measurement contract
- `process-object-schema.zh.md` 负责 process contract 与 output contract
- `metric-process-contract.zh.md` 负责三层分工原则
- `ir-schema-contract.zh.md` 负责 IR node family 与最小公共 schema
