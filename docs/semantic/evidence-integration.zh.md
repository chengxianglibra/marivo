...
2. `metric_ref` / `process_ref` / `entity.*` / `dimension.*` / `time.*` 是 **stable semantic identity**，是跨 artifact、跨 session、跨编译轮次可复用的治理语义引用。
...
5. 这些关联 **只允许** 基于现有 canonical fields、lineage、typed input snapshots、binding contracts 推导；本文 **不引入新的统一映射槽位**。
...
- Semantic Layer 负责 stable semantic contracts、binding contracts、compiler validation、typed input normalization、IR snapshots。
...
| stable semantic ref | `metric_ref`、`process_ref`、`entity.*`、`dimension.*`、`time.*`、其他 `semantic_ref` | "该 evidence / contract 关心的治理语义对象是什么" | "本次 session 中具体是哪条 canonical evidence" |
...
- `process_ref` 采用 `process.{process_key}`，例如 `process.checkout_experiment`
- `dimension.*` 表示 `dimension.{dimension_key}` 家族，例如 `dimension.country`
- `entity.*` 表示 `entity.{entity_key}` 家族，例如 `entity.user`
...
1. **finding/proposition 自身的 subject**
   ...
   - `FindingSubject.entity` → `entity.*` refs
   - `PropositionSubject` 同理

2. **step_ref**
   - `step_ref` 本身是 session-local provenance handle，指向 step metadata
   - semantic identity 来自该 step metadata 中的 `typed_inputs`、`binding_refs`、`process_context` 等字段
   - 推导路径可表示为：`step_ref -> step metadata -> typed_inputs / binding_refs / process_context -> semantic refs`
...
#### `step_ref` → `process_ref` / 其他 semantic refs
...
3. 从 `binding_refs` 找到绑定 contract，从 `process_context` 找到 process 语义上下文
4. 最终推导 `metric_ref`、`process_ref`、`dimension.*`、`time.*`

典型路径：

- `step_ref -> typed_inputs.metric_ref_snapshot -> metric_ref`
- `step_ref -> process_context -> process_ref`
- `step_ref -> binding_refs -> binding.target/process_context -> dimension.* | time.*`

`binding_refs` 与编译期 resolved snapshots 的 ref-only 约束分别见：

- [`typed-binding-contract.zh.md`](./typed-binding-contract.zh.md)
...
- 读取方 **MUST NOT** 通过 `derived_from` 替代 `process_ref` 推导
...
- `process_ref` **MUST** 通过 `step_ref` 对应的 typed inputs、process snapshots、binding lineage 推导。
- 若一个 finding 同时依赖多个 process objects，这种多重关联 **MUST** 保留在 lineage 语义中，不得压扁成单个伪主键。

### PropositionRef 特化规则

- `PropositionRef` **MUST NOT** 被解释为 `metric_ref` 或 `process_ref` 的别名。
...
1. **Compiler**
   - 负责 typed input resolution、dimension normalization、binding compatibility 与 process/metric 兼容校验
   - 若无法从 canonical fields、typed inputs、binding contracts 稳定推导 semantic refs，**MUST** 在编译阶段失败
...
- 仅凭 `artifact_id` 命名约定推断 `metric_ref` 或 `process_ref`
...
- binding 中的 `semantic_ref` **MUST** 继续表达字段服务的语义目标。
- binding **MUST NOT** 把 `semantic_ref` 降级为 artifact-local 字段 locator。
- binding 负责说明 "这个 surface/field 服务哪个 semantic target"，不负责重新发明 canonical evidence id。

### Compiler / IR

- compiler **MUST** 把 semantic refs 解析为 typed input snapshots、binding refs、resolved semantic fields。
- IR **MUST** 继续"使用引用而非复制"：保留 `metric_ref` / `process_ref` / binding refs 等语义快照。
...
- `time.order_created_at` — 由 `observed_window` 结合 binding contract（该 metric 的 primary_time_ref）推导
...
- `entity.user` — 由 `subject.entity` 推导
...
### 2. Process-backed experiment proposition

一个 proposition 由实验上下文驱动，claim 围绕 `metric.conversion_rate` 在 `process.checkout_experiment` 下展开：
...
- `process_ref` = `process.checkout_experiment` — 由 `lineage.source_step_refs` 中的 step binding（该 compare step 绑定的 process context）推导
- `dimension.variant` — 由 seed findings 的 `subject.slice` 或 binding contract 推导

**边界强调**：
- `derived_from_proposition_ref` 不能替代 `process.checkout_experiment`
- `process.checkout_experiment` 也不能替代 `PropositionRef`
...
4. **读取层/binding/compiler/IR 必须遵守各自边界** — provenance、semantic identity、runtime membership 不得混为一谈。
...
| [`metric-process-contract.zh.md`](./metric-process-contract.zh.md) | `metric` / `process object` 的语义分工与 stable refs 定义 |
| [`dimension-schema-contract.zh.md`](./dimension-schema-contract.zh.md) | `dimension.*` 的定义边界、归一化与治理要求 |
| [`typed-binding-contract.zh.md`](./typed-binding-contract.zh.md) | binding 如何承载 `metric_ref` / `process_ref` / dimension refs 的物理落地 |
