...
- 表达共享分析轴的稳定语义身份
- 为 `entity.stable_descriptors`、`metric.required_dimensions`、`process.exported_dimension_refs` 提供可引用的 `dimension_ref`
...
- **可治理**：支持枚举值、null/other bucket、版本与兼容性治理
...
- `metric.required_dimensions / forbidden_dimensions` 应只保存 `dimension.*`
...
- 受治理枚举值集合
- null / other bucket 政策
- drill down / roll up 的语义邻接关系
...
- 它是开放域还是受治理枚举域
- 它是否存在父子层级 / drill down 关系
- 它是否天然适合作为分组、分解、检测切分、实验切分轴
...
| `dimension_ref` | string | yes | 可被 entity / metric / process / compiler 引用的稳定维度标识；必须使用 `dimension.*` |
...
class DimensionValueDomainSpec(TypedDict):
    ...
    null_policy: NotRequired[Literal["allow_null", "forbid_null", "unknown_bucket"] | None]
    other_bucket_policy: NotRequired[
        Literal["not_supported", "optional", "required"] | None
    ]
```
...
- `enum_set_ref` / `enum_version`：若使用受治理枚举，则引用对应枚举集
- `null_policy`：缺失值如何治理
- `other_bucket_policy`：是否允许或要求将长尾归入 `other`
...
class DimensionHierarchySpec(TypedDict):
    ...
    child_dimension_refs: NotRequired[list[str] | None]
    drill_down_dimension_refs: NotRequired[list[str] | None]
```
...
- `child_dimension_refs`：直接子维度
- `drill_down_dimension_refs`：建议性的 drill down 邻接关系

`drill_down_dimension_refs` 只表达语义邻接，不表达物理执行路径。

#### DimensionAnalysisContract

```python
from typing import Literal, NotRequired, TypedDict


class DimensionAnalysisContract(TypedDict):
    supports_grouping: bool
    supports_decomposition: NotRequired[bool | None]
    supports_detection_split: NotRequired[bool | None]
    supports_experiment_split_export: NotRequired[bool | None]
    recommended_cardinality_band: NotRequired[
...
- `supports_decomposition`：是否适合用于 `decompose` / `attribute`
- `supports_detection_split`：是否适合用于 `detect` 候选切分
- `supports_experiment_split_export`：是否适合作为 experiment / cohort 稳定导出轴
- `recommended_cardinality_band`：推荐基数带，仅作为治理提示，不是硬约束
...
class DimensionInterfaceContract(TypedDict):
    ...
    analysis_contract: NotRequired[DimensionAnalysisContract | None]
```
...
- `metric.required_dimensions / forbidden_dimensions`
...
### 3. time-derived dimension 不自行选择时间锚点
...
### 4. hierarchy 只表达语义 drill path，不表达 join / lookup path
...
### 5. analysis contract 只定义维度自身能力，不替代组合校验

例如 `supports_decomposition = true` 的意思是“这个轴在维度语义上适合被分解使用”，但最终是否合法仍取决于：

- metric 的 `additivity`
- metric 的 `required_dimensions / forbidden_dimensions`
- process 是否导出该维度
...
metric 通过以下方式消费 dimension：

- `required_dimensions`
- `forbidden_dimensions`

metric 只声明“该 measurement contract 对哪些维度有要求或限制”，不重新定义维度本身的值域、层级或 drill path。
...
```json
{
  "header": {
    ...
    "quality_gate_refs": ["gate.country_code_normalized"],
    "revision": 2,
  ...
  "interface_contract": {
    "value_domain": {
      ...
      "null_policy": "unknown_bucket",
      "other_bucket_policy": "optional"
    },
    "analysis_contract": {
      "supports_grouping": true,
      "supports_decomposition": true,
      "supports_detection_split": true,
      "recommended_cardinality_band": "medium"
    }
  }
}
```
...
```json
{
  ...
  "interface_contract": {
    "value_domain": {
      ...
      "enum_version": "v1",
      "null_policy": "forbid_null",
      "other_bucket_policy": "not_supported"
    },
    "analysis_contract": {
      "supports_grouping": true,
      "supports_experiment_split_export": true,
      "recommended_cardinality_band": "low"
    }
  }
}
```
...
4. 对齐 compiler
   - `request_dimensions` 先解析 dimension，再做 metric/process/entity 交叉校验
5. 最后再补维度 binding
   - 把 source object、field path、lookup/as-of 规则放到 typed binding
