...
这组文档整体关注的是 Factum 的 **semantic layer / compiler-facing contracts**：它们定义语义对象、对象之间的组合边界，以及这些对象如何被编译为内部 IR。它们**不是**当前实现说明，也**不是**外部 HTTP wire spec。外部接口以 `docs/api/` 为准，intent 设计以 `docs/analysis/intents/` 为准。
...
1. **对象层**：`entity`、`dimension`、`metric`、`process object`
2. **组合层**：metric/process 如何分工、如何组合、哪些组合允许
3. **编译层**：如何把对象组合编译为 IR，并与 lowering / execution 解耦
...
| [`metric-v2-schema.zh.md`](./metric-v2-schema.zh.md) | `metric` 的目标 schema | metric 应承载哪些 measurement contract，哪些内容不应再放进 metric |
| [`process-object-schema.zh.md`](./process-object-schema.zh.md) | `process object` 的目标 schema | 过程对象如何声明稳定接口、subtype 如何建模、哪些执行细节不能泄漏到 public schema |
...
| [`typed-binding-contract.zh.md`](./typed-binding-contract.zh.md) | 语义对象到物理层的绑定契约 | semantic refs 如何映射到底层 source objects / fields / join relations，同时保持执行解耦 |
...
这四篇分别定义 semantic layer 中最核心的四类对象：
...
- field binding 的核心是 **contract target**
...
| 设计 semantic mapping / source binding / physical grounding 的人 | `typed-binding-contract` |
...
- **semantic / physical 分层**：对象层表达“是什么”，binding 表达“如何落地”，compiler/IR 表达“如何组合与编译”。
...
> `entity` 定义“谁”，`dimension` 定义“按什么轴观察”，`metric` 定义“量什么”，`process object` 定义“总体/过程怎样形成”，`binding` 定义“这些语义对象如何落地到底层”，而 compiler / IR 定义“它们如何被安全地组合成分析计划”。
