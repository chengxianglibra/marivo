...
The default `active_propositions` sort key is fixed as `default_active_proposition_order_v1`, which maps to the canonical ordering defined in [`docs/analysis/evidence-engine/schemas/state-surface-schema.md`](../analysis/evidence-engine/schemas/state-surface-schema.md).
