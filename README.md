...
- **Deterministic evidence packaging** that converts SQL results into:
  ...
  - claims with confidence scoring, `status` (`tentative`/`confirmed`/`insufficient`), and `inference_level` (`L0`=correlation; `L1`=cross-scope or cross-slice correlation; `L2`=temporal precedence; `L3`=mechanistic explanation; `L4`–`L5` reserved for stronger causal evidence). Incremental synthesis runs after every primitive step and only maintains tentative claims plus inference-level upgrades
...
- **Deterministic causal checkers** — `CrossSliceConsistencyChecker` (L0→L1), `CrossScopeCorrelationChecker` (L0→L1), `TemporalPrecedenceChecker` (L1+→L2), `MechanisticExplanationChecker` (→L3), `DoseResponseChecker` (bonus justification), and `ReversalChecker` (bonus justification) run automatically after each incremental synthesis step to upgrade claim `inference_level` without LLM involvement.
