...
- **Deterministic evidence packaging** that converts SQL results into:
  ...
  - claims with confidence scoring, `status` (`tentative`/`confirmed`/`insufficient`), and `inference_level` (`L0`=correlation; `L1`=cross-scope, cross-slice, or cross-metric consistency; `L2`=temporal precedence; `L3`=mechanistic explanation; `L4`–`L5` reserved for stronger causal evidence). Incremental synthesis runs after every primitive step and only maintains tentative claims plus inference-level upgrades
  ...
  - recommendations with priority/risk/impact and `causal_basis` metadata (inference level, confounders, suggested validation, claim-graph `causal_chain`), generated only during final synthesis from confirmed claims
...
- **Deterministic causal checkers** — `CrossSliceConsistencyChecker` (L0→L1), `CrossScopeCorrelationChecker` (L0→L1), `CrossMetricCorrelationChecker` (L0→L1 from relation-backed multi-metric groups), `TemporalPrecedenceChecker` (L1+→L2), `MechanisticExplanationChecker` (→L3), `DoseResponseChecker` (bonus justification), and `ReversalChecker` (bonus justification) run automatically after each incremental synthesis step to upgrade claim `inference_level` without LLM involvement.
...
- **Causal inference levels**: `inference_level` on claims is upgraded deterministically by causal checkers running after each incremental synthesis. L0 = correlation; L1 = cross-scope, cross-slice, or cross-metric consistency signal; L2 = temporal precedence; L3 = mechanism. L4–L5 are reserved for confounder elimination and experimental confirmation.
