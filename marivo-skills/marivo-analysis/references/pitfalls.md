...
## Range-style `script_refs` in the report artifact

**Symptom:** the rendered HTML report's Audit Trail shows links to filenames
like `scripts/trino_anomaly_step1-7.py` or `scripts/step4-6.py` — paths that
do not exist on disk. Clicking the link 404s.

**Cause:** `SourceProvenance.script_refs` or `FlowStep.script_refs` was given
one fabricated "range" filename to stand in for several real scripts. The
auditor collapsed "steps 1 through 7" into the invented string
`scripts/trino_anomaly_step1-7.py` instead of enumerating the seven real
files.

**Action:** list each real script path explicitly. Both `script_refs` fields
are tuples of paths — one entry per file that exists.

```python
# Wrong — fabricated aggregate, file does not exist.
provenance = mv.SourceProvenance(
    ...,
    script_refs=("scripts/trino_anomaly_step1-7.py",),
)

# Right — enumerate every real file.
provenance = mv.SourceProvenance(
    ...,
    script_refs=(
...
The same rule applies to `FlowStep.script_refs`: if a step is implemented by
`step4.py`, `step5.py`, and `step6.py`, list all three — not the invented
`"scripts/step4-6.py"`. The Audit Trail renders each entry as its own
clickable link, so a real per-file list is both correct and more useful to
readers.

**Library check:** `validate_report_artifact(artifact, script_root=<dir>)`
cross-checks every `script_refs` entry against the filesystem relative to
`script_root` and emits a `script_ref_missing` issue per broken path. Call it
with the session directory the relative paths resolve against before
rendering — agent-generated scripts live under
`session.layout.scripts_dir`, so `script_refs` like `"scripts/foo.py"`
resolve against `session.layout.session_dir`:

```python
import marivo.analysis as mv
from marivo.analysis import validate_report_artifact

session = mv.session.current()  # or mv.session.get_or_create(name=...)
result = validate_report_artifact(artifact, script_root=session.layout.session_dir)
if not result.ok:
    raise SystemExit(f"artifact has broken script_refs: {result.issues}")
```
