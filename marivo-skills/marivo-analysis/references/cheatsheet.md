...
| Confirm metric ids | `import marivo.semantic as ms; catalog = ms.load(); catalog.list("<model>", kind="metric")` |
...
ids from metric display names; call `catalog.list("<model>", kind="metric")` after loading the
semantic project.
