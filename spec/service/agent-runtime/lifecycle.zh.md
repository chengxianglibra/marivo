...
manifest schema 见 [`manifest-schema.zh.md`](./manifest-schema.zh.md)。CLI 命令语义见 [`cli-contract.zh.md`](./cli-contract.zh.md)。错误结构见 [`error-taxonomy.zh.md`](./error-taxonomy.zh.md)。配置语义见 [`config-contract.zh.md`](./config-contract.zh.md)。工作区布局见 [`workspace-layout.zh.md`](./workspace-layout.zh.md)。
...
| 契约 | 本文与其关系 |
|------|-------------|
| [`manifest-schema.zh.md`](./manifest-schema.zh.md) | manifest schema 契约定义了 `runtime.json` 的校验链和字段规则；本文定义何时读取、如何判定状态、何时复用或重启。本文引用其校验链和错误映射 |
| [`cli-contract.zh.md`](./cli-contract.zh.md) | CLI 命令契约定义了 `serve-local` 的退出码和 stdout JSON；本文将这些退出码映射到 `TargetResolutionError`。`runtime stop` 的清理语义来自 CLI 契约的副作用定义 |
| [`error-taxonomy.zh.md`](./error-taxonomy.zh.md) | 错误 taxonomy 定义了 `runtime_manifest_invalid`、`local_runtime_start_failed`、`workspace_root_required` 的结构；本文定义触发这些错误的具体条件和 `detail` 字段填充规则 |
| [`config-contract.zh.md`](./config-contract.zh.md) | 配置契约定义了 `start_timeout_ms`、`healthcheck_timeout_ms`、`local_host`、`local_port`；本文定义这些超时值在启动流程中的使用语义 |
| [`workspace-layout.zh.md`](./workspace-layout.zh.md) | 工作区布局定义了 `runtime.json` 和 `marivo.pid` 的文件生命周期；本文定义这些文件在运行时发现和生命周期管理中的角色 |
