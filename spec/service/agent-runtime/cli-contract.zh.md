...
v1 产品边界与用户心智模型见 [`scope-note.zh.md`](./scope-note.zh.md)。配置语义见 [`config-contract.zh.md`](./config-contract.zh.md)。workspace root 解析见 [`workspace-root.zh.md`](./workspace-root.zh.md)。错误结构见 [`error-taxonomy.zh.md`](./error-taxonomy.zh.md)。HTTP MCP 适用边界见 [`http-mcp-boundary.zh.md`](./http-mcp-boundary.zh.md)。工作区布局见 [`workspace-layout.zh.md`](./workspace-layout.zh.md)。
...
| 契约 | 本文与其关系 |
|------|-------------|
| [`scope-note.zh.md`](./scope-note.zh.md) | 1.1 定义了 `marivo core` CLI 的职责范围（本地运行时管理）；本文定义具体命令面与语义 |
| [`config-contract.zh.md`](./config-contract.zh.md) | 1.2 定义了 `MARIVO_LOCAL_HOST` 等高级配置字段；本文定义这些字段如何通过 CLI 标志覆盖 |
| [`workspace-root.zh.md`](./workspace-root.zh.md) | workspace root 解析文档定义了优先级链；本文的 `--workspace-root` 标志是优先级链的第一级 |
| [`error-taxonomy.zh.md`](./error-taxonomy.zh.md) | 错误 taxonomy 定义了 `TargetResolutionError` 的结构；本文定义 CLI 退出码到 T1.4 错误码的映射 |
| [`workspace-layout.zh.md`](./workspace-layout.zh.md) | 工作区布局文档定义了 `.marivo/` 内各文件的语义；本文定义各命令对这些文件的创建/读取/写入/删除行为 |
