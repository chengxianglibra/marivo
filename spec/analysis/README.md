...
以下旧文档的主线职责已迁移到 `spec/analysis/evidence-engine/`，兼容页已清理：
...
为避免上下游文档各自发明平行命名，`spec/analysis/` 统一采用以下基线：
...
- `spec/analysis/` 负责设计原则、canonical schema 与 typed intent 语义；若需要对外 HTTP wire contract，应写入 `docs/api/`，而不是在本目录把设计准则写成接口参考
