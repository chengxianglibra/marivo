...
- **可引用**：能被 `metric`、`process object`、entity binding contract 直接引用
...
- `binding.*`：entity typed binding 中的受治理绑定锚点，如 `binding.user_identity`
...
### 3. identity 应以语义键引用表达，由 entity binding 落地
...
1. `entity_ref` 是稳定身份锚点
2. `key_refs` 定义身份组成（语义键，非物理）
3. `uniqueness_scope` 和 `id_stability` 自包含
4. `primary_time_ref` 引用 time taxonomy，不依赖 binding
5. `stable_descriptors` 引用 dimension taxonomy，不依赖 binding
...
| --- | --- | --- | --- |
| `entity_ref` | string | yes | 可被 metric / process / compiler 引用的稳定实体标识；必须使用 `entity.*`；也是 public contract 主标识 |
...
本轮 public entity contract 只保留**稳定描述属性**。若某属性需要 `as_of` 历史语义或 snapshot 解释，它不再进入 `stable_descriptors`，而应交给 binding / process / 独立 attribute history 设计处理。
...
- Population subject 映射在 binding 或 compiler 层处理
...
以下示例按收敛后的 public entity contract 展示，entity 定义独立于 binding。
...
> entity 应是 semantic layer 中“业务实体身份”的稳定锚点；总体主体属于 `subject.*`，样本粒度属于 `grain.*`，过程属于 process object，物理落地只属于 entity binding，组合与 lowering 属于 compiler。
