...
`metric_ref` 是 metric 的 stable semantic identity，例如 `metric.avg_blocked_time`。下游 typed intent、compiler profile、artifact lineage 应继续引用这个 stable ref；若需要物理字段，应引用 entity field，而不是依赖 metric 自己的 binding 或服务端内部 object id。
...
同一 `metric_ref` 可以拥有多个 revision，但最多只有一个 latest active revision 参与默认解析。

| 状态 | 含义 | 默认解析 |
| --- | --- | --- |
| `draft revision` | 基于某个 active base revision 创建、尚未激活的候选定义 | 不参与 |
| `latest active revision` | 当前默认可用定义 | 参与 |
| `superseded revision` | 曾经 active，但已被更新 revision 替代 | 不参与，仍可显式读取 |
| `deprecated identity` | 整个 `metric_ref` 退出新引用 | 不参与 |

激活新 revision 时，旧 active revision 不删除，也不通过 `deprecated` 表达普通替换；旧 revision 进入 `superseded` 或等价的非默认 active 状态。`deprecated` 只表示该 `metric_ref` 整体不再建议新引用，并且仍保留 ref ownership。
...
| Method | Path | 用途 |
| --- | --- | --- |
| `POST` | `/semantic/metrics/{metric_id_or_ref}/revisions` | 基于现有 metric 创建同 ref draft revision |
| `GET` | `/semantic/metrics/{metric_ref}/revisions` | 列出该 stable identity 的 revision history |
| `GET` | `/semantic/metrics/{metric_ref}/revisions/{revision}` | 显式读取历史或当前 revision |
| `POST` | `/semantic/metrics/{metric_id_or_ref}/revisions/{revision}/validate` | 校验 draft revision，不改变默认解析 |
| `POST` | `/semantic/metrics/{metric_id_or_ref}/revisions/{revision}/activate` | 原子切换 latest active revision |
...
`compatible` 适用于 display name、description、unit label、文档化 semantics 文案等不改变 entity-field grounding 和 compiler contract 的修订。`breaking` 适用于 metric family、required input slot、observed entity、observation grain、primary time、sample kind、value semantics、additivity constraints 等会影响 entity binding resolution、profile 或编译结果的修订。
...
底层事实的 late arrival、incomplete-window 行为、外部锚点导入等物理消费细节，属于 entity binding / compiler policy，而不属于 metric 本体。
...
其 schema 由 `spec/semantic/compiler-compatibility-profile.zh.md` 唯一定义，主要包含：
...
Metric component 直接使用 `input_field_ref` 引用 `entity.<entity>.field.<field>`。
不同 `metric_family` 需要的 component slot 如下；这些 slot 不再通过 metric binding
映射物理字段，而是由 metric contract 声明业务消费角色，并通过 entity binding
解析物理来源。

| metric_family | required component slot |
| --- | --- |
...
以下示例只展示 metric public contract。若需要声明对 process 的编译期要求，应通过独立 compiler profile 发布，见 `spec/semantic/compiler-compatibility-profile.zh.md`。
...
- 若 rate metric 依赖实验或归因窗口，窗口本体仍应来自 process object / metric-process contract；metric contract 只声明 component 消费角色，具体字段通过 `input_field_ref` 指向 entity field
