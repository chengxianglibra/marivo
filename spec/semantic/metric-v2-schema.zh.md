...
`metric_ref` 是 metric 的 stable semantic identity，例如 `metric.avg_blocked_time`。下游 typed intent、binding、compiler profile、artifact lineage 应继续引用这个 stable ref，而不是依赖服务端内部 object id。
...
`compatible` 适用于 display name、description、unit label、文档化 semantics 文案等不改变 runtime grounding 和 compiler contract 的修订。`breaking` 适用于 metric family、required input slot、observed entity、observation grain、primary time、sample kind、value semantics、additivity constraints 等会影响 binding、profile 或编译结果的修订。
