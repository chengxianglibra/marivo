...
- `spec/semantic/metric-process-contract.zh.md`
...
- `spec/analysis/intents/primitive-intent-design.md`
- `spec/analysis/intents/derived-intent-design.md`
...
- entity binding 提供 entity-only semantic grounding 与 physical carrier contract
...
4. entity binding contracts（由 catalog 解析得到，内部包含 carrier 信息）
...
- entity binding 是 compiler 在对象解析阶段补全的 grounding 输入，并携带所需 carrier metadata；metric/process/predicate/dimension/time 不提供自己的 physical binding
- governance context 包括 quality / freshness / engine capability / policy 信息
- compiler compatibility profile 的 envelope、发布形态与缺失时语义由 `spec/semantic/compiler-compatibility-profile.zh.md` 定义
...
- request `scope` 只能进一步收窄 metric / entity binding / governance 已声明的过滤约束；无法证明 narrowing 时应 fail closed
...
若需要声明对 process 的编译期要求（如 `contract_modes`、`context_kinds`），应通过独立的 compiler compatibility profile 发布，见 `spec/semantic/compiler-compatibility-profile.zh.md`。
...
若需要声明 inferential 能力（如 `inferential_ready`、`supported_sample_summaries`），应通过独立的 compiler compatibility profile 发布，见 `spec/semantic/compiler-compatibility-profile.zh.md`。
...
### entity binding -> normalized grounding contract

最少应抽取：

- `binding_ref`
- `bound_object_ref`（必须是 active `entity.*`；metric/process scope 仅允许历史诊断）
- `carrier_bindings`
...
- 枚举是否合法
- `scope` 是否含非法时间条件
- `qualifier_refs` / `default_predicate_refs` / `row_filter_refs` / `scope.predicate_ref` 引用的 predicate 是否声明了匹配的 `allowed_usage`（formal specification 见 `predicate-schema-contract.zh.md` "Usage 校验"）
- predicate effective scope 合成是否违反扁平化禁止规则（`default_predicate_refs` 与 `qualifier_refs` 不得合并为单个全局 predicate；见 `predicate-schema-contract.zh.md` "扁平化禁止规则"）
- 结果模式与 shape 组合是否非法
- ref 类型是否匹配当前 intent
...
若 compiler 无法从 object contract 推导某条关键规则，且该 intent 明确依赖它，应返回 `COMPILER_PROFILE_MISSING`。profile 缺失时的固定原则见 `spec/semantic/compiler-compatibility-profile.zh.md`。
...
### Gate 4：entity-grounding compatibility gate

重点校验：

- 所需 entity `binding_ref` 是否存在
- 所需 `carrier_binding` 是否存在
- entity binding 引用的 `surface_ref` 是否都存在于对应 carrier
- `source_object_ref` 是否存在，或 `carrier_locator` 是否可稳定解析到已同步 `source_objects`
- 所需 relation surface / time surface 是否存在
- `carrier_kind` 是否满足 entity grounding 与 intent 的消费前提
...
- 对 `metric` 发起的请求中，`dimension.*` 必须属于当前 metric 的可消费集合；该集合由 metric contract / process exported dimensions / entity stable descriptors 共同推导
- dimension bridge 的 entity anchor 判定顺序固定为：
...
- dimension bridge 只允许消费满足以下条件的来源：
  - 当前 metric/process 组合可唯一确定 entity anchor
  - entity contract 将该 dimension 声明为 stable descriptor，或 process contract 将其声明为 exported dimension
  - 该 dimension 若需要具体字段，则通过 `source_field_ref` 或 entity stable descriptor 指向 `entity.<entity>.field.<field>`
  - entity binding 能解析该 field 的 physical grounding
- 同一 `dimension_ref` 若可由多个 entity anchors 提供，compiler 必须在本 gate fail-fast；不得按隐式优先级裁决来源
- 若请求使用的 `dimension.*` 既不属于 metric/process 可消费集合，也不属于 entity stable descriptor 集合，则必须在 compiler 阶段失败，而不是推迟到 planner 或 engine
...
### Gate 5.5：predicate contract gate（由 predicate_validator 执行）

校验：

- 所有 `qualifier_refs`、`default_predicate_refs`、`row_filter_refs`、`scope.predicate_ref` 引用的 predicate 是否可解析且 active/ready
- predicate `allowed_usage` 是否匹配消费点（`metric_qualifier` / `carrier_row_filter` / `request_scope` / `governance_policy`）
- predicate expression 是否 deterministic、non-time、conjunctive
- request scope 是否只收窄上游过滤（narrowing proof）
- 跨层 predicate 冲突检测（governance vs metric、entity carrier vs qualifier、within-metric、cross-component）
- predicate effective scope 合成是否满足扁平化禁止规则

详细校验规则与 error code 映射见 `app/analysis_core/predicate_validator.py`。
