...
若需要查看 `intent` / `metric` / `process object` 如何进入 compiler 并被转换为 IR，可配合阅读 `spec/semantic/compiler-spec.zh.md`。
...
- compiler profile / governance decision 若需要保留，应作为单独的 compile artifacts 或 snapshot refs 存在，而不应在 IR 中重新定义一套 process/metric 语义结构；profile 本体定义见 `spec/semantic/compiler-compatibility-profile.zh.md`
...
- `spec/analysis/intents/` 中的 typed intent schema
