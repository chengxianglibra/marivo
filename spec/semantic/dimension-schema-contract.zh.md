...
- **可迁移**：允许 runtime 先从字符串数组迁移到 `dimension.*` 引用，并通过 entity field refs 逐步接入物理 grounding
...
- `source_field_ref`：可选的语义字段来源引用；必须指向
  ...
  target，不表达物理绑定；物理落地仍由 entity binding 负责。
...
metric 在最小 public contract 中不直接维护维度兼容矩阵。若系统需要更细的维度限制或版本 pinning，应交给 compiler compatibility profile，而不是回写到 dimension 主 contract；profile 的发布形态与 compiler 消费语义见 `spec/semantic/compiler-compatibility-profile.zh.md`。
...
`source_field_ref` 是 dimension 触达字段的唯一入口。它引用 `entity.field`，不引用物理列；
物理列、source object、join path 和表达式 locator 仍只属于 entity grounding。实现必须拒绝
dimension contract 中的 `physical_column`、carrier、binding target 等对象级物理绑定字段。
...
```json
{
  ...
  "interface_contract": {
    "source_field_ref": "entity.experiment_assignment.field.variant",
    "value_domain": {
        "structure_kind": "flat",
...
```json
{
  ...
  "interface_contract": {
    "source_field_ref": "entity.user.field.lifecycle_state",
    "value_domain": {
      "structure_kind": "hierarchical",
      "semantic_role": "state",
      "value_type": "string",
...
5. 最后补 entity grounding
   - 把 source object、field path、lookup/as-of 规则与时间锚点映射放到 entity binding

一言以蔽之：

> dimension 应是 semantic layer 中“共享分析轴”的稳定锚点；entity 负责字段归属与物理落地，metric 负责使用边界，process object 负责导出边界，compiler 负责组合校验。
