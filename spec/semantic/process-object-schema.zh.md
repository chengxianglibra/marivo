...
本文定义 Marivo semantic layer 中 `process object` 的目标 schema 方向。
...
- 若仍需 capability 词表，应独立成 compiler compatibility profile，而不是直接混入 public schema；其发布形态与字段边界见 `spec/semantic/compiler-compatibility-profile.zh.md`
...
**由 compiler profile 补充（无法推导，见 `spec/semantic/compiler-compatibility-profile.zh.md`）：**
...
若仍需要更细粒度 inferential 能力声明（如 `inferential_ready`、`supported_sample_summaries`），应放入独立的 compiler compatibility profile，见 `spec/semantic/compiler-compatibility-profile.zh.md`。
...
下面的 subtype 示例只展示 process public contract。若需要声明 inferential 能力，应通过独立 compiler profile 发布，见 `spec/semantic/compiler-compatibility-profile.zh.md`。
