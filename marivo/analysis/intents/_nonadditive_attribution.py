...
def _resolution_rows(
    ...
    *,
    ...
    for column in all_axis_columns[level:]:
        out[column] = pd.NA
    out.insert(0, "path", out[list(prefix_columns)].astype(str).agg(" > ".join, axis=1))
    out.insert(0, "driver", out[prefix_columns[-1]])
    out.insert(0, "axis", prefix_columns[-1])
    out.insert(0, "level", level)
    ordered = ["level", "axis", "driver", "path", *all_axis_columns]
