    ...
    descriptors.extend(
        (
            common(
                ...
                requires=("metric",),
                effects="Source execution supports DuckDB and admitted PostgreSQL, MySQL and SQLite scalar Metrics on one unversioned table; physical types and table scope are backend-specific. All source methods use the same implementation registry; concrete adapters own preparation and resource effects. Local methods run in the caller without execution budgets; original exceptions propagate. Source queries may observe different source states; required checks and atomic publication remain enforced. An execution-key hit recovers the existing snapshot, not fresh source rows. For current source rows, use a new named Session or change an explicit row-affecting input.",
                runtime=True,
            ),
            common(
                "actions.show",
