    ...
    descriptors.extend(
        (
            common(
                ...
                requires=("metric",),
                effects="Source execution supports DuckDB and admitted PostgreSQL, MySQL, SQLite, Trino and ClickHouse scalar and individually admitted relational/date methods; physical types, method shapes and table scope are backend-specific. UInt inputs on MySQL/ClickHouse preserve unsigned values; SUM retains its int64 range. Boolean storage is validated. Qualified native timestamps support typed predicates, hour/day buckets and explicit timezones through microseconds; ambiguous local times and lossy conversions fail. Source column checks use the complete required dependency closure, including hidden computations and identity, relationship and version assertions; unused columns do not impose execution type restrictions. Source relations are not restricted by table form: views and every engine or connector type enter, and Trino rejects only `$`-suffixed internal tables. Remote sources require read-only accounts; remote retained import and uploads are unsupported. Datasource connectivity and semantic readiness do not prove method support. Unsupported methods reject with a concrete reason before source work. All source methods use the same implementation registry; concrete adapters own preparation and resource effects. Local methods run in the caller without execution budgets; original exceptions propagate. Source queries may observe different source states; engines whose reads depend on background merge state (for example ReplacingMergeTree) are accepted, results can change with merge progress between runs, and replay of a committed snapshot is unaffected. Required checks and atomic publication remain enforced. An execution-key hit recovers the existing snapshot, not fresh source rows. For current source rows, use a new named Session or change an explicit row-affecting input.",
                runtime=True,
            ),
            common(
                "actions.show",
