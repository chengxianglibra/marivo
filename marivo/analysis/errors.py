    ...
    def _template_fields(self) -> dict[str, str]:
        ...
        return {
            ...
            "fix_snippet": (
                ...
                "catalog = ms.load()\n"
                "catalog.list(kind=ms.SemanticKind.METRIC)  # confirm the exact id\n"
...
class WindowInvalidError(AnalysisError):
    def _template_fields(self) -> dict[str, str]:
        window = self.details.get("window") or self.details.get("time_scope")
        ...
        return {
            ...
            "cause": f"time_scope={window_ref} could not be parsed.",
            "fix_snippet": (
    ...
    def _template_fields(self) -> dict[str, str]:
        ...
        if case == "unexpected_calendar":
            return {
                ...
                "fix_snippet": "mv.window_bucket()  # no calendar argument",
                "doc": "marivo/skills/marivo-analysis/references/pitfalls.md",
            }
        return {
            "location": "mv.AlignmentPolicy(...)",
        ...
        if catalog_misses:
            return {
                ...
                "fix_snippet": (
                    "import marivo.semantic as ms\n"
                    "catalog = ms.load()\n"
                    "catalog.list(kind=ms.SemanticKind.METRIC).show()  # pick a defined metric id, then re-promote"
                ),
                "doc": "marivo/skills/marivo-analysis/references/pitfalls.md",
            }
        return {
...
class TestAlignmentError(AlignmentFailedError):
    def _template_fields(self) -> dict[str, str]:
        return {
            ...
            "fix_snippet": "session.hypothesis_test(cur, base, alignment=mv.window_bucket())",
            "doc": "marivo/skills/marivo-analysis/references/pitfalls.md",
        }
...
class ForecastShapeUnsupportedError(AnalysisError):
    def _template_fields(self) -> dict[str, str]:
        return {
            ...
            "fix_snippet": 'history = session.observe(session.catalog.get("metric.sales.revenue"), time_scope={"start": "2026-01-01", "end": "2026-04-01"}, grain="day")\nsession.forecast(history, horizon=30)',
            "doc": "marivo/skills/marivo-analysis/references/pitfalls.md",
        }
    ...
    def _template_fields(self) -> dict[str, str]:
        ...
        return {
            ...
            "fix_snippet": (
                "import marivo.semantic as ms\n"
                "catalog = ms.load()\n"
                "catalog.list(kind=ms.SemanticKind.DIMENSION)  # confirm available dimensions per entity\n"
