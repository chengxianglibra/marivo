...
| Confirm metric ids | `import marivo.semantic as ms; catalog = ms.load(); catalog.metrics.ids()` |
...
ids from metric display names; call `catalog.metrics.ids()` after loading the
semantic project.
