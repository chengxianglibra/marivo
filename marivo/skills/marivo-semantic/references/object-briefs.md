...
ms.help('DomainBrief')          # one per kind: DomainBrief, EntityBrief,
                                # DimensionBrief, TimeDimensionBrief, MeasureBrief,
                                # MetricBrief, RelationshipBrief,
                                # CrossEntityMetricBrief, DerivedMetricBrief
```
...
`prepare_measure`, `prepare_metric`, `prepare_relationship`, and
`prepare_cross_entity_metric` require their entity arguments to have passed
`verify_object`. Calling these without a prior `verify_object` raises
`LadderOrderError` with a hint naming the missing prerequisite. This guard is
fingerprint-based: if an entity's source changes after verification, the old
verification is stale and must be re-run.
