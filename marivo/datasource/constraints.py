...
CONSTRAINTS: dict[ConstraintId, Constraint] = {
    ConstraintId.DATASOURCE_NAME_GLOBAL: _constraint(
        ...
        "decorator",
        ("DatasourceSpec", "DatasourceRef", "datasource", "ref"),
    ...
    ConstraintId.DATASOURCE_BACKEND_TYPE_REQUIRED: _constraint(
        ...
        "DatasourceSpec requires a non-empty backend_type.",
        "The runtime needs backend_type to select an ibis adapter and validate backend-specific fields.",
        "Pass backend_type='duckdb', backend_type='trino', or another supported backend adapter id.",
        docs_ref=_DATASOURCE_DOC,
    ),
    ...
    ConstraintId.DATASOURCE_SECRET_ENV_REF: _constraint(
        ...
        example='md.DatasourceSpec(name="warehouse", backend_type="trino", password_env="TRINO_PASSWORD")',
        docs_ref=_DATASOURCE_DOC,
    ),
    ConstraintId.DATASOURCE_LOADER_CONTEXT: _constraint(
        ...
        "decorator",
        ("datasource",),
        "md.datasource can only register while loading .marivo/datasource files.",
        "Datasource declarations are collected by the project loader, not registered into global process state.",
        "Put datasource declarations under .marivo/datasource/*.py and load them with md.load_datasources(...).",
        docs_ref=_DATASOURCE_DOC,
    ),
    ConstraintId.DATASOURCE_UNIQUE_NAME: _constraint(
        ...
        "DatasourceDuplicate",
        "assembly",
        ("DatasourceSpec", "datasource", "load_datasources"),
        "Datasource names must be unique within a project.",
        "Duplicate project-level datasource ids make source references ambiguous.",
        "Rename one datasource file entry or merge duplicate declarations into one DatasourceSpec.",
        docs_ref=_DATASOURCE_DOC,
    ),
