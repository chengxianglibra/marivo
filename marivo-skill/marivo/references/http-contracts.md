...
- `relation_not_found`: browse the datasource and update `dataset.source` to the live relation FQN
- `field_expression_invalid`: update `field.expression.dialects[]` for the target datasource dialect
- `datasource_not_ready`: repair datasource connection/configuration before retrying semantic readiness
