...
- `plan/omnidb-design-doc.md`：英文设计文档（当前实现全量描述）
- `plan/omnidb-design-doc.zh.md`：中文设计文档（当前实现全量描述）
...
- **MCP 层已移除**：MCP server/client 已从代码库完全删除，OmniDB 目前是纯 HTTP API 服务
...
HTTP、UI 只是协议与交互层，不承载分析领域逻辑。（MCP 层已从代码库移除，OmniDB 目前是纯 HTTP API 服务。）
