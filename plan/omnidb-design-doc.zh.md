...
- 聚合当前 session 的所有 observations
- 生成 claims，连接 evidence edges（supports / contradicts）
- 产出 recommendations，自动标记高风险项需要审批
- 持久化完整 evidence graph

### 17.3 Evidence Engine 架构

`app/evidence_engine/` 分为三层：

- **Extractor 层**：`ComparisonRowExtractor`、`AggregateRowExtractor` — 从 artifact 中确定性提取 observations
- **Scoring 层**：`app/evidence_engine/scoring.py` — 确定性置信度评分（effect strength、consistency、sample size、data quality、contradiction penalty）
- **Synthesizer 层**：`app/evidence_engine/synthesizers/` — 将 observations 合并为 claims 与 recommendations

## 18. 协议层设计

### 18.1 当前状态：纯 HTTP API

MCP server/client 层已从代码库完全移除（commit 3ebf377）。OmniDB 目前是纯 HTTP API 服务，Agent、UI 和外部工具均直接通过 FastAPI endpoints 交互。
...
- `OMNIDB_CONFIG` — YAML 配置文件路径（默认 `omnidb.yaml`）
...
- 哪些 evidence-extraction 规则可以引入 LLM 辅助（目前全为确定性）？
